...
use crate::request::{ZhtpRequest, ZhtpResponse, ZhtpHeaders};
...
pub fn restore_identity_from_seed_wasm(master_seed: &[u8], device_id: &str) -> Result<JsValue, JsError> {
    let identity = crate::identity::restore_identity_from_seed(master_seed.to_vec(), device_id.to_string())
        .map_err(|e| js_err(&e.to_string()))?;
...
pub fn sign_registration_proof_wasm(identity: &JsValue, timestamp: u64) -> Result<Uint8Array, JsError> {
    let identity = js_to_identity(identity)?;
...
pub fn sign_message_wasm(identity: &JsValue, message: &[u8]) -> Result<Uint8Array, JsError> {
    let identity = js_to_identity(identity)?;
    let signature = crate::identity::sign_message(&identity, message)
        .map_err(|e| js_err(&e.to_string()))?;
...
pub fn verify_signature_wasm(public_key: &[u8], message: &[u8], signature: &[u8]) -> Result<bool, JsError> {
...
pub fn serialize_identity_wasm(identity: &JsValue) -> Result<String, JsError> {
    let identity = js_to_identity(identity)?;
    crate::identity::serialize_identity(&identity)
        .map_err(|e| js_err(&e.to_string()))
}
...
pub fn deserialize_identity_wasm(json: &str) -> Result<JsValue, JsError> {
    let identity = crate::identity::deserialize_identity(json)
        .map_err(|e| js_err(&e.to_string()))?;
...
impl WasmHandshakeState {
    ...
    pub fn create_client_hello(&mut self) -> Result<Uint8Array, JsError> {
        let bytes = self.inner.create_client_hello()
            .map_err(|e| js_err(&e.to_string()))?;
    ...
    pub fn process_server_hello(&mut self, data: &[u8]) -> Result<Uint8Array, JsError> {
        let bytes = self.inner.process_server_hello(data)
            .map_err(|e| js_err(&e.to_string()))?;
        Ok(Uint8Array::from(&bytes[..]))
    }
    ...
    pub fn finalize(&self) -> Result<JsValue, JsError> {
        ...
        let obj = Object::new();
        js_set!(&obj, "sessionKey", &Uint8Array::from(&result.session_key[..]));
        js_set!(&obj, "sessionId", &Uint8Array::from(&result.session_id[..]));
        js_set!(&obj, "peerDid", &JsValue::from_str(&result.peer_did));
        js_set!(&obj, "peerPublicKey", &Uint8Array::from(&result.peer_public_key[..]));
...
impl WasmSession {
    ...
    #[wasm_bindgen(constructor)]
    pub fn new(session_key: &[u8], session_id: &[u8], peer_did: &str) -> Result<WasmSession, JsError> {
...
pub fn serialize_request_wasm(request: &JsValue) -> Result<Uint8Array, JsError> {
    ...
    let bytes = crate::request::serialize_request(&req)
        .map_err(|e| js_err(&e.to_string()))?;
    Ok(Uint8Array::from(&bytes[..]))
}
...
pub fn deserialize_response_wasm(data: &[u8]) -> Result<JsValue, JsError> {
    let response = crate::request::deserialize_response(data)
        .map_err(|e| js_err(&e.to_string()))?;
...
pub fn parse_zhtp_frame_wasm(data: &[u8]) -> Result<Uint8Array, JsError> {
    let payload = crate::request::parse_zhtp_frame(data)
        .map_err(|e| js_err(&e.to_string()))?;
...
pub fn encrypt_oneshot_wasm(key: &[u8], plaintext: &[u8]) -> Result<Uint8Array, JsError> {
    let ciphertext = crate::session::encrypt_oneshot(key, plaintext)
        .map_err(|e| js_err(&e.to_string()))?;
    Ok(Uint8Array::from(&ciphertext[..]))
}
...
pub fn decrypt_oneshot_wasm(key: &[u8], ciphertext: &[u8]) -> Result<Uint8Array, JsError> {
    let plaintext = crate::session::decrypt_oneshot(key, ciphertext)
        .map_err(|e| js_err(&e.to_string()))?;
    Ok(Uint8Array::from(&plaintext[..]))
}
...
fn identity_to_js(identity: &Identity) -> Result<JsValue, JsError> {
    ...
    js_set!(&obj, "publicKey", &Uint8Array::from(&identity.public_key[..]));
    js_set!(&obj, "privateKey", &Uint8Array::from(&identity.private_key[..]));
    js_set!(&obj, "kyberPublicKey", &Uint8Array::from(&identity.kyber_public_key[..]));
...
fn js_to_zhtp_request(js: &JsValue) -> Result<ZhtpRequest, JsError> {
    ...
    // Parse headers
    let headers_js = Reflect::get(js, &"headers".into())
...
fn zhtp_response_to_js(response: &ZhtpResponse) -> Result<JsValue, JsError> {
    ...
    js_set!(&obj, "statusText", &JsValue::from_str(&response.status_text));
    js_set!(&obj, "version", &JsValue::from_str(&response.version));
    js_set!(&obj, "body", &Uint8Array::from(&response.body[..]));
    js_set!(&obj, "timestamp", &JsValue::from_f64(response.timestamp as f64));
    ...
    js_set!(&headers_obj, "contentLength", &JsValue::from_f64(response.headers.content_length as f64));
    js_set!(&headers_obj, "privacyLevel", &JsValue::from_f64(response.headers.privacy_level as f64));
    js_set!(&headers_obj, "encryption", &JsValue::from_str(&response.headers.encryption));
    js_set!(&headers_obj, "daoFee", &JsValue::from_f64(response.headers.dao_fee as f64));
    js_set!(&headers_obj, "networkFee", &JsValue::from_f64(response.headers.network_fee as f64));
...
fn get_string(obj: &JsValue, key: &str) -> Result<String, JsError> {
    let val = Reflect::get(obj, &key.into())
        .map_err(|_| js_err(&format!("Missing field: {}", key)))?;
    val.as_string()
