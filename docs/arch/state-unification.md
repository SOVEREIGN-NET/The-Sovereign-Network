...
| 22 | **Contract code / storage** | `contract_states: HashMap<[u8;32], Vec<u8>>` ([blockchain.rs:122](../../lib-blockchain/src/blockchain.rs#L122)) + `contract_state_history` | `get_contract_code` / `put_contract_code` / `get_contract_storage` / `put_contract_storage` / `delete_contract_storage` | `StateMutator::put_contract_code` / `put_contract_storage` ([tx_apply.rs:391-411](../../lib-blockchain/src/execution/tx_apply.rs#L391)) | `process_contract_transactions` | **Sled** | **DELETE** in-mem |
...
| Field | External access sites | Sub-issue covering migration |
|---|---:|---|
| `oracle_state` | 55 | (DEFERRED — see row 9; needs sled durability story first) |
| `token_contracts` | 36 | [#2637](https://github.com/SOVEREIGN-NET/The-Sovereign-Network/issues/2637) (balances) + cache for metadata |
| `identity_registry` | 31 | [#2639](https://github.com/SOVEREIGN-NET/The-Sovereign-Network/issues/2639) |
| `blocks` (raw) | 22 | [#2636](https://github.com/SOVEREIGN-NET/The-Sovereign-Network/issues/2636) |
| `query_blocks()` callers | 9 | [#2636](https://github.com/SOVEREIGN-NET/The-Sovereign-Network/issues/2636) |
| `pending_transactions` | 20 | (Phase 4 / #2641 — needs `lib_mempool` wiring) |
| `wallet_registry` | 17 | [#2639](https://github.com/SOVEREIGN-NET/The-Sovereign-Network/issues/2639) (after wallet sled wiring) |
| `utxo_set` | 16 | (covered by row 10 — DELETE; new issue or extend #2637) |
| `credential_registry` | 12 | (DEFERRED — no sled tree) |
| `validator_registry` | 12 | [#2639](https://github.com/SOVEREIGN-NET/The-Sovereign-Network/issues/2639) (consensus-critical) |
| `domain_registry` | 4 | (DEFERRED — no sled tree) |
| `web4_contracts` | 4 | (DEFERRED — no sled tree) |
| `nft_collections` | 3 | (DEFERRED — no sled tree) |
| `token_nonces` | 2 | [#2638](https://github.com/SOVEREIGN-NET/The-Sovereign-Network/issues/2638) |
| `gateway_registry` | 1 | (DEFERRED — no sled tree) |
| `observer_registry` | 0 | Already through facade (good — model for the migration) |
...
**Spreadsheet:** [`docs/arch/state-unification-read-sites.csv`](./state-unification-read-sites.csv) — 228 rows across 34 files.

Columns: `file, line, field, class, sub_issue, target_facade, snippet`. Sorted by `sub_issue` then `field` then `file:line` so each Phase 2 PR can grep its rows.

#### How the CSV was built

1. Grep every external root (`zhtp/`, `zhtp-cli/`, `zhtp-daemon/`, `tools/`) for `<binding>.<field>` where `<binding>` is one of `bc | blockchain | b | chain | src | dst | blk | guard | read_guard | write_guard` and `<field>` is one of the 14 duplicate-state fields in §3.
2. Filter to direct Blockchain-bound accesses (shadow struct fields with the same name in handler structs are excluded by the binding-prefix constraint).
3. Classify each snippet by access pattern:
   - **Aggregate** — `.iter() / .values() / .keys() / .len() / .is_empty() / for ... in &x.field`
   - **Recent** — `.last() / .first() / .back() / .front()`
   - **Lookup** — `.get() / .contains() / .field[k] / .field.subfield / .field.method(args)`
   - **Mutate** — `.insert() / .remove() / .push() / .clear() / .field = … / .get_mut() / etc.`
   - **Reference** — bare `&x.field` or `x.field.clone()` (passed by reference; downstream behavior not classified here)

#### Distribution

| Sub-issue | Sites | Field(s) |
|---|---:|---|
| **#2636** | 21 | `blocks` |
| **#2637** | 17 | `token_contracts` (balances + metadata) |
| **#2637+** | 14 | `utxo_set` (extension of #2637 scope or new issue) |
| **#2639** | 55 | `validator_registry`, `identity_registry` |
| **#2639\*** | 23 | `wallet_registry` (blocked on executor wiring per row 6) |
| **#2641** | 20 | `pending_transactions` (post-S2) |
| **DEFER** | 78 | `oracle_state` (55), `credential_registry` (12), `domain_registry` (3), `web4_contracts` (4), `nft_collections` (3), `gateway_registry` (1) |

Class breakdown: 100 Lookup, 81 Aggregate, 28 Mutate, 16 Reference, 3 Recent.

#### Mutate rows are #2640 fuel

The 28 **Mutate**-class rows are external code paths writing directly into Blockchain HashMaps (registries, balance maps, utxo set, etc.) — i.e. the privatization barrier #2640 must enforce. They are not migration targets for #2636–#2639 (which only migrate reads); they are flagged here so #2640 has a candidate list before it lands.

| Sub-issue scope | Mutate sites | Notes |
|---|---:|---|
| `#2639` (validator+identity) | 11 | Most concentrated dirt — direct registry writes from handlers and runtime |
| `#2640` (DEFER reads, Mutate writes) | 7 | External writes into deferred-read fields |
| `#2637` (token) | 5 | Direct `token_contracts` writes outside the executor |
| `#2639*` (wallet) | 3 | Direct wallet_registry writes |
| `#2641` (mempool) | 1 | One direct `pending_transactions` mutate (covered by S2 unwinding) |
| `#2637+` (utxo) | 1 | One direct `utxo_set` write outside the executor |

The Mutate rows surface a phase-ordering question: cleaning them is #2640's job, but if any handler is the *only* writer of a piece of state, deleting it will break that handler. Each #2640 PR must cite the Mutate rows it deletes and prove an equivalent BlockExecutor / StateMutator path exists.
