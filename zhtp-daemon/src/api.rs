...
async fn domains(State(state): State<AppState>) -> Result<Json<serde_json::Value>, AppError> {
...
async fn resolve_query(
    ...
    let resolved = state.service.resolve_domain(&query.domain).await?;
    Ok(Json(resolved))
}
...
async fn resolve_path(
    ...
    let resolved = state.service.resolve_domain(&domain).await?;
    Ok(Json(resolved))
}
...
async fn content_json(
    ...
    let response = state.service.fetch_content(&query.domain, &path).await?;
...
async fn raw_content_root(
    ...
    relay_content(state, domain, "/".to_string()).await
}
...
async fn raw_content(
    ...
    relay_content(state, domain, normalized_path).await
}
...
async fn root_next_asset(
    ...
    relay_root_asset(state, headers, format!("/_next/{}", path)).await
}
...
async fn root_images_asset(
    ...
    relay_root_asset(state, headers, format!("/images/{}", path)).await
}
...
async fn root_favicon(
    ...
    relay_root_asset(state, headers, "/favicon.ico".to_string()).await
}
...
async fn root_site_path(
    ...
    relay_root_asset(state, headers, normalized).await
}
...
async fn relay_root_asset(
    ...
    relay_content(state, domain, path).await
}
...
async fn relay_content(
    ...
    let mut response = state.service.fetch_content(&domain, &path).await?;
