...
use crate::argument_parsing::{format_output, CurveAction, CurveArgs, ZhtpCli};
...
fn parse_curve_type(curve_type: &str) -> CliResult<(u8, u64, u64, Option<u64>)> {
    match curve_type.to_lowercase().as_str() {
        ...
        _ => Err(CliError::ConfigError(format!(
            "Unknown curve type: {}. Use: linear, exponential, or sigmoid",
            curve_type
        ))),
    }
}
        ...
        _ => {
            ...
            if let Ok(amount) = threshold.parse::<u64>() {
                ...
                Err(CliError::ConfigError(format!(
                    "Unknown threshold: {}. Use: standard, low, high, supply, or custom amount",
                    threshold
                )))
            }
        }
...
pub async fn handle_curve_command(args: CurveArgs, cli: &ZhtpCli) -> CliResult<()> {
...
async fn handle_deploy<O: Output>(
    ...
    output.info(&format!(
        "Deploying bonding curve token: {} ({})",
        name, symbol
    ))?;
    output.info(&format!(
        "Curve type: {}, Threshold: {}, Sell enabled: {}",
...
async fn handle_buy<O: Output>(
        ...
        .unwrap_or(false)
    {
        ...
        if let Some(amount) = response_json
            .get("tokens_received")
            .and_then(|v| v.as_u64())
        {
            output.info(&format!("Tokens received: {}", amount))?;
        }
        if let Some(price) = response_json
            .get("effective_price")
            .and_then(|v| v.as_str())
        {
...
/// Handle price query
async fn handle_price<O: Output>(cli: &ZhtpCli, output: &O, token_id: &str) -> CliResult<()> {
...
async fn handle_can_graduate<O: Output>(
    ...
    if let Some(can_graduate) = response_json.get("can_graduate").and_then(|v| v.as_bool()) {
        if can_graduate {
            ...
            if let Some(reserve) = response_json
                .get("current_reserve")
                .and_then(|v| v.as_u64())
            {
...
async fn handle_valuation<O: Output>(cli: &ZhtpCli, output: &O, token_id: &str) -> CliResult<()> {
