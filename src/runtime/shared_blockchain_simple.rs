...
pub struct SharedBlockchainService {
    ...
    operation_tx: mpsc::UnboundedSender<BlockchainOperation>,
}
                ...
                let result = {
                    ...
                    match blockchain.add_pending_transaction(transaction.clone()) {
                        ...
                        Err(e) => {
                            ...
                            Err(format!("Failed to add transaction: {}", e))
                        }
                    }
                };
            ...
            BlockchainOperation::MineBlock { response_tx } => {
                let result = {
                    ...
                    if !blockchain.pending_transactions.is_empty() {
                        ...
                        // Create a new block (simplified for this implementation)
                        let new_block = Block::new(
                            blockchain.height + 1,
                            blockchain.blocks.last().map(|b| b.hash()).unwrap_or_default(),
                            transactions,
                        ...
                        match blockchain.add_block(new_block.clone()) {
                            ...
                            Err(e) => {
                                ...
                                Err(format!("Failed to mine block: {}", e))
                            }
                        }
                    } else {
                        Err("No transactions to mine".to_string())
                    }
                };
                let _ = response_tx.send(result);
            }
