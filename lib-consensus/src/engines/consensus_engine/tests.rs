...
async fn test_consensus_engine_creation_with_broadcaster() {
    ...
    let mock_broadcaster: Arc<dyn MessageBroadcaster> = Arc::new(MockMessageBroadcaster::new());
...
async fn test_broadcast_failure_does_not_affect_consensus() {
    ...
    let vote_count = engine
        ...
        .iter()
        .filter(|(k, _)| {
            k.height == engine.current_round.height && k.round == engine.current_round.round
        })
        .count();
    assert!(
        vote_count > 0,
        "Vote should be stored even if broadcast fails"
    );
}
...
async fn test_multiple_phases_produce_multiple_broadcasts() {
    ...
    assert!(
        ...
        "Expected broadcast in propose phase"
    );
...
async fn test_gap4_message_relevance_invariant() {
    ...
    assert_eq!(
        ...
        0,
        "Irrelevant votes should be ignored"
    );
...
fn test_gap4_timer_token_staleness_detection() {
    ...
    assert!(
        !token.matches(2, 0, &step_propose),
        "Token should not match height 2"
    );

    // Should NOT match different round
    assert!(
        !token.matches(1, 1, &step_propose),
        "Token should not match round 1"
    );
...
async fn test_gap4_receiver_closure_graceful_shutdown() {
    ...
    let mut engine_handle = tokio::spawn(async move { engine.run_consensus_loop().await });
...
fn test_ce_s1_proposal_scoped_quorums_prevent_split_votes() {
    ...
    assert_eq!(
        count_for_a, 0,
        "Empty pool should have 0 votes for proposal A"
    );
    ...
    assert_eq!(
        count_for_b, 0,
        "Empty pool should have 0 votes for proposal B"
    );
    ...
    assert_eq!(
        count_for_c, 0,
        "Empty pool should have 0 votes for proposal C"
    );
...
async fn test_ce_l2_commit_votes_stored_at_any_step() {
    ...
    assert_eq!(
        commit_count, 1,
        "CE-L2: Commit vote should be stored in PreVote step"
    );
...
fn test_hardening_check_supermajority_correct_threshold() {
    ...
    assert!(
        !check_supermajority(4, 7),
        "4/7 should not be supermajority"
    );
    assert!(check_supermajority(5, 7), "5/7 should be supermajority");

    // 100 validators: (100 * 2) / 3 + 1 = 66 + 1 = 67 (integer division)
    assert!(
        !check_supermajority(66, 100),
        "66/100 should not be supermajority"
    );
    assert!(
        check_supermajority(67, 100),
        "67/100 should be supermajority"
    );
}
...
async fn test_hardening_vote_validation_height_mismatch() {
    ...
    assert!(
        ...
        vote.height, engine.current_round.height
    );
}
...
async fn test_hardening_vote_validation_round_mismatch() {
    ...
    assert!(
        ...
        vote.round, engine.current_round.round
    );
}
...
/// Test: Remote vote validation - unsigned votes are rejected
#[tokio::test]
async fn test_hardening_vote_validation_rejects_empty_vote_public_key() {
    let config = ConsensusConfig {
        development_mode: true,
        ..Default::default()
    };
...
/// Test: Remote vote validation - placeholder registered keys are rejected
#[tokio::test]
async fn test_hardening_vote_validation_rejects_placeholder_registered_key() {
    let config = ConsensusConfig {
        development_mode: true,
        ..Default::default()
    };
...
/// Test: NewBlock event path in loop mode must not run deprecated synchronous driver
#[tokio::test]
async fn test_hardening_new_block_event_respects_single_driver_invariant() {
    let config = ConsensusConfig {
        development_mode: true,
        ..Default::default()
    };
    ...
    engine.set_message_receiver(rx);
    let round_before = engine.current_round.round;
    ...
    assert!(
        events
            .iter()
            .any(|e| matches!(e, ConsensusEvent::RoundCompleted { .. })),
        "NewBlock event should emit RoundCompleted"
    );
    assert_eq!(
        engine.current_round.round, round_before,
        "Loop mode NewBlock handling must not execute synchronous round progression"
    );
}
...
async fn test_hardening_commit_vote_accepts_past_round() {
    ...
    assert_eq!(
        commit_count, 1,
        "Past-round commit vote should be stored for catch-up"
    );
}
...
async fn test_canonical_convergence_different_vote_order() {
    ...
    for i in 1..=4 {
        ...
        register_validator_with_keypair(&mut engine_a, validator_id.clone(), &keypair, i == 1)
            .await;
        register_validator_with_keypair(&mut engine_b, validator_id.clone(), &keypair, i == 1)
            .await;
...
async fn test_canonical_convergence_no_quorum_split_votes() {
    ...
    assert_eq!(
        count_a_proposal_a, 2,
        "Node A should count 2 votes for proposal A"
    );
    assert_eq!(
        count_b_proposal_a, 2,
        "Node B should count 2 votes for proposal A"
    );
    ...
    assert_eq!(
        count_a_proposal_b, 1,
        "Node A should count 1 vote for proposal B"
    );
    assert_eq!(
        count_b_proposal_b, 1,
        "Node B should count 1 vote for proposal B"
    );
