...
impl EnhancedBftEngine {
    ...
    pub async fn handle_propose(&mut self, proposal: ConsensusProposal) -> Result<()> {
        ...
        self.verify_consensus_proof(&proposal.consensus_proof)
            .await?;
    ...
    async fn validate_proposal_structure(&self, proposal: &ConsensusProposal) -> Result<()> {
        ...
        if proposal.block_data.len() > 1_000_000 {
            // 1MB limit
        ...
        if proposal.timestamp + 3600 < current_time {
            // 1 hour old
    ...
    async fn verify_proposer_identity_zk(&self, proposal: &ConsensusProposal) -> Result<()> {
        ...
        let expected_proposer = self
            .validator_manager
            .get_proposer_for_round(proposal.height, self.current_round.round);
        ...
        if let Some(zk_did_proof) = &proposal.consensus_proof.zk_did_proof {
            if !self
                .verify_zk_did_proof(zk_did_proof, &proposal.proposer)
                .await?
            {
    ...
    async fn verify_consensus_proof(&self, proof: &ConsensusProof) -> Result<()> {
        ...
        if proof.timestamp > current_time + 300 {
            return Err(anyhow::anyhow!(
                "Consensus proof timestamp too far in future"
            ));
        }
    ...
    fn validate_vote_signature(&self, vote: &ConsensusVote) -> Result<()> {
        ...
        if vote.signature.signature == vec![1, 2, 3] {
            return Err(anyhow::anyhow!(
                "Test signature is not allowed in production"
            ));
        }
    ...
    async fn verify_voter_eligibility(&self, vote: &ConsensusVote) -> Result<()> {
        ...
        if !self
            .validator_manager
            .is_validator_active(&vote.voter, vote.height)
        {
    ...
    fn validate_vote_timing(&self, vote: &ConsensusVote) -> Result<()> {
        ...
        if vote.timestamp > current_time + 300 {
            // 5 minutes in future
            return Err(anyhow::anyhow!("Vote timestamp too far in future"));
        }

        if vote.timestamp + 600 < current_time {
            // 10 minutes old
    ...
    async fn advance_to_precommit(&mut self, proposal_id: &lib_crypto::Hash) -> Result<()> {
        ...
        if let Some(validator_id) = &self.validator_identity {
            self.broadcast_precommit(proposal_id.clone(), validator_id.clone())
                .await?;
        }
    ...
    async fn finalize_block(&mut self, proposal_id: &lib_crypto::Hash) -> Result<()> {
        ...
        println!(
            "Finalizing block at height {} with {} bytes",
            proposal.height,
    ...
    async fn broadcast_prevote(&self, proposal_id: lib_crypto::Hash) -> Result<()> {
        if let Some(validator_id) = &self.validator_identity {
            let _vote = self
                .create_vote(VoteType::PreVote, proposal_id.clone(), validator_id.clone())
                .await?;

            // In production, this would broadcast to the network
            println!(
                "Broadcasting prevote for proposal {}",
    ...
    async fn broadcast_precommit(
        ...
        let _vote = self
            .create_vote(VoteType::PreCommit, proposal_id.clone(), validator_id)
            .await?;

        // In production, this would broadcast to the network
        println!(
            "Broadcasting precommit for proposal {}",
            hex::encode(proposal_id.as_bytes())
        );
    ...
    fn has_validator_voted(
        ...
        if let Some(votes) = self.vote_pool.get(&key) {
            votes
                .values()
                .any(|v| v.voter == *validator && v.vote_type == *vote_type)
        } else {
            false
        }
    }
            ...
            is_proposer: self
                ...
                .map(|id| {
                    self.validator_manager
                        .get_proposer_for_round(self.current_round.height, self.current_round.round)
                        .map(|proposer| &proposer.identity == id)
                        .unwrap_or(false)
                })
                .unwrap_or(false),
        }
...
pub mod testing {
    ...
    /// Create test vote
    fn create_test_vote(vote_type: VoteType, height: u64, round: u32) -> Result<ConsensusVote> {
