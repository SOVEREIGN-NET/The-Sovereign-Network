...
impl ChainEvaluator {
    ...
    pub fn evaluate_chains(local: &ChainSummary, imported: &ChainSummary) -> ChainDecision {
        ...
        if local.genesis_hash != imported.genesis_hash {
            ...
            info!(
                "   Local network: {} validators, {} identities, height {}",
                local.validator_count, local.total_identities, local.height
            );
            info!(
                "   Imported network: {} validators, {} identities, height {}",
    ...
    fn calculate_chain_score(chain: &ChainSummary) -> u64 {
        ...
        let chain_age = chain
            .latest_timestamp
            .saturating_sub(chain.genesis_timestamp);
    ...
    pub fn can_merge_chains(local: &ChainSummary, imported: &ChainSummary) -> bool {
        ...
        if local.height == imported.height && local.total_work == imported.total_work {
            ...
            let has_different_content = local.total_transactions != imported.total_transactions
        ...
        if local.height == imported.height {
            ...
            if work_diff_ratio <= 0.1 {
                // Within 10% work difference
    ...
    fn has_sufficient_validator_overlap(local: &ChainSummary, imported: &ChainSummary) -> bool {
        ...
        if local_count >= MIN_VALIDATORS_FOR_STRICT_RULES
            && imported_count >= MIN_VALIDATORS_FOR_STRICT_RULES
        {
    ...
    fn validate_bft_bridge_requirements(local: &ChainSummary, imported: &ChainSummary) -> bool {
        ...
        let min_bft_nodes =
            Self::calculate_minimum_bft_bridge_nodes(combined_network_size, combined_expected_tps);
    ...
    fn calculate_minimum_bft_bridge_nodes(network_size: u64, expected_tps: u64) -> u64 {
        ...
        let min_nodes = min_security_nodes
            .max(min_throughput_nodes)
            .max(min_scale_nodes);
    ...
    fn calculate_merge_score(chain: &ChainSummary) -> u64 {
        ...
        let total_score =
            validator_score + identity_score + transaction_score + work_score + stake_score;
    ...
    fn are_networks_compatible(local: &ChainSummary, imported: &ChainSummary) -> bool {
        ...
        {
            info!(
                "     Both networks have < {} validators - allowing merge",
                min_validators_for_merge
            );
        ...
        if age_difference > max_age_difference_seconds {
            info!(
                "     Age difference too large: {} days (max 365 days)",
        ...
        if size_ratio > max_size_ratio {
            info!(
                "     Size disparity too large: {:.1}:1 ratio (max {}:1)",
        ...
        info!(
            "    Networks are compatible: age_diff={}d, size_ratio={:.1}:1",
            age_difference / (24 * 3600),
            size_ratio
        );
    ...
    fn should_adopt_despite_genesis_mismatch(
        local: &ChainSummary,
        imported: &ChainSummary,
    ) -> bool {
...
mod tests {
    ...
    fn test_small_mesh_can_merge_easily() {
        ...
        assert!(ChainEvaluator::has_sufficient_validator_overlap(
            &small_local,
            &small_imported
        ));
    }
    ...
    fn test_bft_bridge_node_requirements() {
        ...
        assert!(!ChainEvaluator::validate_bft_bridge_requirements(
            &network_a, &network_b
        ));
    }
