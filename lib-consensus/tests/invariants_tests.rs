...
fn test_check_invariant_fork_detection() {
    ...
    assert!(
        msg.contains("NoFork"),
        "message should mention NoFork: {msg}"
    );
    assert!(
        msg.contains("no two commits may exist at the same height"),
...
fn test_check_invariant_monotonic_height_regression() {
    ...
    let msg = result.unwrap_err();
    assert!(
        msg.contains("MonotonicHeight"),
        "message should mention MonotonicHeight: {msg}"
    );
}
...
fn test_check_invariant_quorum_insufficient() {
    ...
    let msg = result.unwrap_err();
    assert!(
        msg.contains("QuorumRequired"),
        "message should mention QuorumRequired: {msg}"
    );
}
...
fn test_check_invariant_finality_violation() {
    ...
    let msg = result.unwrap_err();
    assert!(
        msg.contains("FinalityIrreversible"),
...
fn test_multiple_violations_reported() {
    ...
    assert!(
        panic_msg.contains("NoFork"),
        "should mention NoFork: {panic_msg}"
    );
    assert!(
        panic_msg.contains("MonotonicHeight"),
        "should mention MonotonicHeight: {panic_msg}"
    );
    assert!(
        panic_msg.contains("QuorumRequired"),
        "should mention QuorumRequired: {panic_msg}"
    );
    assert!(
        panic_msg.contains("FinalityIrreversible"),
        "should mention FinalityIrreversible: {panic_msg}"
    );
}
