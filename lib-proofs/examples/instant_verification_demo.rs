...
use lib_proofs::{
    types::ZkProof,
    verifiers::recursive_aggregator::{
...
fn simulate_blockchain_growth(aggregator: &mut RecursiveProofAggregator) -> Result<()> {
    ...
    for block_height in 1..=5 {
        ...
        println!(
            "   Block aggregated - {} transactions → 1 proof",
            transaction_count
        );
        ...
        let new_chain_proof = aggregator
            .create_recursive_chain_proof(&mock_block_proof, current_chain_proof.as_ref())?;
        ...
        if let Some(ref chain_proof) = current_chain_proof {
            ...
            println!(
                "   O(1) Verification: {} (constant time regardless of chain length)",
            ...
            let total_transactions: usize = (1..=block_height).map(|h| 10 + (h * 5)).sum();

            println!(
                "    Chain Stats: {} blocks, {} total transactions, 1 proof verification",
