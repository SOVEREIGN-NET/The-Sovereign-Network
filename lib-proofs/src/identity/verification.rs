...
use super::{CredentialSchema, ZkCredentialProof, ZkIdentityProof};
...
pub fn verify_identity_proof(proof: &ZkIdentityProof) -> Result<IdentityVerificationResult> {
    ...
    if !commitment_valid {
        return Ok(IdentityVerificationResult {
            basic_result: VerificationResult::Invalid(
                "Identity commitment verification failed".to_string(),
            ),
    ...
    if !knowledge_valid {
        return Ok(IdentityVerificationResult {
            basic_result: VerificationResult::Invalid(
                "Knowledge proof verification failed".to_string(),
            ),
            verified_attributes: vec![],
            proof_age_seconds,
            is_expired,
    ...
    if !attribute_valid {
        return Ok(IdentityVerificationResult {
            basic_result: VerificationResult::Invalid(
                "Attribute proof verification failed".to_string(),
            ),
            verified_attributes: vec![],
            proof_age_seconds,
            is_expired,
...
pub fn verify_credential_proof(
    ...
    if proof.is_expired() {
        return Ok(VerificationResult::Invalid(
            "Credential proof is expired".to_string(),
        ));
    }
    ...
    if !claims_valid {
        return Ok(VerificationResult::Invalid(
            "Claims commitment verification failed".to_string(),
        ));
    }
    ...
    if !revealed_claims_valid {
        return Ok(VerificationResult::Invalid(
            "Revealed claims verification failed".to_string(),
        ));
    }
    ...
    if !validity_valid {
        return Ok(VerificationResult::Invalid(
            "Validity proof verification failed".to_string(),
        ));
    }
...
pub fn verify_batch_identity_proofs(
    ...
    if !aggregated_valid {
        ...
        for proof in &batch.proofs {
            results.push(IdentityVerificationResult {
                basic_result: VerificationResult::Invalid(
                    "Batch aggregation verification failed".to_string(),
                ),
                verified_attributes: vec![],
    ...
    if !merkle_valid {
        for proof in &batch.proofs {
            results.push(IdentityVerificationResult {
                basic_result: VerificationResult::Invalid(
                    "Batch Merkle root verification failed".to_string(),
                ),
                verified_attributes: vec![],
                proof_age_seconds: proof.age_seconds(),
                is_expired: proof.is_expired(),
...
pub fn verify_batch_credential_proofs(
    ...
    if !aggregated_valid {
        for _ in 0..batch.proofs.len() {
            results.push(VerificationResult::Invalid(
                "Batch aggregated validity verification failed".to_string(),
            ));
        }
        return Ok(results);
    }
    ...
    if !commitment_valid {
        for _ in 0..batch.proofs.len() {
            results.push(VerificationResult::Invalid(
                "Batch combined commitment verification failed".to_string(),
            ));
        }
        return Ok(results);
    }
...
fn verify_issuer_signature(proof: &ZkCredentialProof, schema: &CredentialSchema) -> Result<bool> {
    ...
    match verify_signature(&message_hash, &proof.issuer_signature, &issuer_public_key) {
        ...
        Err(e) => {
            println!(
                "Cryptographic signature verification error - no fallbacks allowed: {:?}",
                e
            );
            return Ok(false);
        }
...
fn verify_revealed_claims(proof: &ZkCredentialProof, schema: &CredentialSchema) -> Result<bool> {
    ...
    let revealed_claim_names: std::collections::HashSet<_> = proof
        .revealed_claims
        .iter()
    ...
    for required_field in &schema.required_fields {
        if !revealed_claim_names.contains(required_field) {
            ...
            if proof.validity_proof.len() >= 64 {
                ...
                let proof_contains_field = proof
                    .validity_proof
                    .windows(16)
...
fn verify_credential_validity_proof(
    proof: &ZkCredentialProof,
...
fn verify_batch_combined_commitment(batch: &BatchCredentialProof) -> Result<bool> {
    ...
    for proof in &batch.proofs {
        ...
        issuer_commitment_data.extend_from_slice(&proof.issuer_signature[0..32]);
        // Include issuer binding
    }
...
mod tests {
    use super::*;
    use crate::identity::{
        CredentialSchema, IdentityAttributes, ZkCredentialProof, ZkIdentityProof,
    };
    ...
    fn test_verify_credential_proof() {
        ...
        println!(
            ...
            &proof.issuer_signature[0..8]
        );

        // Test each verification step individually
        println!(
            "1. Schema hash match: {}",
            proof.schema_hash == schema.schema_hash()
        );
    ...
    fn test_verify_credential_proof_schema_mismatch() {
        ...
        let mismatched_schema =
            CredentialSchema::new("test_credential".to_string(), "1.0".to_string(), [1u8; 32])
