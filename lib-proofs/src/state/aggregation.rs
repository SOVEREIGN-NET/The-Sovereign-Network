...
use crate::plonky2::{Plonky2Proof, RecursiveConfig, RecursiveProofBuilder};
use crate::state::{AggregatedStateProof, NetworkStateInfo, StateCommitment};
...
impl StateProofAggregator {
    ...
    fn create_parent_metadata(
        ...
        let total_nodes: u32 = child_proofs.iter().map(|p| p.total_node_count()).sum();

        let max_block_height = child_proofs
            .iter()
    ...
    async fn generate_recursive_proof(
        ...
        if child_proofs.is_empty() {
            return Err(anyhow!(
                "Cannot generate recursive proof from empty child proofs"
            ));
        }

        // Extract Plonky2 proofs from all children
        let child_plonky2_proofs: Vec<Plonky2Proof> = child_proofs
            .iter()
