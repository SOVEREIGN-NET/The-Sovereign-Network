...
use zhtp::monitoring::{
    Alert, AlertLevel, AlertManager, AlertThresholds, HealthMonitor, MetricsCollector,
    MonitoringConfig, MonitoringSystem, NodeHealth,
};
...
async fn test_monitoring_system_basic() -> Result<()> {
    ...
    assert!(matches!(
        ...
        NodeHealth::Healthy | NodeHealth::Warning | NodeHealth::Critical | NodeHealth::Down
    ));
...
async fn test_performance_metrics() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        monitoring
            .record_metric("test_performance", i as f64 * 10.0, tags)
            .await?;
        ...
        let handle = tokio::spawn(async move {
            ...
            for j in 0..10 {
                ...
                if let Err(e) = monitoring
                    .record_metric("concurrent_test", (i * 10 + j) as f64, tags)
                    .await
                {
...
async fn test_stress_monitoring() -> Result<()> {
    ...
    for i in 0..100 {
        ...
        monitoring
            .record_metric("stress_metric", (i as f64).sin() * 100.0, tags)
            .await?;
...
async fn test_component_integration() -> Result<()> {
    ...
    monitoring
        .record_metric("cpu_usage", 95.0, critical_tags.clone())
        .await?;
    monitoring
        .record_metric("memory_usage", 90.0, critical_tags.clone())
        .await?;
    monitoring
        .record_metric("disk_usage", 85.0, critical_tags)
        .await?;
...
async fn test_long_running_monitoring() -> Result<()> {
    ...
    while start_time.elapsed() < test_duration {
        ...
        monitoring
            .record_metric("endurance_test", start_time.elapsed().as_secs_f64(), tags)
            .await?;
