...
async fn test_rate_limiter_hourly_limit() -> Result<()> {
    ...
    assert!(
        result4.is_err(),
        "4th claim should be blocked by hourly limit"
    );
...
async fn test_rate_limiter_stats() -> Result<()> {
    ...
    println!(
        "    After claim: {} claims, cooldown active",
        stats2.claims_in_last_hour
    );
    ...
    println!(
        "    All stats retrieved: routing={}, storage={}",
        all_stats.routing.claims_in_last_hour, all_stats.storage.claims_in_last_hour
    );
...
async fn test_rewards_config_default() -> Result<()> {
    ...
    println!(
        "    Routing threshold: {} SOV",
        config.routing_minimum_threshold
    );
    println!(
        "    Storage threshold: {} SOV",
        config.storage_minimum_threshold
    );
...
async fn test_rewards_config_customization() -> Result<()> {
    ...
    println!(
        "    Custom routing threshold: {} SOV",
        config.routing_minimum_threshold
    );
    println!(
        "    Custom storage threshold: {} SOV",
        config.storage_minimum_threshold
    );
    println!("    Custom max claims: {}/hour", config.max_claims_per_hour);
    println!(
        "    Custom cooldown: {}s (5 minutes)",
...
async fn test_rewards_config_batch_sizes() -> Result<()> {
    ...
    println!(
        "    Routing max batch: {} SOV",
        config.routing_max_batch_size
    );
    println!(
        "    Storage max batch: {} SOV",
        config.storage_max_batch_size
    );
    ...
    println!(
        "    Custom routing batch: {} SOV",
        custom_config.routing_max_batch_size
    );
    println!(
        "    Custom storage batch: {} SOV",
        custom_config.storage_max_batch_size
    );
...
async fn test_rewards_config_check_intervals() -> Result<()> {
    ...
    println!(
        "    Default routing interval: {}s (10 minutes)",
        config.routing_check_interval_secs
    );
    println!(
        "    Default storage interval: {}s (10 minutes)",
        config.storage_check_interval_secs
    );
    ...
    println!(
        "    Custom routing interval: {}s (5 minutes)",
        custom_config.routing_check_interval_secs
    );
    println!(
        "    Custom storage interval: {}s (15 minutes)",
        custom_config.storage_check_interval_secs
    );
