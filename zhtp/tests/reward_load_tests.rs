...
async fn test_concurrent_rate_limit_checks() -> Result<()> {
    ...
    for i in 0..50 {
        ...
        let handle = tokio::spawn(async move { orch.check_rate_limit("routing").await });
...
async fn test_concurrent_stats_queries() -> Result<()> {
    ...
    for i in 0..200 {
        let orch = orchestrator.clone();
        let processor = if i % 2 == 0 { "routing" } else { "storage" };
        let handle = tokio::spawn(async move { orch.get_rate_limit_stats(processor).await });
        handles.push(handle);
    }
...
async fn test_sustained_claim_load() -> Result<()> {
    ...
    println!(
        "    Processed {} sustained claims in {:?}",
        total_claims, elapsed
    );
    println!(
        "    Average throughput: {:.2} claims/sec",
        total_claims as f64 / elapsed.as_secs_f64()
    );
...
async fn test_mixed_operation_load() -> Result<()> {
    ...
    println!(
        ...
        150.0 / elapsed.as_secs_f64()
    );

    // System should remain consistent
    let stats = orchestrator.get_all_rate_limit_stats().await;
    println!(
        "    Final state: routing={}, storage={}",
        stats.routing.claims_in_last_hour, stats.storage.claims_in_last_hour
    );
...
async fn test_multiple_orchestrators_concurrent() -> Result<()> {
    ...
    let orchestrator1 = Arc::new(RewardOrchestrator::with_config(
        network1,
        blockchain1,
        env1,
        config1,
    ));
    let orchestrator2 = Arc::new(RewardOrchestrator::with_config(
        network2,
        blockchain2,
        env2,
        config2,
    ...
    println!(
        "    Processed 100 operations (2 orchestrators) in {:?}",
        elapsed
    );
...
async fn test_rate_limiter_cleanup_under_load() -> Result<()> {
    ...
    println!(
        "    Current claims in last hour: {}",
        stats.claims_in_last_hour
    );
    println!("    Cleanup mechanism working (all recent claims counted)");

    assert_eq!(
        stats.claims_in_last_hour, 100,
        "Should track all recent claims"
    );
...
async fn test_performance_baseline() -> Result<()> {
    ...
    println!("   ┌────────────────────────────────────────┐");
    println!(
        "   │ check_rate_limit: {:.2} ops/sec     │",
        check_throughput
    );
    println!(
        "   │ record_claim:     {:.2} ops/sec     │",
        record_throughput
    );
    println!(
        "   │ get_stats:        {:.2} ops/sec     │",
    ...
    assert!(
        check_throughput > 1000.0,
        "check_rate_limit should handle >1k ops/sec"
    );
    assert!(
        record_throughput > 1000.0,
        "record_claim should handle >1k ops/sec"
    );
    assert!(
        stats_throughput > 1000.0,
        "get_stats should handle >1k ops/sec"
    );
