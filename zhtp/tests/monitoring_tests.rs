...
use zhtp::monitoring::{
    clear_global_alert_manager, get_global_alert_manager, set_global_alert_manager, Alert,
    AlertLevel, AlertManager, HealthMonitor, MetricsCollector, MonitoringSystem, NodeHealth,
    SystemMetrics,
};
...
async fn test_monitoring_system_lifecycle() -> Result<()> {
    ...
    assert!(matches!(
        ...
        NodeHealth::Healthy | NodeHealth::Warning | NodeHealth::Critical
    ));
...
async fn test_metrics_collection() -> Result<()> {
    ...
    metrics_collector
        .record_metric("test_counter", 42.0, tags.clone())
        .await?;
    metrics_collector
        .record_metric("test_gauge", 3.14, tags.clone())
        .await?;
    metrics_collector
        .record_metric("test_histogram", 100.0, tags)
        .await?;
...
async fn test_health_monitoring() -> Result<()> {
    ...
    // Test health history
    let history = health_monitor
        .get_health_history(Duration::from_secs(60))
        .await?;
...
async fn test_alert_management() -> Result<()> {
    ...
    let active_alerts = alert_manager.get_active_alerts().await?;
    let resolved_alert = active_alerts.iter().find(|a| a.id == "test_alert_001");
...
async fn test_alert_levels() -> Result<()> {
    ...
    let emergency_alerts = alert_manager
        .get_alerts_by_level(AlertLevel::Emergency)
        .await?;
...
async fn test_threshold_monitoring() -> Result<()> {
    ...
    if !cpu_alerts.is_empty() {
        assert!(
            cpu_alerts[0].level == AlertLevel::Warning
                || cpu_alerts[0].level == AlertLevel::Critical
        );
    }
...
async fn test_metrics_export_formats() -> Result<()> {
    ...
    for i in 0..10 {
        metrics_collector
            .record_metric(&format!("test_metric_{}", i), i as f64, tags.clone())
            .await?;
    }
        ...
        tokio::spawn({
            ...
            async move {
                for i in 0..100 {
                    ...
                    let _ = monitoring
                        .record_metric("concurrent_test", i as f64, tags)
                        .await;
                }
                Ok::<(), anyhow::Error>(())
            }
...
async fn test_monitoring_memory_usage() -> Result<()> {
    ...
    for i in 0..1000 {
        monitoring
            .record_metric("memory_stress_test", i as f64, tags.clone())
            .await?;
