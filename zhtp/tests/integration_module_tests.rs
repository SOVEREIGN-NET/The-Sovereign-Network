...
async fn test_dependency_validation() -> Result<()> {
    ...
    let crypto_deps = integration
        .get_component_dependencies(ComponentId::Crypto)
        .await?;
    assert!(crypto_deps.is_empty()); // Crypto has no dependencies

    let consensus_deps = integration
        .get_component_dependencies(ComponentId::Consensus)
        .await?;
...
async fn test_component_dependency_mapping() -> Result<()> {
    ...
    for component in &all_components {
        let deps = integration
            .get_component_dependencies(component.clone())
            .await?;

        // Verify no circular dependencies
        assert!(
            !deps.contains(component),
