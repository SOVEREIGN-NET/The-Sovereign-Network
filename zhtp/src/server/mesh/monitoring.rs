...
use crate::server::monitoring::{
    AlertLevel, AlertThresholds, MetricsSnapshot, PeerPerformanceStats, SyncAlert,
...
impl MeshRouter {
    ...
    pub async fn list_peer_performance(&self) -> Vec<PeerPerformanceStats> {
        ...
        reputations
            .iter()
            .map(|(peer_id, rep)| PeerPerformanceStats {
    ...
    pub async fn update_performance_metrics(&self) {
        ...
        if total_blocks > 0 {
            metrics.duplicate_block_ratio =
                (broadcast_metrics.blocks_rejected as f64 / total_blocks as f64) * 100.0;
        }

        let total_txs =
            broadcast_metrics.transactions_received + broadcast_metrics.transactions_relayed;
        if total_txs > 0 {
            metrics.duplicate_tx_ratio =
                (broadcast_metrics.transactions_rejected as f64 / total_txs as f64) * 100.0;
        }

        // Calculate validation success rate
        let total_validated =
            broadcast_metrics.blocks_received + broadcast_metrics.transactions_received;
        let total_rejected =
            broadcast_metrics.blocks_rejected + broadcast_metrics.transactions_rejected;
        if total_validated > 0 {
            metrics.validation_success_rate =
                ((total_validated - total_rejected) as f64 / total_validated as f64) * 100.0;
        }

        // Calculate relay efficiency
        let total_sent = broadcast_metrics.blocks_sent + broadcast_metrics.transactions_sent;
        if total_sent > 0 {
            metrics.relay_efficiency =
                ((total_blocks + total_txs) as f64 / total_sent as f64) * 100.0;
        }
    ...
    pub async fn check_and_generate_alerts(&self) {
        ...
        if metrics.avg_block_propagation_ms > thresholds.max_block_latency_ms as f64 {
            let alert = SyncAlert::new(
                ...
                format!(
                    "Block propagation latency ({:.2}ms) exceeds threshold ({}ms)",
                    metrics.avg_block_propagation_ms, thresholds.max_block_latency_ms
                ),
            )
            .with_metric(
                metrics.avg_block_propagation_ms,
                thresholds.max_block_latency_ms as f64,
            );

            if !alerts
                .iter()
                .any(|a| a.category == "block_latency" && !a.acknowledged)
            {
                alerts.push(alert);
            }
        }
        ...
        if metrics.avg_tx_propagation_ms > thresholds.max_tx_latency_ms as f64 {
            let alert = SyncAlert::new(
                ...
                format!(
                    "Transaction propagation latency ({:.2}ms) exceeds threshold ({}ms)",
                    metrics.avg_tx_propagation_ms, thresholds.max_tx_latency_ms
                ),
            )
            .with_metric(
                metrics.avg_tx_propagation_ms,
                thresholds.max_tx_latency_ms as f64,
            );

            if !alerts
                .iter()
                .any(|a| a.category == "tx_latency" && !a.acknowledged)
            {
                alerts.push(alert);
            }
        }
        ...
        let bandwidth_mbps =
            (metrics.bytes_sent_per_sec + metrics.bytes_received_per_sec) / 1_000_000.0;
        if bandwidth_mbps > thresholds.max_bandwidth_mbps {
            let alert = SyncAlert::new(
                AlertLevel::Critical,
                "bandwidth".to_string(),
                format!(
                    "Bandwidth usage ({:.2} MB/s) exceeds threshold ({:.2} MB/s)",
            ...
            if !alerts
                .iter()
                .any(|a| a.category == "bandwidth" && !a.acknowledged)
            {
                alerts.push(alert);
            }
        }
        ...
        if metrics.validation_success_rate < thresholds.min_validation_success_rate
            && metrics.validation_success_rate > 0.0
        {
            let alert = SyncAlert::new(
                ...
                format!(
                    "Validation success rate ({:.1}%) below threshold ({:.1}%)",
                    metrics.validation_success_rate, thresholds.min_validation_success_rate
                ),
            )
            .with_metric(
                metrics.validation_success_rate,
                thresholds.min_validation_success_rate,
            );

            if !alerts
                .iter()
                .any(|a| a.category == "validation_rate" && !a.acknowledged)
            {
                alerts.push(alert);
            }
        }
        ...
        if metrics.duplicate_block_ratio > thresholds.max_duplicate_ratio {
            let alert = SyncAlert::new(
                ...
                format!(
                    "Duplicate block ratio ({:.1}%) exceeds threshold ({:.1}%)",
                    metrics.duplicate_block_ratio, thresholds.max_duplicate_ratio
                ),
            )
            .with_metric(
                metrics.duplicate_block_ratio,
                thresholds.max_duplicate_ratio,
            );

            if !alerts
                .iter()
                .any(|a| a.category == "duplicate_blocks" && !a.acknowledged)
            {
                alerts.push(alert);
            }
        }
    ...
    pub async fn create_metrics_snapshot(&self) -> MetricsSnapshot {
        ...
        MetricsSnapshot {
            ...
            avg_latency_ms: (performance_metrics.avg_block_propagation_ms
                + performance_metrics.avg_tx_propagation_ms)
                / 2.0,
    ...
    pub async fn start_metrics_snapshot_task(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                debug!(
                    "📊 Metrics snapshot created ({} total snapshots)",
    ...
    pub async fn update_mesh_protocol_stats(&self) {
        ...
        debug!(
            "📊 Updated mesh protocol stats: {} connections, {} data routed, {}ms avg latency",
