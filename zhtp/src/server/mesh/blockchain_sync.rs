...
impl MeshRouter {
    ...
    pub async fn set_broadcast_receiver(
        ...
        mut receiver: tokio::sync::mpsc::UnboundedReceiver<
            lib_blockchain::BlockchainBroadcastMessage,
        >,
    ) {
            ...
            while let Some(msg) = receiver.recv().await {
                match msg {
                    lib_blockchain::BlockchainBroadcastMessage::NewBlock(block) => {
                        info!(
                            "📡 Broadcasting new block {} to mesh network",
                            block.height()
                        );
                        ...
                        let sender_pubkey = if let Some(identity_mgr) = identity_manager.as_ref() {
                            ...
                            if let Some(identity) = mgr.list_identities().first() {
                                ...
                                warn!(
                                    "No identity available for sender - skipping block broadcast"
                                );
                                continue;
                            }
                        } else {
                    ...
                    lib_blockchain::BlockchainBroadcastMessage::NewTransaction(tx) => {
                        debug!(
                            "📡 Broadcasting new transaction {} to mesh network",
                            tx.hash()
                        );
                        ...
                        let sender_pubkey = if let Some(identity_mgr) = identity_manager.as_ref() {
                            ...
                            warn!(
                                "Identity manager not available - skipping transaction broadcast"
                            );
                            continue;
                        };
    ...
    pub async fn set_bluetooth_protocol(
        &self,
        protocol: Arc<lib_network::protocols::bluetooth::BluetoothMeshProtocol>,
    ) {
    ...
    pub async fn set_quic_protocol(
        &self,
        quic: Arc<lib_network::protocols::quic_mesh::QuicMeshProtocol>,
    ) {
    ...
    pub async fn set_blockchain_event_receiver(
        ...
        // Inject into QUIC protocol's message handler
        if let Some(quic) = self.quic_protocol.read().await.as_ref() {
            if let Some(handler) = quic.message_handler.as_ref() {
                let mut handler_lock = handler.write().await;
    ...
    pub async fn get_sender_public_key(&self) -> Result<PublicKey> {
        ...
        Err(anyhow::anyhow!(
            "No identity available for sender public key"
        ))
    }
    ...
    pub async fn send_to_peer(&self, peer_id: &PublicKey, message: ZhtpMeshMessage) -> Result<()> {
        ...
        let our_pubkey = match self.get_sender_public_key().await {
            ...
            Err(e) => {
                ...
                return Err(anyhow::anyhow!(
                    "Cannot route without sender identity: {}",
                    e
                ));
            }
        };
        ...
        match self.send_with_routing(message, peer_id, &our_pubkey).await {
            Ok(message_id) => {
                info!(
                    "✅ Message routed successfully (ID: {}, dest: {:?})",
                    message_id,
                    &peer_id.key_id[..8]
                );
                Ok(())
    ...
    pub async fn broadcast_to_peers(&self, message: ZhtpMeshMessage) -> Result<usize> {
        ...
        let our_pubkey = match self.get_sender_public_key().await {
            ...
            Err(e) => {
                ...
                return Err(anyhow::anyhow!(
                    "Cannot broadcast without sender identity: {}",
                    e
                ));
            }
        };
        ...
        for peer_entry in connections.all_peers() {
            ...
            match self
                .send_with_routing(message.clone(), &peer_pubkey, &our_pubkey)
                .await
            {
                Ok(_message_id) => {
                    success_count += 1;
                ...
                Err(e) => {
                    debug!(
                        "Failed to route broadcast to peer {:?}: {}",
                        &peer_pubkey.key_id[..8],
                        e
                    );
        ...
        self.track_bytes_sent((serialized.len() * success_count) as u64)
            .await;
        info!(
            "📤 Broadcast complete: {} peers reached (routed through MeshRouter)",
            success_count
        );
    ...
    pub async fn sync_blockchain_from_peer(&self, peer_pubkey: &PublicKey) -> Result<u64> {
        ...
        let should_sync = self
            .sync_coordinator
            .register_peer_protocol(peer_pubkey.clone(), protocol.clone(), sync_type)
            .await;

        if !should_sync {
            return Err(anyhow::anyhow!(
                "Already syncing with this peer via {:?}",
                protocol
            ));
        }

        // Create sync request using unified sync manager
        let (request_id, sync_message) = self
            .sync_manager
            .create_blockchain_request(peer_pubkey.clone(), None)
            .await?;
        ...
        self.sync_coordinator
            .start_sync(peer_pubkey.clone(), request_id, sync_type, protocol)
            .await;

        info!(
            "📥 Initiated edge node sync from peer {} (request {})",
    ...
    pub async fn coordinate_multi_peer_sync(
        ...
        for (peer_pubkey, protocol) in available_peers {
            ...
            if should_sync {
                ...
                match self.sync_blockchain_from_peer(&peer_pubkey).await {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to initiate sync with peer {}: {}",
                            hex::encode(&peer_pubkey.key_id[..8]),
                            e
                        );
                    }
                ...
                debug!(
                    "⏭️ Skipping duplicate sync with peer {} via {:?}",
                    hex::encode(&peer_pubkey.key_id[..8]),
                    protocol
                );
            }
        ...
        info!(
            "📊 Multi-peer sync coordination: {} syncs initiated from {} available peers",
            initiated_syncs.len(),
            peer_count
        );
    ...
    pub async fn get_edge_sync_height(&self) -> Result<u64> {
        ...
        if !is_edge {
            return Err(anyhow::anyhow!(
                "Node is not in edge mode - use blockchain height instead"
            ));
        }
