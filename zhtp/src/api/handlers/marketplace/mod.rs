...
impl MarketplaceHandler {
    ...
    async fn handle_request_internal(&self, request: &ZhtpRequest) -> ZhtpResult<ZhtpResponse> {
        ...
        match (request.method, segments.as_slice()) {
            ...
            (ZhtpMethod::Post, ["api", "marketplace", "content", content_hash, "list"]) => {
                self.list_content_for_sale(content_hash, &request.body)
                    .await
            }
            ...
            _ => {
                error!(
                    "Unknown marketplace API endpoint: {:?} {}",
                    request.method, path
                );
    ...
    async fn transfer_content(
        ...
        // Parse hashes
        let content_hash =
            Hash::from_hex(content_hash_str).map_err(|e| anyhow!("Invalid content hash: {}", e))?;
        let from_wallet = Hash::from_hex(&request.from_wallet)
            .map_err(|e| anyhow!("Invalid from_wallet: {}", e))?;
        let to_wallet =
            Hash::from_hex(&request.to_wallet).map_err(|e| anyhow!("Invalid to_wallet: {}", e))?;
        ...
        let tx_hash = if request.price > 0 {
            info!(
                "Creating blockchain transaction for {} SOV payment",
                request.price
            );
        ...
        info!(
            " Content transferred successfully with tx_hash: {}",
            tx_hash
        );
    ...
    async fn list_content_for_sale(
        &self,
        content_hash_str: &str,
        body: &[u8],
    ) -> ZhtpResult<ZhtpResponse> {
        ...
        let record = manager
            .get_ownership_record(&content_hash)
    ...
    async fn buy_content(&self, content_hash_str: &str, body: &[u8]) -> ZhtpResult<ZhtpResponse> {
        ...
        // Get current owner
        let manager = self.wallet_content_manager.read().await;
        let seller_wallet = manager
            .get_content_owner(&content_hash)
    ...
    async fn create_payment_transaction(
        ...
        if identity_id_bytes.len() != 32 {
            return Err(anyhow!(
                "Invalid identity ID length: expected 32 bytes, got {}",
        ...
        let private_key = identity.private_key.as_ref().ok_or_else(|| {
            anyhow!(
                "Private key not found for buyer identity {}",
        ...
        if total_selected < required_amount {
            drop(blockchain);
            return Err(anyhow!(
                "Insufficient funds: have {} SOV, need {} SOV (including {} SOV fee)",
                total_selected,
                required_amount,
                fee
            ));
        ...
        info!(
            " Selected {} UTXOs totaling {} SOV (need {} SOV)",
            selected_utxos.len(),
            total_selected,
            required_amount
        );
        ...
        for (utxo_hash, output_index, _amount) in &selected_utxos {
            ...
            let nullifier =
                lib_blockchain::Hash::from_slice(&lib_crypto::hash_blake3(&nullifier_data)[..32]);
        ...
        let payment_note = lib_blockchain::Hash::from_slice(
            &lib_crypto::hash_blake3(
                &[&b"note:marketplace_payment"[..], content_hash.as_bytes()].concat(),
            )[..32],
        );
        ...
        info!(
            " Marketplace transaction created with real signature: {}",
...
impl ZhtpRequestHandler for MarketplaceHandler {
    ...
    fn can_handle(&self, request: &ZhtpRequest) -> bool {
        ...
        path.starts_with("/api/content/")
            && (path.contains("/transfer") || path.contains("/list") || path.contains("/buy"))
