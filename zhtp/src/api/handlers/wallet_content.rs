...
impl WalletContentHandler {
    ...
    pub async fn handle_request_internal(&self, request: &ZhtpRequest) -> ZhtpResult<ZhtpResponse> {
        ...
        match (request.method.as_str(), segments.as_slice()) {
            ...
            _ => {
                error!(
                    "Unknown wallet content API endpoint: {} {}",
                    request.method, path
                );
    ...
    async fn get_wallet_content(&self, wallet_id_str: &str) -> ZhtpResult<ZhtpResponse> {
        ...
        let wallet_id =
            Hash::from_hex(wallet_id_str).map_err(|e| anyhow!("Invalid wallet ID: {}", e))?;
        ...
        info!(
            " Found {} content items for wallet {}",
            content_items.len(),
            wallet_id_str
        );
    ...
    async fn get_wallet_content_statistics(&self, wallet_id_str: &str) -> ZhtpResult<ZhtpResponse> {
        ...
        info!(
            " Wallet {} owns {} items ({} bytes, {} SOV value)",
    ...
    async fn get_content_transfer_history(
        &self,
    ...
    async fn verify_content_ownership(
        ...
        info!(
            "Verifying ownership of {} by wallet {}",
            content_hash_str, wallet_id_str
        );
