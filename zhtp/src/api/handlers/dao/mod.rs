...
mod tests {
    ...
    fn dao_registry_tx(
        ...
        let execution_data = DaoExecutionData {
            ...
            multisig_signatures: vec![serde_json::to_vec(&event)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check")],
        };
    ...
    fn create_proposal_accepts_legacy_cli_shape() {
        ...
        let parsed: CreateProposalRequest =
            serde_json::from_str(body)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
    ...
    fn oracle_execution_params_encode_committee_update() {
        let mut blockchain = Blockchain::new()// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
    ...
    fn cast_vote_accepts_legacy_and_canonical_fields() {
        ...
        let legacy: CastVoteRequest =
            serde_json::from_str(legacy_body)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
        let canonical: CastVoteRequest =
            serde_json::from_str(canonical_body)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
