    ...
    async fn submit_transaction(
...

        {
            // Placeholder - will be replaced with actual mesh server call
            {
                ...
                info!(
                    " Transaction {} submitted to mesh {} (PLACEHOLDER)",
                    tx_hash, mesh_id
                );
    ...
    async fn produce_block(
...

        {
            // Placeholder - will be replaced with actual mesh server call
            {
                ...
                info!(
                    " Block {} produced for mesh {} (PLACEHOLDER)",
                    block_height, mesh_id
                );
...
impl ZhtpRequestHandler for MeshHandler {
    async fn handle_request(&self, request: ZhtpRequest) -> ZhtpResult<ZhtpResponse> {
        ...
        match (method, path.as_str()) {
            ...
            (ZhtpMethod::Post, "/api/v1/mesh/create") => self.create_mesh(request).await,

            // POST /api/v1/mesh/{mesh_id}/transaction
            (ZhtpMethod::Post, path)
                if path.starts_with("/api/v1/mesh/") && path.ends_with("/transaction") =>
            {
            ...
            (ZhtpMethod::Post, path)
                if path.starts_with("/api/v1/mesh/") && path.ends_with("/produce_block") =>
            {
                let mesh_id = path
                    .strip_prefix("/api/v1/mesh/")
            ...
            (ZhtpMethod::Get, path)
                if path.starts_with("/api/v1/mesh/") && path.ends_with("/status") =>
            {
                let mesh_id = path
                    .strip_prefix("/api/v1/mesh/")
            ...
            (ZhtpMethod::Get, path)
                if path.starts_with("/api/v1/mesh/") && path.contains("/sync/proof") =>
            {
                let mesh_id = path
                    .strip_prefix("/api/v1/mesh/")
