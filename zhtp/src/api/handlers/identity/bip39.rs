// Auto-generated from BIP39 English wordlist
...
/// Extract 32-byte entropy from a 24-word BIP39 mnemonic
/// This matches lib-client's entropy_from_mnemonic function exactly
pub fn entropy_from_mnemonic(phrase: &str) -> Result<Vec<u8>> {
    ...
    if words.len() != 24 {
        return Err(anyhow!(
            "Seed phrase must be exactly 24 words, got {}",
            words.len()
        ));
    }

    // Convert words to 11-bit indices, then to bits
    let mut bits = Vec::with_capacity(words.len() * 11);
    for word in &words {
        let index = BIP39_WORDLIST
            .iter()
            .position(|w| *w == word.as_str())
            .ok_or_else(|| anyhow!("Invalid BIP39 word: {}", word))? as u16;
    ...
    // Extract 256 bits of entropy (32 bytes)
    ...
    // Extract and verify 8-bit checksum
    ...
    if checksum_byte != expected_checksum {
        return Err(anyhow!("Invalid seed phrase checksum"));
    }
