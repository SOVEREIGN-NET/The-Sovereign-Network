...
impl MetricsCollector {
    ...
    pub async fn export_metrics(&self) -> Result<HashMap<String, f64>> {
        ...
        exported.insert(
            ...
            metrics.current_block_height as f64,
        );
            ...
            Ok(storage_lock) => {
                ...
                match storage.get_statistics().await {
                    Ok(stats) => {
                        ...
                        metrics.storage_available_bytes =
                            1024 * 1024 * 1024 * 10 - stats.storage_stats.total_storage_used;
                        // 10GB limit
                    }
                    Err(_) => {
                        ...
                        metrics.storage_available_bytes = 1024 * 1024 * 1024 * 10;
                        // Default 10GB available
                    }
                }
            }
            Err(_) => {
    ...
    pub async fn export_influxdb(&self) -> Result<String> {
        ...
        // System metrics
        influxdb_output.push_str(&format!(
            "system_metrics cpu_usage={},memory_usage={},disk_usage={} {}\n",
            metrics.cpu_usage_percent,
            metrics.memory_usage,
            metrics.disk_usage,
            metrics.timestamp * 1_000_000_000
        ));

        // Network metrics
        influxdb_output.push_str(&format!(
            "network_metrics peer_count={},bytes_sent={},bytes_received={} {}\n",
            metrics.peer_count,
        ...
        // Blockchain metrics
        influxdb_output.push_str(&format!(
            "blockchain_metrics block_height={},total_transactions={},pending_transactions={} {}\n",
            metrics.current_block_height,
    ...
    pub async fn get_metrics_summary(&self) -> Result<MetricsSummary> {
        ...
        Ok(MetricsSummary {
            ...
            memory_usage: (metrics.memory_usage_bytes as f64 / metrics.memory_total_bytes as f64)
                * 100.0,
            ...
            network_throughput: (metrics.network_bytes_sent + metrics.network_bytes_received)
                as f64
