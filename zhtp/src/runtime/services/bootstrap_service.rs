...
impl BootstrapService {
    ...
    pub async fn try_bootstrap_blockchain_from_peer(
        ...
        info!(
            "Attempting incremental blockchain sync from peer: {}",
            peer_label
        );
        ...
        if !tip_response.is_success() {
            return Err(anyhow!(
                "Peer {} returned error for chain tip: {}",
                peer_label,
                tip_response.status_message
            ));
        }
        ...
        info!(
            ...
            local_height, local_genesis
        );
        info!(
            "  Peer:   height={}, genesis={}",
            peer_tip.height, peer_tip.genesis_hash
        );
        ...
        if peer_tip.genesis_hash != local_genesis {
            ...
            if !export_response.is_success() {
                return Err(anyhow!(
                    "Peer {} returned error for export: {}",
                    peer_label,
                    export_response.status_message
                ));
            }

            let blockchain_data = export_response.body;
            info!(
                "Received {} bytes for merge evaluation",
                blockchain_data.len()
            );
            ...
            blockchain_clone
                .evaluate_and_merge_chain(blockchain_data)
                .await?;
            info!(
                "Successfully synced and merged from {} (genesis mismatch)",
                peer_label
            );
        ...
        if peer_tip.height < local_height {
            info!(
                "Local chain is ahead (peer: {}, local: {})",
            ...
            if peer_tip.identity_count > local_identity_count {
                ...
                if export_response.is_success() {
                    ...
                    blockchain_clone
                        ...
                        .await?;
                    info!(
                        "Successfully synced and merged additional data from {}",
                        peer_label
                    );
                    return Ok(blockchain_clone);
                } else {
                    warn!(
                        "Failed to fetch full chain for merge: {}",
                        export_response.status_message
                    );
                }
            } else {
                info!(
                    "Local chain is up-to-date (peer: {} identities, local: {} identities)",
                    peer_tip.identity_count, local_identity_count
                );
            }
            return Ok(blockchain.read().await.clone());
        ...
        info!(
            "Peer is ahead - fetching missing blocks {} to {}",
            local_height + 1,
            peer_tip.height
        );
        ...
        if !blocks_response.is_success() {
            return Err(anyhow!(
                "Peer {} returned error for blocks: {}",
                peer_label,
                blocks_response.status_message
            ));
        }

        let blocks_data = blocks_response.body;
        info!(
            "Received {} bytes ({} blocks requested)",
            blocks_data.len(),
            end - start + 1
        );
        ...
        info!(
            "Successfully synced {} blocks from {} (incremental)",
            blockchain_guard.height - local_height,
            peer_label
        );
