...
mod api_integration_tests {
    ...
    async fn test_register_identity_derives_did_and_node_id() {
        ...
        let keypair = lib_crypto::KeyPair::generate()// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
        let public_key_b64 =
        ...
        let signature =
            lib_crypto::sign_message(&keypair, signed_message.as_bytes())// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
        ...
        let json: serde_json::Value = serde_json::from_slice(&response.body)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
        let did = json
            .get("did")
            .and_then(|v| v.as_str())
            // REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
