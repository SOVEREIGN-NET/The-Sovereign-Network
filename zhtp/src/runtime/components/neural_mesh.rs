...
impl NeuralMeshComponent {
    ...
    // Extracted to simulation.rs — see that module for the full 4-node
    // distributed training harness (Phase 1-6 per cycle).
    ...
    async fn start(&self) -> Result<()> {
        ...
        *self.status.write().await = ComponentStatus::Running;
        ...
        if std::env::var("ZHTP_MULTI_NODE_SIM").as_deref() == Ok("1") {
            ...
            super::simulation::spawn_multi_node_simulation(
