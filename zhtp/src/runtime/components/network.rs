...
impl NetworkComponent {
    ...
    pub async fn get_routing_stats(&self) -> RoutingRewardStats {
        if let Some(ref server) = *self.mesh_server.read().await {
            ...
            debug!(
                "Mesh statistics not yet available (unified_server starting), returning defaults"
            );
    ...
    pub async fn reset_routing_rewards(&self) -> Result<()> {
        if let Some(ref server) = *self.mesh_server.read().await {
            ...
            Err(anyhow::anyhow!(
                "Cannot reset rewards: mesh server not initialized"
            ))
        }
    }
...
impl Component for NetworkComponent {
        ...
        match message {
            ComponentMessage::Custom(msg, _data) if msg == "discover_peers" => {
                if let Some(ref server) = *self.mesh_server.read().await {
                    ...
                    info!(
                        "Network stats - Active connections: {}, Coverage: {:.2} km²",
    ...
    async fn get_metrics(&self) -> Result<HashMap<String, f64>> {
        ...
        if let Some(ref server) = *self.mesh_server.read().await {
            ...
            metrics.insert(
                "total_data_routed".to_string(),
                stats.total_data_routed as f64,
            );
            metrics.insert(
                "long_range_relays".to_string(),
                stats.long_range_relays as f64,
            );
            metrics.insert(
                "average_latency_ms".to_string(),
                stats.average_latency_ms as f64,
            );
            metrics.insert("coverage_area_km2".to_string(), stats.coverage_area_km2);
            metrics.insert(
                "people_with_free_internet".to_string(),
                stats.people_with_free_internet as f64,
            );
        } else {
