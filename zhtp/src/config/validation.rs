...
use super::{ConfigError, NodeConfig, RuntimeRole};
use anyhow::Result;
...
pub async fn validate_complete_configuration(config: &NodeConfig) -> Result<()> {
    info!(
        "Validating configuration across {} packages...",
...
fn validate_role_transport_gateway_invariants(config: &NodeConfig) -> Result<()> {
    ...
    match config.runtime_role {
        RuntimeRole::Validator => {
            if !config.consensus_config.validator_enabled {
                return Err(ConfigError::ResourceConflict {
                    details:
                        "runtime_role=VALIDATOR requires consensus_config.validator_enabled=true"
                            .to_string(),
                }
                .into());
            }
        }
        RuntimeRole::Edge | RuntimeRole::Relay | RuntimeRole::Bootstrap | RuntimeRole::Service => {
            if config.consensus_config.validator_enabled {
                return Err(ConfigError::ResourceConflict {
                    details: format!(
    ...
    if config.protocols_config.gateway_enabled && config.runtime_role != RuntimeRole::Service {
        return Err(ConfigError::ResourceConflict {
            details:
                "protocols_config.gateway_enabled=true is only allowed for runtime_role=SERVICE"
                    .to_string(),
        }
        .into());
    }
    ...
    if config.runtime_role == RuntimeRole::Service && !config.protocols_config.gateway_enabled {
        return Err(ConfigError::ResourceConflict {
            details: "runtime_role=SERVICE requires protocols_config.gateway_enabled=true"
                .to_string(),
        }
        .into());
    }
...
fn validate_port_assignments(config: &NodeConfig) -> Result<()> {
    ...
    if is_unified_mode {
        ...
        info!(
            "Using unified server mode - mesh and API protocols share port {}",
            unified_server_port
        );
    } else {
        // Check mesh port
        if let Some(existing) =
            used_ports.insert(config.network_config.mesh_port, "mesh".to_string())
        {
            conflicts.push((
                config.network_config.mesh_port,
                vec!["mesh".to_string(), existing],
            ));
        }

        // Check API port
        if let Some(existing) = used_ports.insert(
            config.protocols_config.api_port,
            "protocols-api".to_string(),
        ) {
            conflicts.push((
                config.protocols_config.api_port,
                vec!["protocols-api".to_string(), existing],
            ));
        }
    }
    ...
    if let Some(existing) =
        used_ports.insert(config.storage_config.dht_port, "storage-dht".to_string())
    {
        conflicts.push((
            config.storage_config.dht_port,
            vec!["storage-dht".to_string(), existing],
        ));
    }
    ...
    for &port in &standard_ports {
        if used_ports.contains_key(&port) {
            warn!(
                " Using standard system port {}: {}",
                port, used_ports[&port]
            );
        }
    }
    ...
    if config.storage_config.storage_capacity_gb > allocations.max_disk_gb {
        return Err(ConfigError::ResourceConflict {
            details: format!(
                ...
                config.storage_config.storage_capacity_gb, allocations.max_disk_gb
            ),
        }
        .into());
    }
...
fn validate_security_consistency(config: &NodeConfig) -> Result<()> {
    ...
    if config.crypto_config.post_quantum_enabled
        != (config.security_level != super::SecurityLevel::Basic)
    {
...
fn validate_mesh_mode_configuration(config: &NodeConfig) -> Result<()> {
    ...
    config
        .mesh_mode
        .validate_capabilities(&available_protocols)
    ...
    if config.mesh_mode == super::MeshMode::PureMesh
        && config.network_config.bootstrap_peers.is_empty()
    {
...
fn validate_integration_settings(config: &NodeConfig) -> Result<()> {
    ...
    if integration.health_check_interval_ms > 300000 {
        // 5 minutes
    ...
    for (package, timeout_ms) in &integration.cross_package_timeouts {
        if *timeout_ms < 1000 {
            warn!(
                "Short timeout for package '{}' may cause premature failures",
                package
            );
        }
        if *timeout_ms > 60000 {
            // 1 minute
            warn!(
                "Long timeout for package '{}' may delay error detection",
                package
            );
        }
    }
...
pub async fn perform_configuration_health_check(config: &NodeConfig) -> ConfigHealthReport {
    ...
    if !config.crypto_config.post_quantum_enabled
        && config.environment == super::Environment::Mainnet
    {
    ...
    if !config.zk_config.plonky2_enabled {
        ...
        report.add_recommendation(
            "Enable plonky2_enabled = true in zk_config to activate privacy features",
        );
    }
    ...
    if config.network_config.max_peers < 10 {
        ...
        report.add_recommendation(
            "Consider increasing max_peers to at least 20 for better network redundancy",
        );
    }
    ...
    if config.resource_allocations.max_memory_mb < 1024 {
        ...
        report.add_recommendation(
            "Increase max_memory_mb to at least 2048 MB for optimal performance",
        );
    }
    ...
    if config.economics_config.ubi_enabled && config.economics_config.daily_ubi_amount > 1000 {
        ...
        report.add_recommendation(
            "Consider reducing daily_ubi_amount below 1000 to maintain economic balance",
        );
    }
    ...
    if config.environment == super::Environment::Mainnet {
        if config.data_directory.is_empty() {
            report.add_critical(
                "Data directory not configured - DHT storage will not persist across restarts",
            );
        }
        ...
        if config.data_directory.is_empty() {
            ...
            report.add_recommendation(
                "Set data_directory in config to enable persistence for testnet",
            );
        }
    }
