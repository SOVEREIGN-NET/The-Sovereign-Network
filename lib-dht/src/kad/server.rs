...
impl Server {
    ...
    pub fn start(&mut self, port: u16) -> io::Result<()> {
        ...
        self.server = Some(UdpSocket::bind(SocketAddr::from((
    ...
    pub fn register_message(&mut self, constructor: fn() -> Box<dyn MethodMessageBase>) {
        ...
        self.messages.insert(
            MessageKey::new(message.get_method(), message.get_type()),
            constructor,
        );
    }
                    ...
                    MessageType::ReqMsg => {
                        if let Err(e) = || -> Result<(), MessageException> {
                            let message_key = MessageKey::new(
                                ben.get::<BencodeBytes>(t.rpc_type_name())
                                    .ok_or_else(|| MessageException::new("Method Unknown", 204))?
                                    .as_str(),
                                t,
                            );
    ...
    pub fn send_with_callback(
        &mut self,
        message: &mut dyn MethodMessageBase,
    ...
    pub fn send_with_node_callback(
        &mut self,
        message: &mut dyn MethodMessageBase,
        node: Node,
        callback: Box<dyn ResponseCallback>,
