...
run_gate \
  ...
  cargo test -p lib-blockchain --test token_regression_tests test_mint_custom_token_by_creator -- --nocapture

run_gate \
  "TokenMint unauthorized signer rejected" \
  cargo test -p lib-blockchain --test token_regression_tests test_mint_custom_token_unauthorized_rejected -- --nocapture

# Treasury-kernel enforcement.
run_gate \
  "TreasuryKernel mint missing auth rejected" \
  cargo test -p lib-blockchain --lib test_execute_authorized_mint_missing_auth_fails -- --nocapture

run_gate \
  "TreasuryKernel burn delay enforced" \
  cargo test -p lib-blockchain --lib test_execute_authorized_burn_before_delay_fails -- --nocapture
...
run_gate \
  ...
  cargo test -p lib-blockchain --test consensus_integration_tests test_dao_proposal_creation -- --nocapture

run_gate \
  "DAO vote tx integration path" \
  cargo test -p lib-blockchain --test consensus_integration_tests test_dao_vote_creation -- --nocapture
