...
use lib_network::handshake::{HandshakeContext, NetworkEpoch, NonceCache};
use lib_network::{ClientHello, HandshakeCapabilities};
...
async fn test_invalid_node_id_rejected() -> Result<()> {
    ...
    assert_ne!(
        identity.node_id, fake_node_id,
        "Fake NodeId should not match"
    );
...
async fn test_mismatched_did_rejected() -> Result<()> {
    ...
    // Should not match
    assert_ne!(
        identity.node_id, derived_from_wrong,
        "Mismatched DID should produce different NodeId"
    );
...
async fn test_mismatched_device_rejected() -> Result<()> {
    ...
    assert_ne!(
        identity.node_id, derived_from_wrong,
        "Mismatched device should produce different NodeId"
    );
...
async fn test_handshake_serialization() -> Result<()> {
    ...
    let expected_node_id =
        NodeId::from_did_device(&deserialized.identity.did, &deserialized.identity.device_id)?;
...
async fn test_multiple_devices_same_identity() -> Result<()> {
    ...
    assert_ne!(
        identity1.node_id, cross_derived,
        "Cross-device derivation should differ"
    );
...
async fn test_tampered_node_id_detected() -> Result<()> {
    ...
    // Should not match
    assert_ne!(
        identity.node_id, tampered_node_id,
        "Tampered NodeId should not match"
    );
...
async fn test_verification_across_identity_types() -> Result<()> {
    ...
    let iot = ZhtpIdentity::new_unified(IdentityType::Device, None, None, "sensor1", None)?;
