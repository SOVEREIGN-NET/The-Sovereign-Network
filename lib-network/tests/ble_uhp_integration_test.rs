...
use lib_network::protocols::bluetooth::{gatt_adapter::VerificationHook, mock::MockGattLink};
...
async fn test_uhp_verification_rejects_unverified_peers() -> Result<()> {
    ...
    let verification_hook: VerificationHook = Arc::new(|payload| payload.first() == Some(&0x01));
...
async fn test_bidirectional_uhp_over_ble() -> Result<()> {
    ...
    mock_link
        .peripheral
        .send_frame(&response_to_central)
        .await?;
