...
async fn test_deterministic_node_id_from_identity() -> Result<()> {
    ...
    assert_eq!(
        identity.did, identity2.did,
        "Same seed should produce same DID"
    );
    assert_eq!(
        identity.node_id, identity2.node_id,
...
async fn test_different_seeds_produce_different_node_ids() -> Result<()> {
    ...
    assert_ne!(
        identity1.node_id, identity2.node_id,
...
async fn test_node_id_validation_missing() -> Result<()> {
    ...
    assert!(
        peer_without_node_id.node_id.is_none(),
        "PeerInfo should have no NodeId"
    );
...
async fn test_node_id_determinism_golden_vector() -> Result<()> {
    ...
    assert_eq!(
        ...
        "DID must be anchored to root signing public key"
    );
    ...
    assert_eq!(
        identity.node_id, expected_node_id,
        "NodeId should derive from DID + device"
    );
...
async fn test_node_id_case_insensitivity() -> Result<()> {
    ...
    assert_eq!(
        lowercase.node_id, uppercase.node_id,
        "Device names should be case-insensitive"
    );
    assert_eq!(
        lowercase.node_id, mixed.node_id,
        "Device names should be case-insensitive"
    );

    println!("✅ Case insensitivity test passed");
    println!(
        "   All variants produce NodeId: {}",
        lowercase.node_id.to_hex()
    );
...
async fn test_identity_types_all_support_node_id() -> Result<()> {
    ...
    assert!(
        human.node_id.as_bytes().len() == 32,
        "Human should have 32-byte NodeId"
    );
    ...
    assert!(
        device.node_id.as_bytes().len() == 32,
        "Device should have 32-byte NodeId"
    );
    ...
    assert!(
        org.node_id.as_bytes().len() == 32,
        "Organization should have 32-byte NodeId"
    );
