...
mod mesh_tests {
    ...
    async fn test_nodeid_persistence_across_restarts() {
        ...
        restarted.restore();
        assert_eq!(
            restarted.node_id, original_id,
            "NodeId must persist across restarts"
        );
    }
    ...
    async fn test_three_phase_handshake() {
        ...
        assert!(
            ok,
            "Three-phase handshake simulation should succeed and be symmetric"
        );
    }
    ...
    async fn test_network_partition_and_recovery() {
        ...
        let mut group_a = vec![
            Node::new("did:zhtp:pa-0", "a0"),
            Node::new("did:zhtp:pa-1", "a1"),
        ];
        let mut group_b = vec![
            Node::new("did:zhtp:pb-0", "b0"),
            Node::new("did:zhtp:pb-1", "b1"),
        ];
        // Advertise only within groups
        for n in group_a.iter() {
            n.advertise_multicast(&mut fabric_a);
        }
        for n in group_b.iter() {
            n.advertise_multicast(&mut fabric_b);
        }
        for n in group_a.iter() {
            n.discover_peers(&fabric_a);
        }
        for n in group_b.iter() {
            n.discover_peers(&fabric_b);
        }
        ...
        for n in group_a.iter().chain(group_b.iter()) {
            n.advertise_multicast(&mut unified);
        }
        for n in group_a.iter().chain(group_b.iter()) {
            n.discover_peers(&unified);
        }
...
mod net_tests {
    ...
    async fn mesh_integration_multi_node_deterministic_nodeids() {
        ...
        while start.elapsed() < collection_duration {
            if let Ok(ev) =
                tokio::time::timeout(tokio::time::Duration::from_millis(500), peer_rx.recv()).await
            {
        ...
        assert!(
            hello_count >= node_count as usize,
            "expected at least {} hellos, saw {}",
            node_count,
            hello_count
        );
        ...
        assert_eq!(
            id_first, id_second,
            "deterministic NodeId must be reproducible"
        );
        ...
        assert_eq!(
            id_first, id_after_restart,
            "NodeId persisted across restart simulation"
        );
    ...
    async fn mesh_integration_partition_and_recovery() {
        ...
        for i in 0..2u8 {
            ...
            let peer_tx_clone = peer_tx.clone();
            tokio::spawn(
                async move { spawn_simple_node(did, device, seed, srx, peer_tx_clone).await },
            );
        }
        ...
        tokio::spawn(async move {
            spawn_simple_node(did_bridge, device_bridge, 0xDEADBEEFu64, srx, peer_tx).await
        });
        ...
        for _ in 0..10 {
            if let Ok(Some(ev)) =
                tokio::time::timeout(tokio::time::Duration::from_millis(500), peer_rx.recv()).await
            {
                if ev.contains("bridge") {
                    saw_bridge_msgs = true;
                    break;
                }
            }
        }
        ...
        assert!(
            saw_bridge_msgs,
            "expected bridging node to enable cross-partition discovery"
        );
    }
}
