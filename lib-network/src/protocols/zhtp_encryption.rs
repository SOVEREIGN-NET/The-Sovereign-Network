...
use lib_crypto::post_quantum::kyber::{
    kyber1024_decapsulate, kyber1024_encapsulate, kyber1024_keypair,
};
...
impl ZhtpEncryptionSession {
    ...
    pub fn new() -> Result<Self> {
        ...
        debug!(
            "Generated Kyber1024 keypair (public: {} bytes, secret: {} bytes)",
            kyber_public.len(),
            kyber_secret.len()
        );
    ...
    pub fn create_key_exchange_init(&self, session_id: String) -> Result<ZhtpKeyExchangeInit> {
        info!(
            " Creating Kyber key exchange initiation for session: {}",
            &session_id[..8]
        );
    ...
    pub fn respond_to_key_exchange(
        ...
        info!(
            " Responding to Kyber key exchange for session: {}",
            &init.session_id[..8]
        );
        ...
        debug!(
            "Encapsulated shared secret (ciphertext: {} bytes)",
            ciphertext.len()
        );
    ...
    pub fn complete_key_exchange(&mut self, response: &ZhtpKeyExchangeResponse) -> Result<()> {
        info!(
            "🔓 Completing Kyber key exchange for session: {}",
    ...
    pub fn encrypt_message(
        ...
        debug!(
            " Encrypting message ({} bytes) with ChaCha20-Poly1305",
            plaintext.len()
        );
        ...
        debug!(
            " Message encrypted (ciphertext: {} bytes)",
            ciphertext.len()
        );
    ...
    pub fn decrypt_message(&mut self, encrypted_msg: &ZhtpEncryptedMessage) -> Result<Vec<u8>> {
        let shared_secret = self
            .shared_secret
            .ok_or_else(|| anyhow!("Encryption session not established"))?;

        debug!(
            "🔓 Decrypting message ({} bytes) with ChaCha20-Poly1305",
...
pub struct ZhtpEncryptionManager {
    ...
    sessions: std::sync::Arc<
        tokio::sync::RwLock<std::collections::HashMap<String, ZhtpEncryptionSession>>,
    >,
}
