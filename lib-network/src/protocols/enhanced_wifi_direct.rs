...
impl MacOSWiFiDirectManager {
    ...
    pub async fn enumerate_wifi_interfaces(&mut self) -> Result<Vec<MacOSWiFiInterface>> {
            ...
            if line.contains("Wi-Fi") || line.contains("WiFi") {
                if let Some(interface) = current_interface.take() {
                    ...
                    self.interface_cache
                        .insert(interface.device.clone(), interface);
                }
        ...
        info!(
            "🍎 macOS: Found {} WiFi interfaces, {} P2P capable",
            interfaces.len(),
    ...
    pub async fn create_p2p_group(
        &mut self,
        interface: &str,
        group_name: &str,
    ) -> Result<MacOSP2PGroup> {
        use std::process::Command;

        info!(
            "🍎 macOS: Creating P2P group '{}' on interface {}",
                    ...
                    if line.contains("channel") {
                        if let Some(channel_info) = line.split(':').nth(1) {
                            if let Ok(channel) = channel_info.trim().parse::<u16>() {
                                group_info.frequency = 2412 + (channel - 1) * 5;
        ...
        self.p2p_groups
            .insert(group_name.to_string(), group_info.clone());
    ...
    pub async fn connect_to_p2p_group(
        ...
        info!(
            "🍎 macOS: Connecting to P2P group '{}' via interface {}",
            target_ssid, interface
        );
        ...
        let connect_output = Command::new("networksetup").args(&connect_args).output();
    ...
    async fn verify_p2p_connection(&self, interface: &str, expected_ssid: &str) -> Result<()> {
        ...
        if output_str.contains(expected_ssid) {
            info!(
                " macOS: P2P connection verified for SSID '{}'",
                expected_ssid
            );
            Ok(())
        } else {
    ...
    pub async fn transmit_p2p_message(&self, target_device: &str, message: &[u8]) -> Result<()> {
        ...
        info!(
            "🍎 macOS: Transmitting {} bytes to P2P device {}",
            message.len(),
            target_device
        );
