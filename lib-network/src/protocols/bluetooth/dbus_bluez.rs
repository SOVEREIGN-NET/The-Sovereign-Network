...
impl BlueZDBusClient {
    ...
    pub fn start_discovery(&self) -> Result<(), anyhow::Error> {
        ...
        let proxy =
            self.connection
                .with_proxy(BLUEZ_SERVICE, &self.adapter_path, Duration::from_secs(5));

        proxy
            .method_call(ADAPTER_INTERFACE, "StartDiscovery", ())
    ...
    pub fn stop_discovery(&self) -> Result<(), anyhow::Error> {
        ...
        proxy
            .method_call(ADAPTER_INTERFACE, "StopDiscovery", ())
    ...
    pub fn get_devices(&self) -> Result<Vec<DeviceInfo>> {
        ...
        let proxy = self
            .connection
            .with_proxy(BLUEZ_SERVICE, "/org/bluez", Duration::from_secs(5));

        // Get managed objects (all BlueZ objects)
        let (objects,): (
            HashMap<dbus::Path, HashMap<String, HashMap<String, Variant<Box<dyn RefArg>>>>>,
        ) = proxy
            .method_call(
    ...
    pub fn connect_device(&self, device_address: &str) -> Result<(), anyhow::Error> {
        ...
        proxy
            .method_call(DEVICE_INTERFACE, "Connect", ())
    ...
    pub fn disconnect_device(&self, device_address: &str) -> Result<(), anyhow::Error> {
        ...
        let proxy = self
            .connection
            .with_proxy(BLUEZ_SERVICE, device_path, Duration::from_secs(10));

        proxy
            .method_call(DEVICE_INTERFACE, "Disconnect", ())
    ...
    pub fn read_gatt_characteristic(
        ...
        let proxy = self
            .connection
            .with_proxy(BLUEZ_SERVICE, char_path, Duration::from_secs(10));
    ...
    pub fn write_gatt_characteristic(
        ...
        debug!(
            "✍️ Writing {} bytes to GATT characteristic {} via D-Bus",
            data.len(),
            char_uuid
        );
    ...
    pub fn enable_notifications(
        ...
        info!(
            " Enabling notifications for characteristic {} via D-Bus",
            char_uuid
        );
        ...
        proxy
            .method_call(GATT_CHARACTERISTIC_INTERFACE, "StartNotify", ())
    ...
    pub fn disable_notifications(
        ...
        debug!(
            "🔕 Disabling notifications for characteristic {}",
            char_uuid
        );
        ...
        proxy
            .method_call(GATT_CHARACTERISTIC_INTERFACE, "StopNotify", ())
