...
use crate::protocols::zhtp_auth::{NodeCapabilities, ZhtpAuthManager, ZhtpAuthVerification};
...
use super::common::{get_system_bluetooth_mac, parse_mac_address};
...
impl BluetoothClassicProtocol {
    ...
    pub async fn disable(&self) -> Result<()> {
        ...
        for peer_id in connections {
            if let Err(e) = self.disconnect_peer(&peer_id).await {
                warn!(
                    "Failed to disconnect peer {} during disable: {}",
                    peer_id, e
                );
            }
        }
    ...
    pub async fn start_advertising(&self) -> Result<()> {
        if !self.is_enabled() {
            return Err(anyhow!(
                "Bluetooth Classic is disabled. Call enable() first."
            ));
        }
    ...
    async fn windows_register_rfcomm_service(&self) -> Result<()> {
        use windows::{
            Devices::Bluetooth::Rfcomm::*, Foundation::TypedEventHandler, Networking::Sockets::*,
            Storage::Streams::*,
        };
    ...
    async fn linux_register_rfcomm_service(&self) -> Result<()> {
        ...
        let output = std::process::Command::new("sdptool")
            .args(&[
                "add",
                "--channel",
                &rfcomm_channels::MESH_DATA.to_string(),
        ...
        match output {
            Ok(result) if result.status.success() => {
                ...
                if power_state == "1" {
                    ...
                    warn!(
                        "  macOS: Bluetooth may be disabled (power state: {})",
                        power_state
                    );
                }
            }
    ...
    async fn windows_accept_rfcomm(&self) -> Result<RfcommStream> {
        ...
        {
            use windows::{Networking::Sockets::*, Storage::Streams::*};
            ...
            let socket = tokio::time::timeout(std::time::Duration::from_secs(60), rx.recv())
                .await
            ...
            Ok(RfcommStream::from_windows_socket(
                socket,
                reader,
                writer,
                peer_address,
        ...
        #[cfg(not(feature = "windows-gatt"))]
        {
            ...
            Err(anyhow!(
                "Windows RFCOMM support requires --features windows-gatt"
            ))
        }
    }
    ...
    async fn linux_accept_rfcomm(&self) -> Result<RfcommStream> {
        ...
        info!(
            " Linux: RFCOMM socket listening on channel {}",
            RFCOMM_CHANNEL
        );
    ...
    async fn macos_accept_rfcomm(&self) -> Result<RfcommStream> {
        ...
        // SAFETY: This calls libc socket APIs with valid OS file descriptors and pointers to stack/borrowed buffers that remain valid for the duration of each syscall.
        let sock_fd = unsafe { libc::socket(AF_BLUETOOTH, libc::SOCK_STREAM, BTPROTO_RFCOMM) };
        ...
        info!(
            " macOS: RFCOMM socket listening on channel {}",
            RFCOMM_CHANNEL
        );
        ...
        let peer_address = format!(
            ...
            peer_addr.rc_bdaddr[0],
            peer_addr.rc_bdaddr[1],
            peer_addr.rc_bdaddr[2],
            peer_addr.rc_bdaddr[3],
            peer_addr.rc_bdaddr[4],
            peer_addr.rc_bdaddr[5]
        );

        info!(
            " macOS: RFCOMM connection accepted from {} (fd: {})",
            peer_address, client_fd
        );
    ...
    pub async fn send_mesh_message(&self, target_address: &str, message: &[u8]) -> Result<()> {
        info!(
            " Sending RFCOMM message to {}: {} bytes",
            target_address,
            message.len()
        );
    ...
    pub async fn send_mesh_envelope(
        ...
        let target_address = target_address.ok_or_else(|| {
            anyhow!(
                "No active Bluetooth connection to peer {:?}",
                hex::encode(&peer_id.key_id[0..8])
            )
        })?;

        info!(
            "📤 Sending mesh envelope {} to {:?} via RFCOMM",
            envelope.message_id,
    ...
    async fn linux_transmit_rfcomm(&self, data: &[u8], address: &str) -> Result<()> {
        ...
        debug!(
            "Linux: RFCOMM transmit to {} ({} bytes)",
            address,
            data.len()
        );
        ...
        info!(
            " Linux: Transmitted {} bytes to {} via RFCOMM",
            data.len(),
            address
        );
        Ok(())
    ...
    async fn windows_transmit_rfcomm(&self, data: &[u8], address: &str) -> Result<()> {
        #[cfg(feature = "windows-gatt")]
        {
            ...
            debug!(
                "Windows: RFCOMM transmit to {} ({} bytes)",
                address,
                data.len()
            );
            ...
            info!(
                " Windows: Transmitted {} bytes to {} via RFCOMM",
                data.len(),
                address
            );
            Ok(())
        ...
        {
            debug!(
                "Windows: RFCOMM transmit to {} ({} bytes) - feature disabled",
                address,
                data.len()
            );
    ...
    async fn macos_transmit_rfcomm(&self, data: &[u8], address: &str) -> Result<()> {
        debug!(
            "macOS: RFCOMM transmit to {} ({} bytes)",
            address,
            data.len()
        );
        ...
        info!(
            " macOS: Transmitted {} bytes to {} via RFCOMM",
            data.len(),
            address
        );
    ...
    pub async fn query_rfcomm_services(
        &self,
        device_address: &str,
    ) -> Result<Vec<RfcommServiceInfo>> {
        #[cfg(target_os = "windows")]
        {
    ...
    pub async fn connect_to_peer(&self, device_address: &str, channel: u8) -> Result<RfcommStream> {
        ...
        let stream = self
            .connect_to_peer_internal(device_address, channel)
            .await?;
        ...
        self.active_streams
            .write()
            .await
            .insert(device_address.to_string(), stream_arc.clone());
        ...
        Err(anyhow!(
            "Stream stored successfully. Use send_mesh_message() to transmit data to {}",
            device_address
        ))
    }
    ...
    async fn discover_paired_devices_windows(&self) -> Result<Vec<BluetoothDevice>> {
        #[cfg(feature = "windows-gatt")]
        {
            ...
            let count = device_info_collection
                .Size()
                    ...
                    if let Ok(id) = device_info.Id() {
                        ...
                        if let Ok(async_op) = WinBluetoothDevice::FromIdAsync(&id) {
                            if let Ok(bt_device) = async_op.get() {
                                ...
                                let device_class = bt_device
                                    .ClassOfDevice()
                                    .ok()
                                    .and_then(|cod| cod.RawValue().ok())
                                    .unwrap_or(0);

                                let is_connected = bt_device
                                    .ConnectionStatus()
                                    .ok()
        ...
        {
            Err(anyhow!(
                "Windows discovery requires --features windows-gatt"
            ))
        }
    }

    #[cfg(all(target_os = "windows", feature = "windows-gatt"))]
    fn format_bluetooth_address_windows(
        device: &windows::Devices::Bluetooth::BluetoothDevice,
    ) -> Result<String> {
        let address = device
            .BluetoothAddress()
    ...
    async fn query_services_windows(&self, device_address: &str) -> Result<Vec<RfcommServiceInfo>> {
        #[cfg(feature = "windows-gatt")]
        {
            use windows::Devices::Bluetooth::{BluetoothDevice, Rfcomm::RfcommDeviceService};
            ...
            info!(
                " Windows: Found {} RFCOMM services on {}",
                rfcomm_services.len(),
                device_address
            );
            Ok(rfcomm_services)
        ...
        {
            Err(anyhow!(
                "Windows service query requires --features windows-gatt"
            ))
        }
    }
    ...
    fn parse_bluetooth_address_to_u64(address: &str) -> Result<u64> {
        ...
        u64::from_str_radix(&clean, 16).map_err(|e| anyhow!("Failed to parse address: {}", e))
    }

    #[cfg(target_os = "windows")]
    async fn connect_to_peer_windows(
        &self,
        device_address: &str,
        channel: u8,
    ) -> Result<RfcommStream> {
        #[cfg(feature = "windows-gatt")]
        {
            ...
            info!(
                " Windows: Connecting to RFCOMM service on {} channel {}",
            ...
            let socket =
                StreamSocket::new().map_err(|e| anyhow!("Failed to create socket: {:?}", e))?;

            let hostname = bt_device
                .HostName()
        ...
        {
            Err(anyhow!(
                "Windows connection requires --features windows-gatt"
            ))
        }
    }
                    ...
                    for device_entry in device_dirs.flatten() {
                        ...
                        if let Ok(content) = std::fs::read_to_string(&info_file) {
                            if content.contains("Paired=true") {
                                let address =
                                    device_entry.file_name().to_string_lossy().replace('_', ":");
    ...
    async fn query_services_linux(&self, device_address: &str) -> Result<Vec<RfcommServiceInfo>> {
                    ...
                    if line.contains("Service RecHandle:") || line.contains("Service Class ID:") {
                        if let Some(uuid_part) = line.split("0x").nth(1) {
                            if let Some((_, name, channel)) = current_service.as_ref() {
                                current_service =
                                    Some((uuid_part.trim().to_string(), name.clone(), *channel));
                            }
                        }
                    }
        ...
        info!(
            " Linux: Found {} RFCOMM services on {}",
            services.len(),
            device_address
        );
        Ok(services)
    ...
    #[cfg(target_os = "linux")]
    async fn connect_to_peer_linux(
        &self,
        device_address: &str,
        channel: u8,
    ) -> Result<RfcommStream> {
        info!(
            " Linux: Connecting to RFCOMM service on {} channel {}",
            device_address, channel
        );
        ...
        info!(
            " Linux: Connected to {} channel {} (fd: {})",
            device_address, channel, sock_fd
        );
        ...
        Ok(RfcommStream::from_linux_socket(
            sock_fd,
    ...
    async fn query_services_macos(&self, device_address: &str) -> Result<Vec<RfcommServiceInfo>> {
        ...
        let services = vec![RfcommServiceInfo {
            service_uuid: "6ba7b810-9dad-11d1-80b4-00c04fd430ca".to_string(),
        ...
        info!(
            " macOS: Returning default ZHTP service for {}",
            device_address
        );
        Ok(services)
    }
    ...
    #[cfg(target_os = "macos")]
    async fn connect_to_peer_macos(
        &self,
        device_address: &str,
        channel: u8,
    ) -> Result<RfcommStream> {
        info!(
            "🍎 macOS: Connecting to RFCOMM service on {} channel {}",
            device_address, channel
        );
        ...
        if connect_result < 0 {
            ...
            let errno = unsafe { *libc::__error() };
            return Err(anyhow!(
                "Failed to connect to RFCOMM device: errno {}",
                errno
            ));
        }
    ...
    pub async fn handle_incoming_messages(&self, stream: Arc<RwLock<RfcommStream>>) -> Result<()> {
        ...
        info!(
            "📬 Starting message handler for RFCOMM connection: {}",
            peer_addr
        );
        ...
        loop {
            ...
            match buffer.get(0) {
                ...
                Some(0x02) => {
                    ...
                    if let Err(e) = self
                        .handle_quantum_routing_message(&buffer[1..], &peer_addr)
                        .await
                    {
                ...
                Some(0x03) => {
                    ...
                    if let Err(e) = self
                        .handle_mesh_data_message(&buffer[1..], &peer_addr)
                        .await
                    {
                ...
                Some(0x04) => {
                    ...
                    if let Err(e) = self
                        .handle_coordination_message(&buffer[1..], &peer_addr)
                        .await
                    {
                ...
                Some(msg_type) => {
                    warn!(
                        " Unknown message type from {}: 0x{:02X}",
                        peer_addr, msg_type
                    );
                }
                None => {
    ...
    async fn handle_zk_auth_message(&self, data: &[u8], peer_addr: &str) -> Result<()> {
        info!(
            " Processing ZK auth message from {} ({} bytes)",
            peer_addr,
            data.len()
        );
        ...
        if let Ok(challenge) =
            serde_json::from_slice::<crate::protocols::zhtp_auth::ZhtpAuthChallenge>(data)
        {
            ...
            serde_json::from_slice::<crate::protocols::zhtp_auth::ZhtpAuthResponse>(data)
        {
                    ...
                    Ok(verification) => {
                        if verification.authenticated {
                            info!(
                                ...
                                peer_addr, verification.trust_score
                            );
                            self.authenticated_peers
                                .write()
                                .await
                                .insert(peer_addr.to_string(), verification);
                        } else {
    ...
    async fn handle_quantum_routing_message(&self, data: &[u8], peer_addr: &str) -> Result<()> {
        info!(
            "🔮 Processing quantum routing message from {} ({} bytes)",
            peer_addr,
            data.len()
        );
    ...
    async fn handle_mesh_data_message(&self, data: &[u8], peer_addr: &str) -> Result<()> {
        info!(
            " Received mesh data message from {} ({} bytes)",
            peer_addr,
            data.len()
        );
        ...
        if let Some(handler) = &self.message_handler {
            ...
            match message {
                ...
                ZhtpMeshMessage::BlockchainRequest {
                    ...
                    handler_guard
                        .handle_blockchain_request(requester, request_id, request_type)
                        .await?;
                }
    ...
    async fn forward_message(&self, mut envelope: MeshMessageEnvelope) -> Result<()> {
        ...
        info!(
            "🔀 Forwarding message {} (hop {})",
            envelope.message_id, envelope.hop_count
        );
        ...
        if let Some(router) = &self.message_router {
            ...
            match router_guard
                .find_next_hop_for_destination(&dest_unified)
                .await
            {
                Ok(next_hop) => {
                    info!(
                        " Next hop: {:?}",
                        hex::encode(&next_hop.public_key().key_id[0..4])
                    );

                    // Send to next hop (extract PublicKey for envelope)
                    self.send_mesh_envelope(next_hop.public_key(), &envelope)
                        .await?;
    ...
    async fn handle_coordination_message(&self, data: &[u8], peer_addr: &str) -> Result<()> {
        info!(
            " Processing coordination message from {} ({} bytes)",
            peer_addr,
            data.len()
        );
            ...
            loop {
                match self_clone.accept_connection_on_channel(channel).await {
                    Ok(stream) => {
                        ...
                        info!(
                            " RFCOMM connection accepted on channel {}: {}",
                            channel, peer_addr
                        );
                        ...
                        connections
                            .write()
                            .await
                            .insert(peer_addr.clone(), connection);
                        ...
                        streams
                            .write()
                            .await
                            .insert(peer_addr.clone(), stream_arc.clone());
    ...
    async fn accept_connection_on_channel(&self, channel: u8) -> Result<RfcommStream> {
        ...
        info!(
            "Accepting connection on channel {} (using default listener)",
            channel
        );
    ...
    pub async fn start_full_service(&self) -> Result<()> {
        ...
        self.listen_on_channel(rfcomm_channels::QUANTUM_ROUTING)
            .await?;
        info!(
            " Channel {} (QUANTUM_ROUTING) listening",
            rfcomm_channels::QUANTUM_ROUTING
        );

        self.listen_on_channel(rfcomm_channels::MESH_DATA).await?;
        info!(
            " Channel {} (MESH_DATA) listening",
            rfcomm_channels::MESH_DATA
        );

        self.listen_on_channel(rfcomm_channels::COORDINATION)
            .await?;
        info!(
            " Channel {} (COORDINATION) listening",
            rfcomm_channels::COORDINATION
        );
...
impl super::super::Protocol for BluetoothClassicProtocol {
    ...
    async fn connect(
        &mut self,
        target: &super::super::PeerAddress,
    ) -> Result<super::super::ProtocolSession> {
        ...
        let peer_address = match target {
            ...
            _ => {
                return Err(anyhow!(
                    "BluetoothClassicProtocol only supports Bluetooth addresses"
                ))
            }
        };
        ...
        let _stream = self
            .connect_to_peer(&peer_address, rfcomm_channels::MESH_DATA)
            .await
    ...
    async fn accept(&mut self) -> Result<super::super::ProtocolSession> {
        ...
        let mac_bytes = hex::decode(peer_address.replace(':', "").as_str()).unwrap_or_default();
        if mac_bytes.len() != 6 {
    ...
    async fn receive_message(
        ...
        Err(anyhow!(
            "Receive message not fully implemented for BluetoothClassic"
        ))
    }
