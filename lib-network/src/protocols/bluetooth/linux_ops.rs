...
impl LinuxBluetoothOps {
    ...
    pub async fn read_gatt_characteristic(
        ...
        self.cli_read_gatt_characteristic(device_address, char_uuid)
            .await
    }
    ...
    pub async fn write_gatt_characteristic(
        ...
        self.cli_write_gatt_characteristic(device_address, char_uuid, data)
            .await
    }
        ...
        for device in devices {
            ...
            if let Some(ref name) = device.name {
                if name.contains("ZHTP") || name.contains("SOVNET") {
                    ...
                    info!(
                        " Found ZHTP mesh peer: {} ({})",
                        peer.peer_id, device.address
                    );
                    peers.push(peer);
                }
            }
    ...
    async fn cli_read_gatt_characteristic(
        &self,
        _device_address: &str,
        char_uuid: &str,
    ) -> Result<Vec<u8>> {
    ...
    async fn cli_write_gatt_characteristic(
        &self,
        _device_address: &str,
        char_uuid: &str,
        _data: &[u8],
