    ...
    pub fn add_fragment(&mut self, fragment: Vec<u8>) -> Result<Option<Vec<u8>>> {
        ...
        if received_count == total_fragments as usize {
            ...
            for i in 0..total_fragments {
                if let Some(fragment_data) = self
                    .fragments
                    .get(&message_id)
                    .and_then(|f| f.get(&(i as u8)))
                {
            ...
            info!(
                " Reassembled message {} from {} fragments ({} bytes)",
                message_id,
                total_fragments,
                complete_data.len()
            );
                ...
                if data.len() >= 11 && data.starts_with(&[0xED, 0x6E]) {
                    ...
                    if let Ok(edge_msg) = bincode::deserialize::<EdgeSyncMessage>(&data[2..]) {
                        match edge_msg {
                            ...
                            EdgeSyncMessage::HeadersResponse {
                                request_id,
                                headers,
                            ...
                            EdgeSyncMessage::BootstrapProofRequest {
                                request_id,
                                current_height,
                            ...
                            EdgeSyncMessage::BootstrapProofResponse {
                                request_id,
                                proof_data,
                                proof_height,
                                headers,
                ...
                // Check for fragmented message
                    ...
                    GattMessage::FragmentHeader {
                        ...
                        total_fragments: u16::from_le_bytes(
                            data[8..10].try_into().unwrap_or_default(),
                        ),
                        fragment_index: u16::from_le_bytes(
                            data[10..12].try_into().unwrap_or_default(),
                        ),
                        data: data[12..].to_vec(),
                    }
                } else if data.len() >= 8 {
