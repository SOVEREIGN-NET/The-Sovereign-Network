...
use windows::{
    core::*, Devices::Bluetooth::Advertisement::*, Devices::Bluetooth::GenericAttributeProfile::*,
    Devices::Bluetooth::*, Foundation::Collections::*, Foundation::*, Storage::Streams::*,
};

// Import common Bluetooth utilities
use crate::constants::BLE_MESH_SERVICE_UUID;
#[cfg(all(target_os = "windows", feature = "windows-gatt"))]
use crate::protocols::bluetooth::common::{format_mac_address, parse_uuid_to_guid};
#[cfg(target_os = "windows")]
...
impl WindowsGattManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        #[cfg(all(target_os = "windows", feature = "windows-gatt"))]
        {
            ...
            info!(
                " Windows GATT manager initialized - radio state will be checked during operation"
            );
        }
    ...
    pub fn create_event_channel() -> (
        mpsc::UnboundedSender<GattEvent>,
        mpsc::UnboundedReceiver<GattEvent>,
    ) {
                    ...
                    if let Some(args) = args {
                        if let Ok(tx_lock) = event_tx.try_lock() {
                            if let Some(ref tx) = *tx_lock {
                                let address =
                                    format!("{:012X}", args.BluetoothAddress().unwrap_or(0));
                                ...
                                let zhtp_uuid_clean =
                                    BLE_MESH_SERVICE_UUID.replace("-", "").to_uppercase();
                                ...
                                if has_zhtp_service {
                                    ...
                                    if let Ok(mut discovered) = discovered_devices.try_write() {
                                        if discovered.insert(address.clone()) {
                                            ...
                                            info!(
                                                " Windows: Discovered ZHTP device {} RSSI: {}",
    ...
    pub async fn read_characteristic(
        ...
        info!(
            "📖 Reading GATT characteristic {}/{} on {}",
            service_uuid, char_uuid, address
        );

        #[cfg(feature = "windows-gatt")]
        {
            let characteristic = self
                .find_characteristic(address, service_uuid, char_uuid)
                .await?;
    ...
    pub async fn write_characteristic(
        ...
        info!(
            "✍️ Writing {} bytes to GATT characteristic {}/{} on {}",
            data.len(),
            service_uuid,
            char_uuid,
            address
        ...
        #[cfg(feature = "windows-gatt")]
        {
                ...
                .await
            {
                ...
                Err(e) => {
                    return Err(anyhow!(
                        "Step 1 failed - Cannot find characteristic {}/{} on {}: {}",
                        service_uuid,
                        char_uuid,
                        address,
                        e
            ...
            let data_writer = DataWriter::new()
                .map_err(|e| anyhow!("Step 2 failed - DataWriter creation: {}", e))?;
            data_writer
                .WriteBytes(data)
                .map_err(|e| anyhow!("Step 2 failed - WriteBytes: {}", e))?;
            let buffer = data_writer
                .DetachBuffer()
                .map_err(|e| anyhow!("Step 2 failed - DetachBuffer: {}", e))?;
            ...
            let properties = characteristic
                ...
                .map_err(|e| anyhow!("Step 3 failed - Cannot get properties: {}", e))?;
            info!(" Properties: {:?}", properties);

            let can_write = (properties & GattCharacteristicProperties::Write).0 != 0;
            let can_write_no_response =
                (properties & GattCharacteristicProperties::WriteWithoutResponse).0 != 0;
            ...
            if !can_write && !can_write_no_response {
                return Err(anyhow!(
                    "Step 3 failed - Characteristic does not support writing! Properties: {:?}",
                    properties
                ));
            }
            ...
            if can_write {
                ...
                if write_result != GattCommunicationStatus::Success {
                    return Err(anyhow!(
                        "Step 6 failed - GATT write returned status: {:?}",
                        write_result
                    ));
                }

                info!(
                    " Successfully wrote {} bytes to characteristic {} (WriteWithoutResponse)",
                    data.len(),
                    char_uuid
                );
            } else {
    ...
    pub async fn enable_notifications(&self, address: &str, char_uuid: &str) -> Result<()> {
        info!(
            " Enabling notifications for characteristic {} on {}",
            char_uuid, address
        );
        ...
        #[cfg(feature = "windows-gatt")]
        {
                ...
                move |_characteristic: &Option<GattCharacteristic>,
                    ...
                    if let Some(args) = args {
                        ...
                        if let Ok(buffer) = args.CharacteristicValue() {
                            info!(
                                "   Buffer obtained, length: {}",
                                buffer.Length().unwrap_or(0)
                            );
                                ...
                                if data_reader.ReadBytes(&mut data).is_ok() {
                                    ...
                                    if let Ok(tx_lock) = event_tx.try_lock() {
                                        if let Some(ref tx) = *tx_lock {
                                            ...
                                            let _ =
                                                tx.send(GattEvent::CharacteristicValueChanged {
            ...
            info!(
                "   CCCD write completed successfully - peripheral should now send notifications"
            );
    ...
    pub async fn start_gatt_server(
        ...
        info!(
            " Starting Windows GATT server with service {}",
            service_uuid
        );
            ...
            for (char_uuid, initial_value) in characteristics {
                ...
                let read_handler = TypedEventHandler::new({
                    ...
                    move |_char: &Option<GattLocalCharacteristic>,
                          _args: &Option<GattReadRequestedEventArgs>| {
                        if let Ok(tx_lock) = event_tx.try_lock() {
                            if let Some(ref tx) = *tx_lock {
    ...
    async fn find_characteristic(
        ...
        info!(
            " Finding characteristic: service={}, char={}, device={}",
            service_uuid, char_uuid, address
        );
        ...
        if services_result.Status()? != GattCommunicationStatus::Success {
            ...
            return Err(anyhow!(
                "GATT service discovery failed for device {}: status={:?}",
                address,
                status
            ));
        }
            ...
            if found_service_uuid == target_service_uuid {
                ...
                if chars_result.Status()? == GattCommunicationStatus::Success {
                    ...
                    for j in 0..char_count {
                        ...
                        info!(
                            "   Char {}: {:?} (properties: {:?})",
                            j, found_char_uuid, properties
                        );

                        if found_char_uuid == target_char_uuid {
                            info!(
                                " Found matching characteristic with properties: {:?}",
                                properties
                            );
        ...
        Err(anyhow!(
            "Characteristic not found: service={}, char={}, device={}",
            service_uuid,
            char_uuid,
            address
        ))
...
impl WindowsGattManager {
    pub fn new() -> anyhow::Result<Self> {
        Err(anyhow::anyhow!(
            "Windows GATT manager only available on Windows"
        ))
    }
}
