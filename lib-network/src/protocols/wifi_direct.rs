...
use crate::protocols::enhanced_wifi_direct::{
    AdvancedWPSSecurity, MacOSP2PGroup, MacOSWiFiDirectManager, MacOSWiFiInterface,
};
...
pub struct WiFiDirectMeshProtocol {
    ...
    pub wifi_direct_publisher:
        Arc<RwLock<Option<windows::Devices::WiFiDirect::WiFiDirectAdvertisementPublisher>>>,
    ...
    pub authenticated_peers:
        Arc<RwLock<HashMap<String, crate::protocols::zhtp_auth::ZhtpAuthVerification>>>,
...
impl WiFiDirectMeshProtocol {
    ...
    pub async fn disable(&self) -> Result<()> {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if let Some(publisher) = publisher_guard.take() {
                ...
                if publisher
                    .Status()
                    .unwrap_or(WiFiDirectAdvertisementPublisherStatus::Aborted)
    ...
    pub async fn authenticate_peer(&self, device_id: &str, peer_data: &[u8]) -> Result<bool> {
        ...
        let auth_manager = match auth_guard.as_ref() {
            ...
            None => {
                warn!(
                    "  WiFi Direct authentication not initialized - rejecting connection from {}",
                    device_id
                );
                return Ok(false);
            }
        ...
        if let Ok(response) = serde_json::from_slice::<ZhtpAuthResponse>(peer_data) {
            info!(
                " Verifying ZHTP authentication from WiFi Direct peer {}",
            ...
            match auth_manager.verify_response(&response).await {
                Ok(verification) => {
                    if verification.authenticated {
                        ...
                        self.authenticated_peers
                            .write()
                            .await
                            .insert(device_id.to_string(), verification);

                        return Ok(true);
                    } else {
                        warn!(
                            " WiFi Direct peer {} authentication FAILED",
                            &device_id[..16.min(device_id.len())]
                        );
                ...
                Err(e) => {
                    warn!(
                        " WiFi Direct peer {} authentication error: {}",
                        &device_id[..16.min(device_id.len())],
                        e
                    );
                    return Ok(false);
        ...
        warn!(
            " WiFi Direct device {} did not provide ZHTP authentication",
            &device_id[..16.min(device_id.len())]
        );
    ...
    async fn init_windows_wifi_direct(&self) -> Result<()> {
        ...
        {
            use windows::{Devices::WiFiDirect::*, Foundation::*};
                ...
                Ok(selector) => {
                    ...
                    match WiFiDirectAdvertisementPublisher::new() {
                        ...
                        Err(e) => {
                            ...
                            Err(anyhow::anyhow!(
                                "WiFi Direct publisher creation failed: {:?}",
                                e
                            ))
                        }
                    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Ok(interfaces) = macos_manager.enumerate_wifi_interfaces().await {
                    let p2p_interfaces: Vec<_> =
                        interfaces.into_iter().filter(|i| i.p2p_capable).collect();
    ...
    async fn windows_scan_p2p_devices() -> Result<Vec<WiFiDirectConnection>> {
        {
            ...
            use windows::{Devices::Enumeration::*, Devices::WiFiDirect::*, Foundation::*};
            ...
            let watcher = {
                ...
                let added_handler = TypedEventHandler::new(
                    move |_watcher: &Option<DeviceWatcher>, info: &Option<DeviceInformation>| {
    ...
    async fn perform_go_negotiation(
        &mut self,
        peer_address: &str,
        ...
        if i_should_be_go {
            info!(
                " Won GO negotiation - becoming Group Owner (intent: {} vs {})",
                my_intent, peer_intent
            );
            self.go_negotiation.group_capability.p2p_group_owner = true;
        } else {
            info!(
                "Lost GO negotiation - will be Group Client (intent: {} vs {})",
                my_intent, peer_intent
            );
    ...
    async fn should_become_group_owner(&self) -> Result<bool> {
        ...
        info!(
            " Discovered {} peer(s) - will join existing network instead of creating new group",
            peers.len()
        );
    ...
    async fn create_group(&mut self) -> Result<()> {
        ...
        info!(
            "WiFi Direct group created: SSID={}, Channel={}",
    ...
    async fn windows_create_p2p_group(&mut self) -> Result<()> {
...

        {
            ...
            let publisher = WiFiDirectAdvertisementPublisher::new().map_err(|e| {
                anyhow::anyhow!("Failed to create advertisement publisher: {:?}", e)
            })?;
            ...
            info!(
                "  Note: Custom SSID '{}' requested but WinRT API manages names automatically",
                self.ssid
            );
                        ...
                        if let Ok(info_result) = info_output {
                            ...
                            if info_str.contains(ssid) || info_str.contains("ZHTP") {
                                ...
                                if let Ok(connect_result) = connect_output {
                                    let connect_str =
                                        String::from_utf8_lossy(&connect_result.stdout);
    ...
    async fn get_p2p_interface_ip(&self) -> Result<String> {
        ...
        let interface_output = Command::new("ip").args(&["link", "show"]).output();
                            ...
                            if let Ok(ip_result) = ip_output {
                                ...
                                for ip_line in ip_str.lines() {
                                    if ip_line.contains("inet ") && !ip_line.contains("127.0.0.1") {
                                        let parts: Vec<&str> =
                                            ip_line.trim().split_whitespace().collect();
                                        if parts.len() >= 2 {
    ...
    async fn perform_wps_handshake(
        &self,
        peer_address: &str,
    ...
    async fn perform_wps_pbc(&self, peer_address: &str) -> Result<String> {
        info!(
            "🔘 Starting WPS Push Button Configuration with {}",
            peer_address
        );
    ...
    async fn perform_wps_pin_display(&self, peer_address: &str, pin: &str) -> Result<String> {
        info!(
            " Starting WPS PIN Display (PIN: {}) with {}",
            pin, peer_address
        );
        ...
        #[cfg(not(any(target_os = "linux", target_os = "windows", target_os = "macos")))]
        Err(anyhow::anyhow!(
            "WPS PIN Display not supported on this platform"
        ))
    }
    ...
    async fn perform_wps_pin_keypad(&self, peer_address: &str, pin: &str) -> Result<String> {
        info!(
            "⌨️ Starting WPS PIN Keypad (entering PIN: {}) with {}",
            pin, peer_address
        );
        ...
        #[cfg(not(any(target_os = "linux", target_os = "windows", target_os = "macos")))]
        Err(anyhow::anyhow!(
            "WPS PIN Keypad not supported on this platform"
        ))
    }
    ...
    async fn linux_wps_pin_display(&self, peer_address: &str, pin: &str) -> Result<String> {
        ...
        if connect_str.contains("OK") {
            tokio::time::sleep(tokio::time::Duration::from_secs(10)).await;
            return Ok(format!(
                "WPS PIN Display connection established (PIN: {})",
                pin
            ));
        }
    ...
    async fn linux_wps_pin_keypad(&self, peer_address: &str, pin: &str) -> Result<String> {
        ...
        info!(
            " Linux WPS PIN Keypad: entering {} for {}",
            pin, peer_address
        );
        ...
        if connect_str.contains("OK") {
            tokio::time::sleep(tokio::time::Duration::from_secs(10)).await;
            return Ok(format!(
                "WPS PIN Keypad connection established (entered PIN: {})",
                pin
            ));
        }
    ...
    async fn windows_wps_pbc(&self, peer_address: &str) -> Result<String> {
        info!(
            " Starting WPS Push Button Configuration for peer: {}",
            peer_address
        );
    ...
    async fn windows_wps_pin_display(&self, peer_address: &str, pin: &str) -> Result<String> {
        info!(
            " Starting WPS PIN Display for peer: {} with PIN: {}",
            peer_address, pin
        );
    ...
    async fn windows_wps_pin_keypad(&self, peer_address: &str, pin: &str) -> Result<String> {
...

        {
            ...
            info!(
                "📱 Found device, using entered PIN {} for connection...",
                pin
            );
    ...
    async fn macos_wps_pin_keypad(&self, peer_address: &str, pin: &str) -> Result<String> {
        ...
        info!(
            "🍎 macOS WPS PIN Keypad: entering {} for {}",
            pin, peer_address
        );
    ...
    async fn send_p2p_invitation(
        ...
        info!(
            "📨 Sending P2P invitation to {} ({:?})",
            peer_address, invitation_type
        );
        ...
        #[cfg(not(any(target_os = "linux", target_os = "windows", target_os = "macos")))]
        Err(anyhow::anyhow!(
            "P2P invitations not supported on this platform"
        ))
    }
    ...
    async fn decline_p2p_invitation(
        &self,
        peer_address: &str,
        reason: InvitationStatus,
    ) -> Result<P2PInvitationResponse> {
        info!(
            " Declining P2P invitation from {} (reason: {:?})",
            peer_address, reason
        );
    ...
    async fn handle_received_invitation(&self, invitation: P2PInvitationRequest) -> Result<()> {
        info!(
            "📩 Received P2P invitation from {} for group {}",
        ...
        if should_auto_accept {
            ...
            let _response = self
                .accept_p2p_invitation(&invitation.invitee_address)
                .await?;
        } else {
    ...
    async fn join_active_group(&self, invitation: &P2PInvitationRequest) -> Result<()> {
        info!(
            " Joining active P2P group on channel {}",
            invitation.operating_channel
        );
        ...
        #[cfg(not(any(target_os = "linux", target_os = "windows", target_os = "macos")))]
        Err(anyhow::anyhow!(
            "Join active group not supported on this platform"
        ))
    }
    ...
    async fn reinvoke_persistent_group(&self, invitation: &P2PInvitationRequest) -> Result<()> {
        ...
        Err(anyhow::anyhow!(
            "Persistent group not found or not supported on this platform"
        ))
    }

    // Platform-specific P2P invitation implementations
    #[cfg(target_os = "linux")]
    async fn linux_send_p2p_invitation(
        &self,
        ...
        info!(
            " Linux sending P2P invitation to {}",
    ...
    #[cfg(target_os = "windows")]
    async fn windows_send_p2p_invitation(
        &self,
        invitation: &P2PInvitationRequest,
    ) -> Result<P2PInvitationResponse> {
        info!(
            " Windows sending P2P invitation to {}",
            invitation.invitee_address
        );

        {
            ...
            if found_devices.is_empty() {
                warn!(
                    "  Device {} not found for invitation",
                    invitation.invitee_address
                );
    ...
    async fn windows_join_active_group(&self, invitation: &P2PInvitationRequest) -> Result<()> {
        info!(
            " Windows joining active P2P group for channel {}",
            invitation.operating_channel
        );

        {
            ...
            if found_devices.is_empty() {
                warn!(
                    "  No matching P2P group found for {}",
                    invitation.persistent_group_id
                );
    ...
    async fn windows_reinvoke_persistent_group(&self, group: &PersistentGroup) -> Result<()> {
...

        {
            ...
            info!(
                "  Reinvoking group '{}' (WinRT manages credentials automatically)",
                group.group_id
            );
            ...
            info!(
                " Persistent group {} reinvoked successfully",
                group.group_id
            );
    ...
    #[cfg(target_os = "macos")]
    async fn macos_send_p2p_invitation(
        &self,
        invitation: &P2PInvitationRequest,
    ) -> Result<P2PInvitationResponse> {
        use std::process::Command;
        ...
        info!(
            "🍎 macOS sending P2P invitation to {}",
            invitation.invitee_address
        );
        ...
        let _output = Command::new("sh").args(&["-c", &script]).output()?;
    ...
    async fn macos_reinvoke_persistent_group(&self, group: &PersistentGroup) -> Result<()> {
        info!(
            "🍎 macOS reinvoking persistent group {} (via system preferences)",
            group.group_id
        );
        // macOS would require CoreWLAN framework integration
        Ok(())
    ...
    async fn register_zhtp_service(&self) -> Result<()> {
        ...
        if let Some(daemon) = &self.mdns_daemon {
            ...
            let txt_tuples: Vec<(String, String)> = service
                .txt_records
                .iter()
            ...
            let txt_refs: Vec<(&str, &str)> = txt_tuples
                .iter()
            ...
            info!(
                ...
                service_name, local_ip, service.port
            );
    ...
    async fn create_zhtp_txt_records(&self) -> HashMap<String, String> {
        ...
        // Node capabilities
        txt_records.insert("node_id".to_string(), hex::encode(&self.node_id[0..8])); // First 8 bytes for identification
                        ...
                        Ok(Ok(event)) => {
                            match event {
                                mdns_sd::ServiceEvent::ServiceResolved(info) => {
                                    ...
                                    let is_router = info.get_properties().iter().any(|prop| {
                                    ...
                                    for addr in addresses {
                                        ...
                                        if addr == own_ip {
                                            info!(
                                                "  Skipping self-discovery: {} is our own IP",
                                                addr
                                            );
                                            continue;
                                        }
                                        ...
                                        if is_new_peer {
                                            ...
                                            info!(
                                                " Added ZHTP peer {} to discovered peers",
                                                peer_addr
                                            );
                                        } else {
                                        ...
                                        if is_new_peer {
                                            ...
                                            info!(
                                                "   Peer {} available for mesh routing",
                                                peer_addr
                                            );
                                        }
                                    }
    ...
    async fn process_discovered_service(
        ...
        let group_owner = service_info
            .get("group_owner")
            .map(|s| s == "true")
            .unwrap_or(false);
        let channel = service_info
            .get("channel")
        ...
        for (mac_addr, connection) in devices.iter() {
            if !peers.contains_key(mac_addr) {
                ...
                service_info.insert(
                    ...
                    connection.signal_strength.to_string(),
                );
                service_info.insert(
                    "data_rate".to_string(),
    ...
    pub async fn get_discovered_peer_addresses(&self) -> Vec<String> {
        ...
        let addresses: Vec<String> = peers
            ...
            .collect();
        info!(
            " Returning {} discovered peer addresses for bootstrap",
            addresses.len()
        );
        addresses
    }
    ...
    async fn process_received_mesh_message(data: &[u8]) {
        ...
        if let Some(content_start) = message_str.find("\r\n\r\n") {
            ...
            if let Ok(envelope) =
                crate::types::mesh_message::MeshMessageEnvelope::from_bytes(payload)
            {
                info!(
                    " Envelope {} from {:?} to {:?} (hop {}/{})",
                    envelope.message_id,
    ...
    pub async fn send_mesh_envelope(
        ...
        info!(
            "📤 Sending mesh envelope {} via WiFi Direct to {:?}",
            envelope.message_id,
        ...
        let target_address = devices
            .iter()
            .next()
            .map(|(address, _device)| {
                info!(
                    "Using WiFi Direct device: {} for peer {:?}",
                    address,
                    hex::encode(&peer_id.key_id[0..4])
                );
                address.clone()
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "No active WiFi Direct connection to peer {:?}",
                    hex::encode(&peer_id.key_id[0..8])
                )
            })?;
    ...
    pub async fn send_mesh_message(&self, target_address: &str, message: &[u8]) -> Result<()> {
        info!(
            " Sending WiFi Direct mesh message to {}: {} bytes",
            target_address,
            message.len()
        );
        ...
        if let Some(device) = devices.get(target_address) {
            ...
            let encrypted_message = if let Some(session_key) = &device.session_key {
                ...
                warn!(
                    "⚠️  No session key for {}, sending unencrypted (WPA2/3 only)",
                    target_address
                );
                message.to_vec()
            };
            ...
            let result = self
                .transmit_over_wifi_direct(device, &encrypted_message)
                .await;

            if result.is_ok() {
                info!(
                    "Message sent via WiFi Direct to {} ({} Mbps)",
                    target_address, device.data_rate
                );
    ...
    async fn transmit_over_wifi_direct(
        ...
        #[cfg(not(any(target_os = "linux", target_os = "windows", target_os = "macos")))]
        {
            ...
            info!(
                " Generic: Sent {} bytes to {} via UDP fallback",
                message.len(),
                target_addr
            );
            Ok(())
    ...
    #[cfg(target_os = "linux")]
    async fn linux_transmit_wifi_direct(
        &self,
        device: &WiFiDirectConnection,
        message: &[u8],
    ) -> Result<()> {
    ...
    #[cfg(target_os = "windows")]
    async fn windows_transmit_wifi_direct(
        &self,
        device: &WiFiDirectConnection,
        message: &[u8],
    ) -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    async fn macos_transmit_wifi_direct(
        &self,
        device: &WiFiDirectConnection,
        message: &[u8],
    ) -> Result<()> {
        info!(
            "🍎 macOS: Transmitting {} bytes to WiFi Direct device {}",
            message.len(),
            device.mac_address
        );
        ...
        #[cfg(all(target_os = "macos", feature = "enhanced-wifi-direct"))]
        {
            ...
            return manager
                .transmit_p2p_message(&device.ip_address, message)
                .await;
        }
        ...
        #[cfg(not(feature = "enhanced-wifi-direct"))]
        {
            ...
            let _notification = Command::new("sh").args(&["-c", &script]).output();
            ...
            match TcpStream::connect(&address).await {
                ...
                Err(_) => {
                    ...
                    let header =
                        format!("ZHTP/1.0 MESH MACOS UDP\nTarget: {}\n", device.mac_address);
                    let mut packet = header.into_bytes();
                    packet.extend_from_slice(message);
                    ...
                    let transmission_time_ms =
                        (message.len() as f64 / (device.data_rate as f64 * 125.0)) * 1000.0;
                    tokio::time::sleep(tokio::time::Duration::from_millis(
                        transmission_time_ms as u64 + 10,
                    ))
                    .await;
    ...
    pub async fn start_connection_monitoring(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                info!(
                    "WiFi Direct monitoring: {} active connections",
    ...
    pub async fn get_mesh_status(&self) -> WiFiDirectMeshStatus {
        ...
        let avg_signal = if !devices.is_empty() {
            devices
                .values()
                .map(|d| d.signal_strength as i32)
                .sum::<i32>()
                / devices.len() as i32
        } else {
            -35 // Default
    ...
    async fn derive_p2p_bssid(&self, network_name: &str) -> Result<String> {
                    ...
                    if let Some(mac_start) = line.find("link/ether ") {
                        if let Some(mac_part) = line[mac_start + 11..].split_whitespace().next() {
                            ...
                            if mac_bytes.len() == 6 {
                                ...
                                let p2p_mac =
                                    format!("02:{:02x}:{}", hash_byte, &mac_bytes[2..].join(":"));
        ...
        Err(anyhow::anyhow!(
            "Could not derive P2P BSSID from network interface"
        ))
    }
...
impl super::Protocol for WiFiDirectMeshProtocol {
        ...
        let peer_address = match target {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "WiFiDirect only supports IP socket addresses"
                ))
            }
        };
    ...
    async fn receive_message(
        ...
        Err(anyhow::anyhow!(
            "Receive message not fully implemented for WiFiDirect"
        ))
    }
