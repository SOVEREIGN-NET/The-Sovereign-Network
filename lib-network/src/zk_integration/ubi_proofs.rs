...
pub async fn generate_ubi_proof(recipient: &PublicKey, amount: u64, round: u64) -> Result<Vec<u8>> {
    info!(
    ...
    match lib_proofs::backend::get_backend().prove_identity(
        ...
        Ok(backend_proof) => {
            ...
            info!(
                "UBI proof generated successfully ({} bytes)",
                serialized.len()
            );
            Ok(serialized)
        }
...
pub async fn verify_ubi_proof(proof: &[u8]) -> Result<bool> {
    ...
    // Parse the ZK proof
    let zk_proof: lib_proofs::ZeroKnowledgeProof =
        serde_json::from_slice(proof).map_err(|e| anyhow!("Failed to parse UBI proof: {}", e))?;
...
async fn verify_ubi_economic_constraints(amount: u64, round: u64) -> Result<bool> {
    info!(
        "Verifying UBI economic constraints for amount: {}, round: {}",
        amount, round
    );
    ...
    if round > estimated_current_round + MAX_ROUNDS_AHEAD {
        warn!(
            "UBI round {} is too far in the future (estimated current: {})",
            round, estimated_current_round
        );
        return Ok(false);
    }
...
pub async fn generate_batch_ubi_proofs(
    ...
    info!(
        "🎁 Generating batch UBI proofs for {} recipients",
        recipients.len()
    );
    ...
    for (i, (recipient, &amount)) in recipients.iter().zip(amounts.iter()).enumerate() {
        ...
        match generate_ubi_proof(recipient, amount, round).await {
            ...
            Err(e) => {
                ...
                return Err(anyhow!(
                    "Batch UBI proof generation failed at recipient {}: {}",
                    i,
                    e
                ));
            }
...
pub async fn verify_batch_ubi_proofs(proofs: &[Vec<u8>]) -> Result<Vec<bool>> {
    ...
    info!(
        "Batch UBI verification complete: {}/{} proofs valid",
        valid_count,
        proofs.len()
    );
...
pub async fn generate_ubi_eligibility_proof(identity: &PublicKey) -> Result<Vec<u8>> {
...
pub async fn verify_ubi_eligibility_proof(proof: &[u8], identity: &PublicKey) -> Result<bool> {
    ...
    let proof_str =
        String::from_utf8(proof.to_vec()).map_err(|_| anyhow!("Invalid proof data format"))?;
