...
mod tests {
    ...
    async fn test_generate_identity_proof() -> Result<()> {
        ...
        assert!(
            ...
            "Identity proof should not be empty"
        );
        assert!(
            proof_bytes.len() > 100,
            "Identity proof should be substantial (>100 bytes)"
        );
        ...
        assert!(
            proof_str.contains("public_inputs"),
            "Should contain public_inputs field"
        );
        assert!(
            proof_str.contains("verification_key_hash"),
            "Should contain verification_key_hash field"
        );

        println!(
            "Identity proof generation test passed: {} bytes",
            proof_bytes.len()
        );
        Ok(())
    }
    ...
    async fn test_identity_proof_with_custom_parameters() -> Result<()> {
        ...
        assert!(
            is_valid,
            "Identity proof should be valid with matching parameters"
        );
    ...
    async fn test_identity_proof_parameter_mismatch() -> Result<()> {
        ...
        let is_valid =
            verify_identity_proof_with_params(&proof_bytes, &verification_params).await?;
        assert!(
            !is_valid,
            "Identity proof should be invalid with mismatched parameters"
        );
    ...
    async fn test_identity_proof_invalid_data() -> Result<()> {
        ...
        assert!(
            wrong_result.is_err(),
            "Wrong structure should fail verification"
        );
    ...
    async fn test_identity_proof_mesh_participation() -> Result<()> {
        ...
        assert!(
            is_valid,
            "Identity proof should be valid for mesh participation"
        );

        // Verify proof structure contains expected fields for mesh
        let proof_str = String::from_utf8(proof_bytes)?;
        assert!(
            proof_str.contains("ZHTP-Optimized-Identity"),
            "Should use ZHTP identity proof system"
        );
    ...
    async fn test_multiple_identity_proofs() -> Result<()> {
        ...
        for i in 0..proof_bytes_list.len() {
            for j in (i + 1)..proof_bytes_list.len() {
                assert_ne!(
                    proof_bytes_list[i], proof_bytes_list[j],
                    "Identity proofs {} and {} should be different",
                    i, j
                );
            }
        }
    ...
    async fn test_identity_proof_jurisdiction_requirements() -> Result<()> {
        ...
        let is_valid_2 =
            verify_identity_proof_with_params(&proof_bytes_2, &no_jurisdiction_params).await?;
        assert!(
            is_valid_2,
            "Identity proof with no jurisdiction requirement should be valid"
        );
