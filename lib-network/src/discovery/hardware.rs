...
impl HardwareCapabilities {
    ...
    pub async fn detect() -> Result<Self> {
        ...
        capabilities.bluetooth_available =
            detect_bluetooth_hardware(&mut capabilities.hardware_details).await;

        // Detect WiFi Direct hardware
        capabilities.wifi_direct_available =
            detect_wifi_direct_hardware(&mut capabilities.hardware_details).await;
    ...
    pub fn lorawan_radios_for_band(
        &self,
        band: &super::lorawan_hardware::FrequencyBand,
    ) -> Vec<&LoRaWANHardware> {
        self.lorawan_radios
            .iter()
                    ...
                    let device = HardwareDevice {
                        ...
                        properties: {
                            ...
                            props.insert(
                                ...
                                radio.connection_type.clone(),
                            );
                            props
                                .insert("max_tx_power".to_string(), radio.max_tx_power.to_string());
                            props.insert(
                                "class_a".to_string(),
                                radio.capabilities.class_a.to_string(),
                            );
                            props.insert(
                                "class_b".to_string(),
                                radio.capabilities.class_b.to_string(),
                            );
                            props.insert(
                                "class_c".to_string(),
                                radio.capabilities.class_c.to_string(),
                            );
                            props.insert(
                                "otaa_support".to_string(),
                                radio.capabilities.otaa_support.to_string(),
                            );
                            props.insert(
                                "max_payload_size".to_string(),
                                radio.capabilities.max_payload_size.to_string(),
                            );
                            props
                        },
                    };
...
async fn detect_wifi_direct_hardware(
    hardware_details: &mut HashMap<String, HardwareDevice>,
) -> bool {
    #[cfg(target_os = "linux")]
    {
        use std::process::Command;
...
mod tests {
    ...
    fn test_lorawan_radio_filtering() {
        use crate::discovery::lorawan_hardware::{
            FrequencyBand, LoRaWANCapabilities, LoRaWANHardware,
        };
