...
pub async fn discover_lorawan_gateways_with_capabilities(
    capabilities: &HardwareCapabilities,
) -> Result<Vec<LoRaWANGatewayInfo>> {
...
async fn check_lorawan_hardware() -> bool {
    ...
    if let Ok(result) = output {
        ...
        if usb_devices.contains("1a86:7523") || // CH340 (common in LoRaWAN modules)
           ...
           usb_devices.contains("10c4:ea60")
        {
            // Silicon Labs (CP210x)
    ...
    let output = Command::new("i2cdetect").args(&["-y", "1"]).output();
...
async fn perform_lorawan_scan(frequency_hz: u32) -> Result<LoRaWANGatewayInfo> {
    ...
    let output = Command::new("which").arg("lora_pkt_fwd").output();
    ...
    let output = Command::new("which").arg("rtl_sdr").output();
...
async fn simulate_gateway_detection(frequency_hz: u32) -> Result<LoRaWANGatewayInfo> {
    ...
    if rand::random::<f32>() > 0.15 {
        return Err(anyhow!(
            "No LoRaWAN gateway detected on frequency {}",
            frequency_hz
        ));
    }
