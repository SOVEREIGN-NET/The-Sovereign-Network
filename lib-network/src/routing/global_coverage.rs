...
impl GlobalCoverageAnalyzer {
    ...
    async fn calculate_continental_coverage(&mut self) -> Result<()> {
        ...
        for (continent, population, area_km2) in continental_data {
            let coverage = self
                .calculate_continent_coverage(continent, population, area_km2)
                .await?;
            self.coverage_map
                .continental_coverage
                .insert(continent.to_string(), coverage);
        }
    ...
    async fn calculate_continent_coverage(
        ...
        for (relay_id, relay) in &self.relays {
            let relay_coverage_area =
                std::f64::consts::PI * relay.coverage_radius_km * relay.coverage_radius_km;
            ...
            match relay.relay_type {
                LongRangeRelayType::Satellite => {
                    ...
                    continent_relays += 1;
                    info!(
                        "🛰️ Satellite {} contributes {:.0} km² to {} coverage",
                        relay_id,
                        relay_coverage_area * 0.8,
                        continent
                    );
                }
                LongRangeRelayType::LoRaWAN => {
                    ...
                    if rand::random::<f32>() < 0.3 {
                        ...
                        continent_relays += 1;
                        info!(
                            "LoRaWAN {} provides {:.0} km² coverage in {}",
                ...
                LongRangeRelayType::WiFiRelay => {
                    ...
                    continent_relays += 1;
                    info!(
                        "Internet bridge {} provides {:.0} km² global access for {}",
                        relay_id,
                        relay_coverage_area * 0.5,
                        continent
                    );
                }
                _ => {
    ...
    async fn calculate_global_metrics(&mut self) -> Result<()> {
        ...
        if satellite_count > 0 {
            ...
            info!(
                "🛰️ Satellite bonus coverage: +{:.0} km² and +{} people",
        ...
        self.coverage_map.population_served = total_population_served.min(total_world_population);
    ...
    async fn log_coverage_summary(&self) {
        ...
        info!(
            "   Global coverage: {:.1}% of Earth's land area",
            self.coverage_map.global_coverage_percentage
        );
        info!(
            "   Population served: {} people ({:.1}% of global population)",
        ...
        let satellite_relays = self
            .relays
            .values()
            .filter(|relay| matches!(relay.relay_type, LongRangeRelayType::Satellite))
            .count();

        let internet_bridges = self
            .relays
            .values()
