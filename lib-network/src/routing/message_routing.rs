...
impl MeshMessageRouter {
    ...
    #[deprecated(
        since = "0.1.0",
        note = "PeerRegistry handles indexing automatically. This is a no-op."
    )]
    ...
    fn peer_entry_to_mesh_connection(
        entry: &crate::peer_registry::PeerEntry,
    ) -> Option<crate::mesh::connection::MeshConnection> {
    ...
    ///
    /// Uses PeerRegistry's built-in index for O(1) constant-time lookup, preventing timing side-channel attacks.
    pub async fn find_connection_by_public_key(
        &self,
        public_key: &PublicKey,
    ) -> Option<MeshConnection> {
        // Ticket #149: Use PeerRegistry's built-in index
        let registry = self.peer_registry.read().await;
    ...
    pub fn set_mesh_server(
        &mut self,
        mesh_server: Arc<RwLock<crate::mesh::server::ZhtpMeshServer>>,
    ) {
        ...
        match message {
            ...
            ZhtpMeshMessage::IdentityEnvelope(envelope) => {
                envelope
                    .payloads
                    .iter()
                    .map(|p| p.ciphertext.len() + p.device_id.len())
                    .sum::<usize>()
                    + 128
            }
    ...
    pub async fn route_message(
        ...
        info!(
            " Routing message {} to destination {:?}",
            message_id,
        ...
        {
            let mut tracking: tokio::sync::RwLockWriteGuard<HashMap<u64, DeliveryStatus>> =
                self.delivery_tracking.write().await;
    ...
    pub async fn find_optimal_route(
        ...
        if let Some(cached_route) = self.get_cached_route(destination).await {
            info!(
                "Using cached route to destination (quality: {:.2})",
        ...
        while !unvisited.is_empty() {
            ...
            if let Some(current_peer) = current {
                if current_peer == *destination {
                    ...
                    return self
                        .construct_route_from_path(&previous, destination, &registry)
                        .await;
                }
                ...
                // Check neighbors
                if let Some(neighbors) = routing_table
                    .topology_map
                    .peer_connections
                    .get(&current_peer)
                {
                    for neighbor in neighbors {
                        if unvisited.contains(neighbor) {
                            let edge_weight = self
                                .calculate_edge_weight(&current_peer, neighbor, &registry)
                                .await;
                            let alt_distance =
                                distances.get(&current_peer).unwrap_or(&f64::INFINITY)
                                    + edge_weight;
    ...
    async fn execute_routing(
        ...
        info!(
            " Executing routing for message {} ({} hops)",
            message_id,
            route.len()
        );
        ...
        for (hop_index, hop) in route.iter().enumerate() {
            info!(
                " Routing to hop {}: {:?} via {:?}",
                hop_index + 1,
                hex::encode(&hop.peer_id.public_key().key_id[0..4]),
                hop.protocol
            );
        ...
        if let Some(mesh_server) = &self.mesh_server {
            ...
            if let Err(e) = mesh_server
                .read()
                .await
                .record_routing_activity(message_size, hop_count, primary_protocol, avg_latency)
                .await
            {
                warn!("Failed to record routing activity: {}", e);
            } else {
                info!(
                    " Routing rewards recorded: {} bytes, {} hops, avg {}ms latency",
            ...
            match update {
                ...
                TopologyUpdate::PeerDisconnection { peer_a, peer_b } => {
                    if let Some(connections) =
                        routing_table.topology_map.peer_connections.get_mut(&peer_a)
                    {
                        connections.remove(&peer_b);
                    }
                    if let Some(connections) =
                        routing_table.topology_map.peer_connections.get_mut(&peer_b)
                    {
    ...
    pub async fn find_next_hop_for_destination(
        ...
        debug!(
            "Finding next hop for destination {:?}",
            hex::encode(&destination.public_key().key_id[0..4])
        );
        ...
        self.cache_route(destination.clone(), full_route.clone(), quality_score)
            .await;
...
pub enum TopologyUpdate {
    PeerConnection {
        peer_a: UnifiedPeerId,
        peer_b: UnifiedPeerId,
    },
    PeerDisconnection {
        peer_a: UnifiedPeerId,
        peer_b: UnifiedPeerId,
...
mod tests {
    ...
    async fn test_route_caching() {
        ...
        router
            .cache_route(destination.clone(), route.clone(), 0.9)
            .await;
