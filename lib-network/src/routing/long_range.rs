...
impl LongRangeRoutingManager {
    ...
    async fn initialize_satellite_constellations(&self) -> Result<()> {
        ...
        for (name, count, altitude_km) in constellations {
            ...
            routing_table
                ...
                .insert(name.to_string(), constellation);
            info!(
                "🛰️ Added {} constellation: {} satellites, {:.0}km altitude",
        ...
        info!(
            "🛰️ Created {} satellite uplinks for global access",
            uplinks.len()
        );
        Ok(())
    }
    ...
    async fn generate_global_coverage_areas(&self, altitude_km: f64) -> Vec<SatelliteCoverageArea> {
        ...
        info!(
            "Generated {} global coverage areas (radius: {:.0}km each)",
            coverage_areas.len(),
            coverage_radius_km
        );
        coverage_areas
    ...
    async fn initialize_internet_bridges(&self) -> Result<()> {
        ...
        for (name, lat, lon, bandwidth_mbps) in ix_points {
            ...
            routing_table
                .internet_bridges
                .insert(bridge.bridge_id.clone(), bridge);
            info!("Added internet bridge: {} ({} Mbps)", name, bandwidth_mbps);
        }

        info!(
            "Initialized {} internet bridges for global connectivity",
        ...
        for (chain_id, ocean, relay_stations) in oceanic_chains {
            ...
            for (station_id, station_type, lat, lon) in relay_stations {
                ...
                if !stations.is_empty() {
                    ...
                    let distance =
                        self.calculate_distance(&prev_station.position, &station.position);
                    total_distance += distance;
                }

                stations.push(station);
                info!(
                    "🌊 Added oceanic relay: {} ({}) in {}",
                    station_id, station_type, ocean
                );
            }
            ...
            routing_table
                .oceanic_relays
                .insert(chain_id.to_string(), chain);
            info!(
                "🌊 Created oceanic relay chain: {} ({:.0}km total)",
                chain_id, total_distance
            );
        }

        info!(
            "Initialized {} oceanic relay chains for trans-oceanic routing",
    ...
    async fn build_intercontinental_routes(&self) -> Result<()> {
        ...
        for source in &continents {
            for destination in &continents {
                if source != destination {
                    let paths = self
                        .find_intercontinental_paths(source.clone(), destination.clone())
                        .await?;
                    routing_table
                        .continental_routes
                        .insert((source.clone(), destination.clone()), paths);

                    info!(
                        "🗺️ Created intercontinental routes: {:?} → {:?}",
                        source, destination
                    );
                }
            }
        ...
        info!(
            "Built intercontinental routing table for {} continent pairs",
    ...
    async fn initialize_backbone_routes(&self) -> Result<()> {
        ...
        for (route_id, source, destination, capacity_gbps) in backbone_configs {
            ...
            info!(
                " Added backbone route: {:?} → {:?} ({} Gbps)",
                source, destination, capacity_gbps
            );
        }

        info!(
            "Initialized {} backbone routes for high-capacity traffic",
            backbone_routes.len()
        );
        Ok(())
    }
    ...
    pub async fn find_global_route(
        ...
        if let Some(paths) = routing_table
            .continental_routes
            .get(&(source_continent, destination_continent))
        {
            // Filter paths based on quality requirements
            let suitable_paths: Vec<&IntercontinentalPath> = paths
                .iter()
            ...
            if let Some(best_path) = suitable_paths.first() {
                info!(
                    "Selected {} path with quality {:.2}",
                    format!("{:?}", best_path.path_type),
                    best_path.quality_score
                );

                // Convert global relay hops to route hops
                // **MIGRATION (Ticket #146):** RouteHop now uses UnifiedPeerId
                #[allow(deprecated)]
                let route_hops: Vec<RouteHop> = best_path
                    .relay_hops
                    .iter()
                    .map(|global_hop| {
    ...
    pub async fn get_global_routing_stats(&self) -> GlobalRoutingStats {
        ...
        let total_satellite_capacity: u32 = satellite_uplinks
            .iter()
            .map(|uplink| uplink.uplink_capacity_mbps)
            .sum();

        let total_backbone_capacity: u32 = backbone_routes
            .iter()
            .map(|route| route.capacity_gbps * 1000) // Convert to Mbps
            .sum();

        let total_internet_bridge_capacity: u32 = routing_table
            .internet_bridges
            .values()
    ...
    async fn assess_global_coverage_quality(&self) -> f64 {
        ...
        let satellite_score = if !routing_table.satellite_constellations.is_empty() {
            0.4
        } else {
            0.0
        };
        let internet_score = if !routing_table.internet_bridges.is_empty() {
            0.3
        } else {
            0.0
        };
        let oceanic_score = if !routing_table.oceanic_relays.is_empty() {
            0.2
        } else {
            0.0
        };
        let backbone_score = if !self.backbone_routes.read().await.is_empty() {
            0.1
        } else {
            0.0
        };
