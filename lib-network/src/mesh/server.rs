...
impl EmergencyHaltPolicy {
    ...
    pub fn validate_consensus_halt(
        ...
        if self.requires_consensus_quorum
            && !self.quorum_satisfied(agreeing_validators, total_validators)
        {
            return Err(format!(
...
use crate::dht::{DHTNetworkStatus, ZkDHTIntegration};
...
pub struct ZhtpMeshServer {
    ...
    pub wifi_direct_protocol:
        Option<Arc<RwLock<crate::protocols::wifi_direct::WiFiDirectMeshProtocol>>>,
...
impl MeshNode {
    ...
    pub async fn start_pure_mesh(&mut self) -> Result<()> {
        ...
        info!(
            "Mesh node initialized with {} configured protocols",
            self.protocols.len()
        );
...
impl ZhtpMeshServer {
    ...
    pub async fn get_permission_level(&self, wallet_key: &PublicKey) -> PermissionLevel {
        ...
        if admin_keys
            .iter()
            .any(|key| key.as_bytes() == wallet_key.as_bytes())
        {
    ...
    pub async fn add_admin_wallet(
        &self,
        caller_wallet_key: &PublicKey,
        admin_wallet_key: PublicKey,
    ) -> Result<()> {
        ...
        self.log_security_operation(
            ...
            Some(format!(
                "admin:{}",
                hex::encode(admin_wallet_key.as_bytes())
            )),
            "Owner".to_string(),
            true,
        ...
        match permission_level {
            PermissionLevel::Owner | PermissionLevel::Admin => {
                *self.emergency_stop.write().await = true;
                ...
                warn!(
                    " EMERGENCY STOP activated by wallet: {}",
                    hex::encode(caller_wallet_key.as_bytes())
                );
                Ok(())
            }
            _ => {
                ...
                Err(anyhow!(
                    "Only owner or admin wallets can trigger emergency stop"
                ))
            }
        }
    }
    ...
    async fn start_bluetooth_discovery(&mut self) -> Result<()> {
        ...
        if *self.emergency_stop.read().await {
            return Err(anyhow!(
                " Cannot start discovery - emergency stop is active"
            ));
        }
        ...
        if at_limit {
            warn!(
                " Connection limit reached ({}/{}), skipping Bluetooth discovery",
        ...
        self.active_protocols
            .write()
            .await
            .insert(NetworkProtocol::BluetoothLE, true);
    ...
    async fn start_wifi_direct_discovery(&mut self) -> Result<()> {
        ...
        if at_limit {
            warn!(
                " Connection limit reached ({}/{}), skipping WiFi Direct discovery",
                current_connections, max_connections
            );
            return Err(anyhow!("Connection limit reached"));
        }
        ...
        self.active_protocols
            .write()
            .await
            .insert(NetworkProtocol::WiFiDirect, true);
    ...
    async fn start_quic_discovery(&mut self) -> Result<()> {
        ...
        self.active_protocols
            .write()
            .await
            .insert(NetworkProtocol::QUIC, true);

        info!(
            "🚀 QUIC mesh protocol active with PQC encryption on port {}",
        ...
        if !bootstrap_peers.is_empty() {
            info!(
                "📡 Connecting to {} bootstrap peer(s) via QUIC...",
                bootstrap_peers.len()
            );
            ...
            for peer_str in &bootstrap_peers {
                ...
                let peer_addr = if let Some(colon_pos) = addr_str.rfind(':') {
                    ...
                    info!(
                        "   ℹ️  Peer has no port, using configured QUIC mesh port {}: {}",
                        self.config.mesh_port, peer_str
                    );
                    match format!("{}:{}", addr_str, self.config.mesh_port)
    ...
    async fn start_bluetooth_monitoring(
        &self,
        protocol: Arc<RwLock<crate::protocols::bluetooth::BluetoothMeshProtocol>>,
    ) -> Result<()> {
    ...
    async fn start_wifi_direct_monitoring(
        &self,
        _protocol: Arc<RwLock<crate::protocols::wifi_direct::WiFiDirectMeshProtocol>>,
    ) -> Result<()> {
        let _peer_registry = self.peer_registry.clone();
    ...
    pub async fn new(
        ...
        let mesh_node = Arc::new(RwLock::new(
            MeshNode::new_with_hardware_detection(network_config.clone()).await?,
        ));
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        // Display routing statistics
        {
            ...
            println!(
                " Theoretical Earnings: {} tokens (display only)",
    ...
    async fn initialize_long_range_relays(
        ...
        self.discover_lorawan_gateways_with_capabilities(hardware_caps)
            .await?;

        // Search for satellite uplink capabilities (GLOBAL coverage)
        self.discover_satellite_uplinks_with_capabilities(hardware_caps)
            .await?;

        // Find high-power WiFi relays (internet bridge points)
        self.discover_wifi_relays_with_capabilities(hardware_caps)
            .await?;
        ...
        let total_coverage_km: f64 = relays.values().map(|relay| relay.coverage_radius_km).sum();

        let has_satellite = relays
            .values()
            .any(|relay| matches!(relay.relay_type, LongRangeRelayType::Satellite));

        let has_internet_bridge = relays
            .values()
    ...
    async fn discover_lorawan_gateways_with_capabilities(
        &self,
        ...
        for gateway_info in discovered_gateways {
            ...
            println!(
                "LoRaWAN gateway discovered: {} - {} km range",
    ...
    async fn discover_satellite_uplinks_with_capabilities(
        ...
        for satellite_info in discovered_satellites {
            ...
            println!(
                "🛰️ Satellite uplink discovered: {} - GLOBAL coverage",
                uplink_id
            );
        }
    ...
    async fn discover_wifi_relays_with_capabilities(
        &self,
        capabilities: &HardwareCapabilities,
    ) -> Result<()> {
        ...
        for wifi_info in discovered_networks {
            ...
            println!(
                "WiFi relay network discovered: {} - {} Mbps",
    ...
    pub async fn get_network_stats(&self) -> MeshProtocolStats {
        // Return the currently tracked mesh protocol statistics.
        // Active connections should be maintained by the stats subsystem,
        // not inferred from the total number of peers in the registry.
    ...
    pub async fn verify_credentials(
        ...
        if now > credentials.timestamp + 300 {
            // 5 minute expiry
            warn!(
                " Credential verification failed: timestamp expired (age: {} seconds)",
                now - credentials.timestamp
            );
            return Ok(false);
        }
            ...
            .verify(message.as_bytes(), &signature)
        {
            Ok(is_valid) => {
                if is_valid {
                    info!(
                        " Credential verification successful for operation: {}",
                        operation
                    );
                } else {
                    warn!(
                        " Credential verification failed: invalid signature for operation: {}",
                        operation
                    );
                }
                Ok(is_valid)
            ...
            Err(e) => {
                error!(
                    " Credential verification error for operation {}: {}",
                    operation, e
                );
    ...
    */
                ...
                if let Some(ref caps) = hardware_caps {
                    match crate::discovery::wifi::discover_wifi_relays_with_capabilities(caps).await
                    {
                        Ok(discovered_networks) => {
                            info!(
                                "Discovered {} WiFi relay networks for P2P mesh",
                                discovered_networks.len()
                            );

                            for wifi_info in discovered_networks {
                                info!(
                                    "WiFi relay available: {} - {} Mbps capacity",
                                    wifi_info.ssid, wifi_info.bandwidth_estimate_mbps
                                );
                            }
                        }
    ...
    pub async fn serve_web4_content(&self, domain: &str, path: &str) -> Result<Vec<u8>> {
        ...
        let content_hash = self.dht.write().await.resolve_content(domain, path).await?;
        ...
        let content = format!(
            "<h1>Content from {}{}</h1><p>DHT integration pending</p>",
            domain, path
        );
...
impl ZhtpMeshServer {
    ...
    pub async fn record_retrieval_operation(&self) {
        ...
        info!(
            "📤 Retrieval served: total retrievals = {}",
            stats.retrievals_served
        );
    }
