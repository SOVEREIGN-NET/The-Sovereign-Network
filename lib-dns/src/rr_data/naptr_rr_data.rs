...
impl RRData for NaptrRRData {
            ...
            .split(",")
        {
            ...
            flags.push(
                NaptrFlags::try_from(
                    flag.chars()
                        .next()
                        .ok_or_else(|| RRDataError("empty NAPTR flag token".to_string()))?,
                )
                .map_err(|e| RRDataError(e.to_string()))?,
            );
        }
    ...
    fn to_bytes(&self) -> Result<Vec<u8>, RRDataError> {
        ...
        let service = self
            .service
            .as_ref()
            .ok_or_else(|| RRDataError("service param was not set".to_string()))?
            .as_bytes();
        ...
        let regex = self
            .regex
            .as_ref()
            .ok_or_else(|| RRDataError("regex param was not set".to_string()))?
            .as_bytes();
            ...
            .split(",")
        {
            ...
            flags.push(
                NaptrFlags::try_from(
                    flag.chars()
                        .next()
                        .ok_or_else(|| WireError::Format("empty NAPTR flag token".to_string()))?,
                )
                .map_err(|e| WireError::Format(e.to_string()))?,
            );
        }
...
impl ToWire for NaptrRRData {
    fn to_wire(&self, context: &mut ToWireContext) -> Result<(), WireError> {
        ...
        let service = self
            .service
            .as_ref()
            .ok_or_else(|| WireError::Format("service param was not set".to_string()))?
            .as_bytes();
        ...
        let regex = self
            .regex
            .as_ref()
            .ok_or_else(|| WireError::Format("regex param was not set".to_string()))?
            .as_bytes();
        ...
        context.write_name(
            self.replacement
                .as_ref()
                .ok_or_else(|| WireError::Format("replacement param was not set".to_string()))?,
            false,
        )
    }
}
...
fn test() {
    let buf = vec![
        0x0, 0x64, 0x0, 0xa, 0x3, 0x55, 0x2c, 0x50, 0x7, 0x45, 0x32, 0x55, 0x2b, 0x73, 0x69, 0x70,
        0x19, 0x21, 0x5e, 0x2e, 0x2a, 0x24, 0x21, 0x73, 0x69, 0x70, 0x3a, 0x69, 0x6e, 0x66, 0x6f,
        0x40, 0x66, 0x69, 0x6e, 0x64, 0x39, 0x2e, 0x6e, 0x65, 0x74, 0x21, 0x0,
    ];
    let record = NaptrRRData::from_bytes(&buf).ok();
