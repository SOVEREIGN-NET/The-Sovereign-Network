...
impl SvcParams {
        ...
        Ok(match key {
            SvcParamKeys::Mandatory => {
                ...
                for c in buf.chunks_exact(2) {
                    out.push(
                        SvcParamKeys::try_from(u16::from_be_bytes([c[0], c[1]]))
                            .map_err(|e| SvcParamParseError(e.to_string()))?,
                    );
                }
            ...
            SvcParamKeys::Ipv4Hint => Self::Ipv4Hint(
                buf.chunks_exact(4)
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        match self {
            ...
            Self::Ipv6Hint(_count, raw) => raw.clone(),
        }
    }
...
impl FromStr for SvcParams {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.split_once('=') {
            Some((key, value)) => match SvcParamKeys::from_str(key) {
                Ok(key) => match key {
                    SvcParamKeys::Mandatory => {
                        let mut out = Vec::new();
                        for k in value.trim_matches('"').split(',') {
                            out.push(
                                SvcParamKeys::from_str(k)
                                    .map_err(|e| SvcParamParseError(e.to_string()))?,
                            );
                        }
                    ...
                    SvcParamKeys::Port => Ok(SvcParams::Port(
                        value
                            .parse::<u16>()
                            .map_err(|e| SvcParamParseError(e.to_string()))?,
                    )),
                    ...
                    SvcParamKeys::Ech => Ok(SvcParams::Ech(
                        base64::decode(value).map_err(|e| SvcParamParseError(e.to_string()))?,
                    )),
