...
impl RRData for HttpsRRData {
    fn from_bytes(buf: &[u8]) -> Result<Self, RRDataError> {
        ...
        while i < buf.len() {
            ...
            params.push(
                SvcParams::from_bytes(key, &buf[i + 4..i + 4 + length])
                    .map_err(|e| RRDataError(e.to_string()))?,
            );
...
impl FromWireLen for HttpsRRData {
    fn from_wire_len(context: &mut FromWireContext, len: u16) -> Result<Self, WireError> {
        ...
        while i < len {
            ...
            params.push(
                SvcParams::from_bytes(key, context.take(length as usize)?)
                    .map_err(|e| WireError::Format(e.to_string()))?,
            );
...
fn test() {
    let buf = vec![
        0x0, 0x1, 0x3, 0x77, 0x77, 0x77, 0x5, 0x66, 0x69, 0x6e, 0x64, 0x39, 0x3, 0x6e, 0x65, 0x74,
        0x0, 0x0, 0x1, 0x0, 0x6, 0x2, 0x68, 0x33, 0x2, 0x68, 0x32, 0x0, 0x4, 0x0, 0x8, 0x68, 0x15,
        0x2a, 0x89, 0xac, 0x43, 0xce, 0x1c, 0x0, 0x5, 0x0, 0x47, 0x0, 0x45, 0xfe, 0xd, 0x0, 0x41,
        0xda, 0x0, 0x20, 0x0, 0x20, 0xad, 0xee, 0x8b, 0x18, 0xce, 0xda, 0xba, 0x2b, 0x15, 0xe4,
        0x6e, 0x16, 0x57, 0xc1, 0xf4, 0x91, 0x27, 0x41, 0xc0, 0xd8, 0xbf, 0x6, 0x22, 0x55, 0xa1,
        0xd6, 0x80, 0x27, 0x63, 0x7e, 0x4e, 0x10, 0x0, 0x4, 0x0, 0x1, 0x0, 0x1, 0x0, 0x12, 0x63,
        0x6c, 0x6f, 0x75, 0x64, 0x66, 0x6c, 0x61, 0x72, 0x65, 0x2d, 0x65, 0x63, 0x68, 0x2e, 0x63,
        0x6f, 0x6d, 0x0, 0x0, 0x0, 0x6, 0x0, 0x20, 0x26, 0x6, 0x47, 0x0, 0x30, 0x30, 0x0, 0x0, 0x0,
        0x0, 0x0, 0x0, 0x68, 0x15, 0x2a, 0x89, 0x26, 0x6, 0x47, 0x0, 0x30, 0x35, 0x0, 0x0, 0x0,
        0x0, 0x0, 0x0, 0xac, 0x43, 0xce, 0x1c,
    ];
    let record = HttpsRRData::from_bytes(&buf).ok();
