...
impl ZoneRRData for UriRRData {
    fn set_data(&mut self, index: usize, value: &str) -> Result<(), ZoneReaderError> {
        Ok(match index {
            0 => {
                self.priority = value.parse().map_err(|_| {
                    ZoneReaderError::new(
                        ErrorKind::Format,
                        "unable to parse priority param for record type URI",
                    )
                })?
            }
            1 => {
                self.weight = value.parse().map_err(|_| {
                    ZoneReaderError::new(
                        ErrorKind::Format,
                        "unable to parse weight param for record type URI",
                    )
                })?
            }
            ...
            _ => {
                return Err(ZoneReaderError::new(
                    ...
                    "extra record data found for record type URI",
                ))
            }
        })
    }
...
fn test() {
    let buf = vec![
        0x0, 0x1, 0x66, 0x69, 0x6e, 0x64, 0x39, 0x3a, 0x2f, 0x2f, 0x6e, 0x61, 0x6d, 0x65, 0x73,
        0x65, 0x72, 0x76, 0x65, 0x72,
    ];
    let record = UriRRData::from_bytes(&buf).ok();
