...
impl RRData for AaaaRRData {
    ...
    fn to_bytes(&self) -> Result<Vec<u8>, RRDataError> {
        ...
        buf.extend_from_slice(
            &self
                .address
                .ok_or_else(|| RRDataError("address param was not set".to_string()))?
                .octets(),
        );
...
impl ToWire for AaaaRRData {
    fn to_wire(&self, context: &mut ToWireContext) -> Result<(), WireError> {
        context.write(
            &self
                .address
                .ok_or_else(|| WireError::Format("address param was not set".to_string()))?
                .octets(),
        )
    }
}
...
impl ZoneRRData for AaaaRRData {
    fn set_data(&mut self, index: usize, value: &str) -> Result<(), ZoneReaderError> {
        Ok(match index {
            0 => {
                self.address = Some(value.parse().map_err(|_| {
                    ZoneReaderError::new(
                        ErrorKind::Format,
                        "unable to parse address param for record type AAAA",
                    )
                })?)
            }
            _ => {
                return Err(ZoneReaderError::new(
                    ...
                    "extra record data found for record type AAAA",
                ))
            }
        })
    }
