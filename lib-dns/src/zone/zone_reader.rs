...
impl ZoneReader {
    pub fn open<P: Into<PathBuf>>(
        ...
        origin: &str,
        class: RRClasses,
    ) -> Result<Self, ZoneReaderError> {
        let file = File::open(file_path.into())
            .map_err(|e| ZoneReaderError::new(ErrorKind::PathNotFound, &e.to_string()))?;
    ...
    pub fn read_record(
        &mut self,
    ) -> Result<Option<(String, RRTypes, u32, Box<dyn ZoneRRData>)>, ZoneReaderError> {
        ...
        loop {
            ...
            match self.reader.read_line(&mut line) {
                Ok(length) => {
                    ...
                    for part in line.as_bytes().split_inclusive(|&b| {
                        b == b' ' || b == b'\t' || b == b'\n' || b == b'(' || b == b')'
                    }) {
                        ...
                        match state {
                            ...
                            ParserState::Common => {
                                ...
                                if let Ok(c) = RRClasses::from_str(&word) {
                                    ...
                                    record = Some((
                                        ...
                                        _type,
                                        ttl,
                                        <dyn ZoneRRData>::new(_type, &self.class).ok_or_else(
                                            || {
                                                ZoneReaderError::new(
                                    ...
                                    ttl = word.parse().map_err(|_| {
                                        ZoneReaderError::new(
                                            ErrorKind::Parsing,
                                            &format!("unable to parse ttl {}", word),
                                        )
                                    })?;
                                }
                            }
                            ...
                            ParserState::Data => {
                                if part[0] == b'"' {
                                    if part[word_len - 1] == b'"' {
                                        ...
                                        quoted_buf = format!(
                                            "{}{}",
                                            String::from_utf8(part[1..word_len].to_vec()).map_err(
                                                |_| ZoneReaderError::new(
                                                    ErrorKind::Parsing,
                                                    "unable to parse string"
                                                )
                                            )?,
                                            part[word_len] as char
                                        );
                                    }
                                } else {
                                    record.as_mut().ok().3.set_data(
                            ...
                            ParserState::QString => {
                                if part[word_len - 1] == b'"' {
                                    quoted_buf.push_str(&format!(
                                        "{}",
                                        String::from_utf8(part[0..word_len - 1].to_vec()).map_err(
                                            |_| ZoneReaderError::new(
                                                ErrorKind::Parsing,
                                                "unable to parse string"
                                            )
