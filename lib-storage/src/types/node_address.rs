...
impl NodeAddress {
    ...
    pub fn new_wallet_identity(
        ...
        let wallet_address =
            Hash::from_bytes(&lib_crypto::hash_blake3(&wallet_public_key.as_bytes()));
    ...
    pub fn has_quantum_identity(&self) -> bool {
        match self {
            ...
            Self::HybridAddress(addrs) => addrs
                .iter()
                .any(|addr| matches!(addr, AddressType::Quantum(_))),
            _ => false,
        }
    }
    ...
    pub fn has_wallet_identity(&self) -> bool {
        match self {
            ...
            Self::HybridAddress(addrs) => addrs
                .iter()
                .any(|addr| matches!(addr, AddressType::Wallet(_))),
            _ => false,
        }
    }
    ...
    pub fn controls_node(&self) -> bool {
        if let Some(wallet) = self.get_wallet_identity() {
            wallet.node_public_key.is_some()
                && matches!(
                    wallet.wallet_type,
                    WalletType::NodeOperator | WalletType::Validator
                )
        } else {
            false
        }
            ...
            Self::QuantumIdentity(qid) => {
                if let Some(name) = &qid.display_name {
                    write!(
                        ...
                        "qid://{}@{}",
                        name,
                        hex::encode(qid.identity_hash.as_bytes())
                    )
                } else {
            ...
            Self::WalletIdentity(wid) => {
                if let Some(name) = &wid.wallet_name {
                    write!(
                        ...
                        "wallet://{}@{}",
                        name,
                        hex::encode(wid.wallet_address.as_bytes())
                    )
                } else {
...
impl WalletIdentity {
    ...
    pub fn can_govern(&self) -> bool {
        matches!(
            self.wallet_type,
            WalletType::Governance | WalletType::Validator
        )
    }
}
...
mod tests {
    ...
    fn test_quantum_identity() {
        ...
        let node_addr = NodeAddress::new_quantum_identity(public_key, Some("alice".to_string()));
