...
impl DhtPeerManager {
    ...
    pub fn update_peer_stats(
        ...
        if let Some(stats) = self.peer_stats.get_mut(peer_id) {
            ...
            if total_requests > 0 {
                stats.avg_response_time = (stats.avg_response_time * (total_requests - 1) as f64
                    + response_time)
        ...
        let mut peer_scores: Vec<_> = self
            ...
            .map(|info| {
                ...
                let latency_score = stats
                    .map(|s| 1.0 / (s.avg_response_time + 1.0))
                    .unwrap_or(0.0);
    ...
    pub async fn cleanup_disconnected(&mut self, timeout: Duration) -> Result<usize> {
        ...
        let disconnected_peers: Vec<NodeId> = self
            .peers
            .iter()
            .filter_map(|(id, info)| {
                if matches!(
                    info.status,
                    crate::types::dht_types::PeerStatus::Disconnected
    ...
    pub fn find_closest_peers(&self, target: &NodeId, count: usize) -> Vec<&DhtNode> {
        self.router
            .find_closest_nodes(target, count)
            .iter()
    ...
    pub fn get_management_stats(&self) -> PeerManagementStats {
        ...
        let avg_reputation = if total_peers > 0 {
            let total_reputation: u32 = self.peers.values().map(|info| info.node.reputation).sum();
