...
impl DhtReplication {
    ...
    pub fn get_policy(&self, data_type: &str) -> ReplicationPolicy {
        self.policies
            .get(data_type)
            .cloned()
            .unwrap_or_else(|| ReplicationPolicy {
    ...
    pub async fn replicate_data(
        ...
        if successful_replicas.len() < policy.repair_threshold {
            return Err(anyhow!(
                "Replication failed: only {} out of {} replicas created",
    ...
    async fn replicate_to_node(
        ...
        if target_node.reputation < 500 {
            return Err(anyhow!(
                "Target node reputation too low: {}",
                target_node.reputation
            ));
        }
        ...
        let _message = crate::types::dht_types::DhtMessage {
            message_id: hex::encode(
                &blake3::hash(
                    &[key.as_bytes(), value, target_node.peer.node_id().as_bytes()].concat(),
                )
                .as_bytes()[..8],
            ),
        ...
        if rand::random::<f64>() < success_rate {
            ...
            Err(anyhow!(
                "Network error during replication to node with reputation {}",
                target_node.reputation
            ))
        }
    }
    ...
    pub async fn repair_replicas(&mut self, available_nodes: &[DhtNode]) -> Result<RepairStats> {
        ...
        let keys_needing_repair: Vec<String> = self
            .replication_status
            .iter()
    ...
    async fn repair_single_key(&mut self, key: &str, available_nodes: &[DhtNode]) -> Result<()> {
        ...
        let policy = self.get_policy("default");
        let needed_replicas = policy
            .replication_factor
            .saturating_sub(status.total_replicas);
        ...
        let candidate_nodes: Vec<&DhtNode> = available_nodes
            .iter()
            .filter(|node| {
                !status.replica_nodes.contains(node.peer.node_id())
                    && *node.peer.node_id() != self.local_id
            })
    ...
    async fn retrieve_data_for_repair(&self, key: &str) -> Result<Vec<u8>> {
        ...
        if let Some(status) = self.replication_status.get(key) {
            if !status.replica_nodes.is_empty() {
                ...
                return Ok(format!(
                    "repaired_data_for_{}_hash_{}",
                    key,
                    hex::encode(&key_hash.as_bytes()[..8])
                )
                .into_bytes());
            }
        }
...
mod tests {
    ...
    fn test_keys_needing_repair() {
        ...
        replication
            .replication_status
            .insert("test_key".to_string(), status);
