...
async fn test_complete_citizen_onboarding_flow() {
    ...
    let citizenship_result = manager
    ...
    assert_eq!(
        citizenship_result.ubi_registration.monthly_amount,
        SOV_UBI_MONTHLY_SOV * SOV_ATOMIC_UNITS
    );
    assert!(citizenship_result.ubi_registration.daily_amount > 0);

    // Verify Web4 access
    assert_eq!(
        citizenship_result.web4_access.access_level,
        AccessLevel::FullCitizen
    );
    assert!(citizenship_result.web4_access.service_tokens.len() >= 10);
    assert!(citizenship_result
        .web4_access
        .service_tokens
        .contains_key("zhtp.browse"));
    assert!(citizenship_result
        .web4_access
        .service_tokens
        .contains_key("zhtp.wallet"));
    assert!(citizenship_result
        .web4_access
        .service_tokens
        .contains_key("zhtp.voting"));
...
async fn test_credential_management_lifecycle() {
    ...
    assert_eq!(
        age_result.credential.credential_type,
        CredentialType::AgeVerification
    );
    ...
    assert_eq!(
        reputation_result.credential.credential_type,
        CredentialType::Reputation
    );

    // Test custom credential
    let custom_result = factory
    ...
    assert_eq!(
        custom_result.credential.credential_type,
        CredentialType::Education
    );
    ...
    assert_eq!(
        stats.by_type.get(&CredentialType::AgeVerification),
        Some(&1)
    );
...
async fn test_wallet_integration() {
    ...
    let (primary_wallet, _seed1) = wallet_manager
...
async fn test_activity_tracking() {
    ...
    assert!(record
        .activity_types
        .contains(&ActivityType::IdentityCreation));
    ...
    tracker.add_activity_to_session(
        &identity_id,
        &session_id,
        ActivityType::VerificationRequested,
    );
...
async fn test_citizenship_system_integration() {
    ...
    for result in &citizen_results {
        ...
        assert_eq!(
            result.ubi_registration.monthly_amount,
            SOV_UBI_MONTHLY_SOV * SOV_ATOMIC_UNITS
        );
        assert_eq!(
...
async fn test_performance_benchmarks() {
    ...
    assert!(
        credential_duration.as_millis() < 1000,
        "Credential creation too slow"
    );
    assert!(
