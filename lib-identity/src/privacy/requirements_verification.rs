...
use crate::privacy::{verify_age_requirement, verify_location_requirement, PrivacyCredentials};
...
pub fn verify_requirements(
    ...
    let verification_proofs =
        generate_verification_proofs(identity, requirements, &failed_requirements);
...
fn calculate_privacy_score(
    ...
    // Age privacy score
    let age_privacy =
        if requirements.min_age.is_some() && privacy_credentials.age_verification.is_some() {
    ...
    let location_privacy = if requirements.required_location.is_some()
        && privacy_credentials.location_proof.is_some()
    {
    ...
    let credential_privacy = if requirements.required_credentials.is_empty() {
        ...
        let avg_privacy: u32 = requirements
            .required_credentials
            .iter()
            .map(|cred| calculate_credential_privacy_score(cred.as_str()))
            .sum::<u32>()
            / requirements.required_credentials.len() as u32;
        avg_privacy
    };
...
pub fn verify_privacy_requirements(
    ...
    if privacy_score.overall_score < min_privacy_threshold {
        return Err(format!(
            ...
            privacy_score.overall_score, min_privacy_threshold
        ));
    }
