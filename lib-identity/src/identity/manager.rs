...
impl IdentityManager {
    ...
    /// Add an identity together with its private data (used by runtime/bootstrap)
    ...
    /// Get private data for an identity if stored
    pub fn get_private_data(&self, identity_id: &IdentityId) -> Option<&PrivateIdentityData> {
        self.private_data.get(identity_id)
    }

    /// Store wallet balance snapshots gathered from the blockchain (dev helper)
    pub fn sync_wallet_balances(&mut self, balances: &HashMap<String, u64>) -> Result<()> {
        // Keep a lightweight summary so callers know the sync happened
        let total_wallets = balances.len();
        let _total_balance: u64 = balances.values().copied().sum();
        if total_wallets == 0 {
            return Ok(());
        }
        // For now we just acknowledge the sync; detailed accounting will live in wallet manager
        Ok(())
    }

    /// Prepare payment transaction context (used by runtime marketplace flows)
    pub fn create_payment_transaction(
        &self,
        identity_id: &IdentityId,
        utxos: Vec<(lib_crypto::Hash, u32, u64)>,
        _recipient_pubkey: &[u8],
        amount: u64,
        fee: u64,
    ) -> Result<(Vec<u8>, u64, u64, Vec<u8>)> {
        let private_data = self.private_data.get(identity_id)
            .ok_or_else(|| anyhow!("Private data not found for identity"))?;

        if utxos.is_empty() {
            return Err(anyhow!("No UTXOs supplied for payment transaction"));
        }

        let total_input: u64 = utxos.iter().map(|(_, _, amount)| *amount).sum();
        let required = amount.checked_add(fee).ok_or_else(|| anyhow!("Payment amount overflow"))?;

        if total_input < required {
            return Err(anyhow!(
                "Insufficient UTXO value: required {}, provided {}",
                required,
                total_input
            ));
        }

        let change_amount = total_input - required;

        Ok((
            private_data.quantum_keypair.private_key.clone(),
            total_input,
            change_amount,
            private_data.quantum_keypair.public_key.clone(),
        ))
    }
    ...
    async fn verify_credential_proof(&self, credential: &ZkCredential) -> Result<bool> {
        ...
        if proof_data.is_empty()
            ...
            || verification_key.as_ref().map_or(true, |vk| vk.is_empty())
        {
            return Ok(false);
        }
        ...
        let verification_match = verification_key
            .as_ref()
            .map(|vk| vk == &verification_check.to_vec())
            .unwrap_or(false);
    ...
    async fn generate_ownership_proof(&self, private_key: &[u8], public_key: &[u8]) -> Result<ZeroKnowledgeProof> {
        ...
        Ok(ZeroKnowledgeProof::new(
            ...
            None,
            Some(verification_commitment.to_vec()),
            public_key.to_vec(),
            proof_response.to_vec(),
        ))
    }
}
