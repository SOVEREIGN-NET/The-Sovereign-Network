...
fn display_identity_details(identity: &ZhtpIdentity, label: &str) -> Result<()> {
    ...
    println!(
        "      Wallets:              {}",
        identity.wallet_manager.wallets.len()
    );
    println!(
        "      Total Balance:        {} SOV",
        identity.wallet_manager.total_balance
    );
...
fn verify_determinism(id1: &ZhtpIdentity, id2: &ZhtpIdentity) -> Result<()> {
    ...
    println!(
        "   ✓ DAO Member ID Match:    {}",
        if dao_id_match { "✅ PASS" } else { "❌ FAIL" }
    );
