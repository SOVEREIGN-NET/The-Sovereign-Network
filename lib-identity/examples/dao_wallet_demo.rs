...
async fn main() -> Result<()> {
    ...
    println!(
        ...
        if let Some(ref owner) = forprofit_wallet.owner_id {
            format!(
                "DID: {} (Correct for ForProfit)",
                hex::encode(&owner.0[..8])
            )
        } else {
    ...
    for (i, tx) in nonprofit_txs.iter().enumerate() {
        ...
        println!(
            "     Authorized by: {}",
            hex::encode(&tx.authorized_by.0[..8])
        );
    ...
    for (i, tx) in forprofit_txs.iter().enumerate() {
        ...
        println!(
            "     Counterparty: {}",
            if tx.counterparty_wallet.is_some() {
                "Disclosed"
            } else {
                "Private (Partial Transparency)"
            }
        );
        println!("     Time: {}", tx.timestamp);
    }
    ...
    for (i, wallet) in wallets.iter().enumerate() {
        ...
        println!(
            "  Is DAO: {}",
            if wallet.is_dao_wallet { "Yes" } else { "No" }
        );
        ...
        println!(
            "  Standalone: {}",
            if wallet.is_standalone { "Yes" } else { "No" }
        );
    }
    ...
    println!(
        "NonProfit DAOs: {}",
        manager.get_dao_wallets_by_type(true).len()
    );
    println!(
        "ForProfit DAOs: {}",
