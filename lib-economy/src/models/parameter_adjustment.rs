...
pub fn adjust_rates_for_utilization(
    ...
    if adjustment_multiplier == 100 {
        ...
        return Ok((
            current_routing_rate,
            current_storage_rate,
            current_compute_rate,
        ));
    }
...
pub fn calculate_optimal_rates(
    ...
    let (mut routing_rate, mut storage_rate, mut compute_rate) = adjust_rates_for_utilization(
        base_routing_rate,
        base_storage_rate,
        base_compute_rate,
        network_stats,
    ...
    if health_score < 0.5 {
        ...
        info!(
            "🏥 Poor network health ({:.2}), increasing incentives by 10%",
            health_score
        );
        ...
        info!(
            "✨ Excellent network health ({:.2}), optimizing incentives",
            health_score
        );
    }
