...
use crate::network_types::{
    get_active_peer_count, get_discovery_statistics, get_mesh_status, get_network_statistics,
    DiscoveryStatistics, MeshStatus,
};
...
impl MeshDiscoveryRewardManager {
    ...
    pub async fn record_discovery_work(&mut self, work: MeshDiscoveryWork) -> Result<()> {
        ...
        self.validate_discovery_work(&work, &discovery_stats, &mesh_status)
            .await?;
        self.validate_discovery_work_against_network_stats(&work, &network_stats)
            .await?;
        ...
        let validated_quality = self
            .calculate_validated_discovery_quality(&work, &discovery_stats)
            .await?;
        ...
        self.update_topology_contribution(&mesh_status, &discovery_stats)
            .await?;
        ...
        self.update_reliability_metrics(&work, &discovery_stats)
            .await?;
    ...
    pub async fn calculate_discovery_rewards(
        &mut self,
        ...
        let discovery_effectiveness = if discovery_stats.successful_connections > 0 {
            discovery_stats.successful_connections as f64
                / (discovery_stats.successful_connections + discovery_stats.failed_connections)
                    as f64
        } else {
        ...
        let topology_bonus = self
            .calculate_topology_improvement_bonus(&mesh_status)
            .await?;
        ...
        let network_health_bonus = self
            .calculate_network_health_bonus(&mesh_status, peer_count as usize)
            .await?;
        ...
        let total_base_reward = base_discovery_reward
            + topology_bonus
            + diversity_bonus
            + network_health_bonus
            + reliability_bonus;
        let economically_adjusted_reward =
            (total_base_reward as f64 * economic_multiplier * discovery_effectiveness) as u64;
        let demand_adjusted_reward =
            (economically_adjusted_reward as f64 * (1.0 + network_demand_factor * 0.1)) as u64; // Network demand bonus
        ...
        self.record_discovery_performance(&comprehensive_reward, &mesh_status)
            .await?;
    ...
    async fn validate_discovery_work(
        ...
        if work.peers_discovered > discovery_stats.total_peers_discovered_per_hour {
            return Err(anyhow::anyhow!(
                "Reported peer discoveries exceed network capability"
            ));
        }
        ...
        if work.discovery_quality > discovery_stats.average_discovery_success_rate + 0.1 {
            return Err(anyhow::anyhow!(
                "Reported discovery quality exceeds network measurements"
            ));
        }
        ...
        if work.topology_improvements > max_topology_improvements {
            return Err(anyhow::anyhow!(
                "Reported topology improvements exceed network capacity"
            ));
        }
    ...
    async fn validate_discovery_work_against_network_stats(
        ...
        if work.peers_discovered > max_discoverable_peers {
            return Err(anyhow::anyhow!(
                "Peer discoveries exceed network size limitations"
            ));
        }
        ...
        if work.discovery_quality > 1.0 || work.discovery_quality < 0.0 {
            return Err(anyhow::anyhow!(
                "Discovery quality out of valid range: {:.3}",
        ...
        if high_congestion && work.discovery_requests_handled > 1000 {
            return Err(anyhow::anyhow!(
                "High discovery workload during network congestion is suspicious"
            ));
        }
        ...
        if u64::from(work.routing_updates) > max_routing_updates {
            return Err(anyhow::anyhow!(
                "Routing updates exceed reasonable proportion of network activity"
            ));
        }
    ...
    async fn calculate_validated_discovery_quality(
        ...
        let validated_quality = (network_quality * 0.7 + reported_quality * 0.3)
            .min(1.0)
            .max(0.0);
    ...
    async fn update_topology_contribution(
        ...
        let discovery_contribution =
            (discovery_stats.average_discovery_success_rate * 0.5).min(0.5);

        let new_contribution =
            stability_contribution + redundancy_contribution + discovery_contribution;
        self.topology_contribution_score =
            (self.topology_contribution_score * 0.8 + new_contribution * 0.2).min(1.0);
    ...
    async fn update_geographic_coverage(
        ...
        self.geographic_coverage.coverage_diversity_score =
            discovery_stats.geographic_diversity_index;
        ...
        self.geographic_coverage.rural_connectivity_score =
            discovery_stats.rural_connectivity_index;
    ...
    async fn update_reliability_metrics(
        ...
        self.reliability_metrics.discovery_success_rate =
            (self.reliability_metrics.discovery_success_rate * 0.8 + work.discovery_quality * 0.2)
                .min(1.0);
        self.reliability_metrics.average_response_time_ms =
            (self.reliability_metrics.average_response_time_ms * 0.8
                + discovery_stats.average_response_time_ms * 0.2)
                .max(0.0);

        // Calculate uptime percentage
        let total_hours = 24; // Assume 24-hour periods
        let uptime_percentage =
            (work.discovery_uptime_hours as f64 / total_hours as f64 * 100.0).min(100.0);
        self.reliability_metrics.uptime_percentage =
            (self.reliability_metrics.uptime_percentage * 0.8 + uptime_percentage * 0.2).min(100.0);

        // Consistency score based on stable discovery patterns
        let consistency = if discovery_stats.discovery_variance < 0.2 {
            0.9
        } else {
            0.7
        };
        self.reliability_metrics.consistency_score =
            (self.reliability_metrics.consistency_score * 0.8 + consistency * 0.2).min(1.0);
    ...
    async fn calculate_network_health_bonus(
        &self,
        mesh_status: &MeshStatus,
        peer_count: usize,
    ) -> Result<u64> {
    ...
    fn update_discovery_work_metrics(&mut self, work: &MeshDiscoveryWork) {
        ...
        let _discovery_operations = work.peers_discovered as u64
            + work.discovery_requests_handled
            + work.routing_updates as u64;
...
pub async fn calculate_mesh_discovery_rewards(
    ...
    manager
        .calculate_discovery_rewards(economic_model, network_stats)
        .await
}
