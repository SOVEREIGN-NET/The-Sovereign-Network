...
use lib_economy::incentives::{
    cost_savings::CostSavings, infrastructure_rewards::InfrastructureRewards,
};
...
mod edge_case_tests {
    ...
    fn test_fee_calculation_edge_cases() {
        ...
        let (max_net_fee, max_dao_fee, max_total_fee) =
            model.calculate_fee(max_size, 1000, Priority::Normal);
    ...
    fn test_priority_boundary_conditions() {
        ...
        for priority in priorities.iter() {
            ...
            assert_eq!(
                dao_fee,
                (amount * DEFAULT_DAO_FEE_RATE / 10000).max(MINIMUM_DAO_FEE)
            );
