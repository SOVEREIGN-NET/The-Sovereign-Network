...
pub fn generate_key_image(private_key: &[u8]) -> Result<[u8; 32]> {
    ...
    let scalar = Scalar::from_bytes_mod_order_wide(&{
        let mut wide = [0u8; 64];
        wide[..private_key.len().min(64)]
            .copy_from_slice(&private_key[..private_key.len().min(64)]);
        wide
    });
...
pub fn scalar_to_point(scalar_bytes: &[u8]) -> RistrettoPoint {
    let scalar = Scalar::from_bytes_mod_order_wide(&{
        let mut wide = [0u8; 64];
        wide[..scalar_bytes.len().min(64)]
            .copy_from_slice(&scalar_bytes[..scalar_bytes.len().min(64)]);
        wide
    });
...
mod tests {
    ...
    fn test_curve_operations() {
        ...
        let point2 = bytes_to_point(&bytes)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
