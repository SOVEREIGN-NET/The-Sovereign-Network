//! Validator Discovery Protocol (CONS-501)
//!
//! Consensus-layer protocol for validators to announce themselves and discover
//! other validators. Validators publish their information for network-wide
//! discovery and consensus participation.
//!
//! Migrated from `lib-consensus/src/validators/validator_discovery.rs` per
//! CONS-501. The old path stays available as a re-export in
//! `lib_consensus::validators::validator_discovery` so internal callers in
//! `lib-consensus` keep compiling — `lib-network`, however, now imports
//! directly from this module (acceptance criterion: zero deep-imports of
//! `lib_consensus::validators::validator_discovery` from outside
//! `lib-consensus`).
