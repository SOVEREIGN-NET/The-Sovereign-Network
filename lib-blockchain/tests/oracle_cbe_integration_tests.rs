...
fn test_cbe_graduation_blocked_without_fresh_oracle_price() {
    ...
    harness.blockchain.bonding_curve_registry.register(token).unwrap();
    
    // Try to validate graduation without any finalized price
    let result = harness.blockchain.validate_cbe_graduation_oracle_gate(
        token_id,
...
fn test_cbe_graduation_accepted_with_fresh_oracle_price() {
    ...
    assert!(result.is_ok(), "should accept with fresh oracle price: {:?}", result);
}
