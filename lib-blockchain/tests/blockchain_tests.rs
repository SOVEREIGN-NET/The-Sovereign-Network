...
async fn test_finality_tracking_basic() -> Result<()> {
    ...
    assert!(
        !blockchain.is_block_finalized(1),
        "Block should not be finalized initially"
    );
    ...
    assert!(
        blockchain.is_block_finalized(1),
        "Block should be finalized after marking"
    );
...
async fn test_finality_depth_calculation() -> Result<()> {
    ...
    for block in finalized {
        assert!(
            block.header.height <= finality_height,
            "Block height {} exceeds finality height {}",
            block.header.height,
            finality_height
        );
    }
...
async fn test_register_validator_added_to_blockchain() -> Result<()> {
    ...
    assert!(
        blockchain.get_validator("validator_001").is_some(),
...
async fn test_is_validator_active_check() -> Result<()> {
    ...
    // Should be active
    assert!(
        blockchain.is_validator_active("validator_001"),
        "Registered validator should be active"
    );

    // Non-existent validator should not be active
    assert!(
        !blockchain.is_validator_active("validator_999"),
        "Non-existent validator should not be active"
    );
