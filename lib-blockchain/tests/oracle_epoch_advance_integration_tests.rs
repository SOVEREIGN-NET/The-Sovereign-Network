...
use lib_blockchain::oracle::{OracleConfig, OracleCommitteeState};
...
fn test_pending_committee_activates_at_epoch_boundary() {
    ...
    harness.schedule_committee_update(new_committee, target_epoch)
    ...
    assert!(harness.blockchain.oracle_state.committee.pending_update().is_none());
}
...
fn test_pending_config_activates_at_epoch_boundary() {
    ...
    let target_epoch = initial_epoch + 1;
    harness.schedule_config_update(new_config, target_epoch)
        .expect("config should be valid");
    
    // Config unchanged before epoch
    assert_eq!(harness.blockchain.oracle_state.config().epoch_duration_secs, initial_duration);
    
    // Advance to next epoch
    harness.advance_oracle_epoch();
    
    // Config changed after epoch boundary
    assert_eq!(harness.blockchain.oracle_state.config().epoch_duration_secs, initial_duration * 2);
    
    // Verify pending config update is cleared
    assert!(harness.blockchain.oracle_state.pending_config_update.is_none());
}
...
fn test_multiple_pending_updates_activate_correctly() {
    ...
    harness.schedule_committee_update(new_committee.clone(), initial_epoch + 1)
        .expect("schedule should succeed");
    harness.schedule_config_update(new_config, initial_epoch + 1)
        .expect("schedule should succeed");
...
fn test_committee_member_can_attest_after_epoch_advance() {
    ...
    assert!(result.is_ok(), "committee member should be able to attest after epoch advance");
}
...
fn test_stale_price_detection_after_epoch_advance() {
    ...
    let max_staleness = harness.blockchain.oracle_state.config().max_price_staleness_epochs;
    ...
    let fresh_price = harness.blockchain.oracle_state.latest_fresh_price(current_epoch);
    assert!(fresh_price.is_none(), "price should be stale after max_staleness epochs");
}
