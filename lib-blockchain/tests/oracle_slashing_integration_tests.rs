...
use lib_blockchain::oracle::{
    OracleAttestationAdmissionError, OracleSlashReason,
};
...
fn test_double_sign_is_rejected() {
    ...
    assert!(result.is_err(), "conflicting attestation should be rejected");
...
fn test_slashed_validator_cannot_attest() {
    ...
    assert!(result.is_err(), "slashed validator should not be able to attest");
}
...
fn test_slashing_preserved_across_restart() {
    ...
    assert!(!reloaded.oracle_state.committee.members().contains(&validator_key));
}
...
fn test_multiple_validators_can_finalize_after_slashing() {
    ...
    assert!(count >= 4, "should be able to finalize with remaining validators");
}
...
fn test_slash_event_contains_correct_metadata() {
    ...
    assert_eq!(harness.blockchain.oracle_slash_events.len(), initial_events + 1);
