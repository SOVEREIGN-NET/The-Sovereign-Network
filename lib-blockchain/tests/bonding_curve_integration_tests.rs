...
fn test_bonding_curve_full_lifecycle() {
    ...
    assert!(
        expected_tokens > 0,
        "Should receive tokens for stable input"
    );
    ...
    assert!(
        sim_result.price_impact_bps < 10000,
        "Price impact should be < 100%"
    );
...
fn test_multiple_tokens_different_phases() {
    ...
    assert_eq!(
        blockchain
            .bonding_curve_registry
            .count_by_phase(Phase::Graduated),
        1
    );
    assert_eq!(
        blockchain.bonding_curve_registry.count_by_phase(Phase::AMM),
        1
    );
...
fn test_curve_math_variations() {
    ...
    assert!(
        linear_price >= 1_000_000,
        "Linear price should be >= base price"
    );
    ...
    assert!(
        exp_price >= 1_000_000,
        "Exponential price should be >= base price"
    );
    ...
    assert!(
        sig_price <= 100_000_000,
        "Sigmoid price should be <= max price"
    );
...
fn test_amm_swap_invariants() {
    ...
    for i in 1..=10 {
        ...
        assert!(
            new_k >= initial_k || i == 1,
            "K invariant violated after swap {}: {} < {}",
            i,
            new_k,
            initial_k
        );
