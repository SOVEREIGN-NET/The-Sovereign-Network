...
fn test_oracle_committee_update_pipeline() {
    ...
    let new_committee: Vec<[u8; 32]> = harness.validators.iter()
        .map(|v| v.key_id)
        .collect();
    
    // Schedule committee update through oracle state (simulating governance execution)
    let target_epoch = initial_epoch + 1;
    harness.schedule_committee_update(new_committee.clone(), target_epoch)
...
fn test_oracle_config_update_through_governance_pipeline() {
    ...
    let target_epoch = initial_epoch + 1;
    harness.schedule_config_update(new_config.clone(), target_epoch)
    ...
    assert_eq!(harness.blockchain.oracle_state.config().max_source_age_secs, 300);
    
    // New epoch duration should affect epoch calculation
    let epoch_at_time = harness.blockchain.oracle_state.epoch_id(
        harness.current_timestamp + initial_duration * 3
    );
...
fn test_multiple_governance_updates_queue_correctly() {
    ...
    harness.schedule_committee_update(reduced_committee, initial_epoch + 1)
        .expect("schedule should succeed");
...
fn test_threshold_recalculation_after_committee_change() {
    ...
    assert!(finalized.is_some(), "should be able to finalize with new threshold");
}
