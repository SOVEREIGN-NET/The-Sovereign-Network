...
fn test_validate_cbe_graduation_oracle_gate_uses_latest_fresh_price() {
    ...
    assert!(err_msg.contains("no fresh finalized oracle price"), "Expected fresh price error: {}", err_msg);
}
...
fn test_cbe_graduation_rejects_missing_price() {
    ...
    let err_msg = result.unwrap_err().to_string();
    assert!(err_msg.contains("no fresh finalized oracle price"), "Expected no fresh price error: {}", err_msg);
}
