...
fn test_cbe_graduation_rejects_reserve_below_threshold() {
    ...
    assert!(
        result.is_err(),
        "Should reject when reserve below threshold"
    );
    let err_msg = result.unwrap_err().to_string();
    assert!(
        err_msg.contains("below threshold"),
        "Expected below threshold error: {}",
        err_msg
    );
}
...
fn test_cbe_graduation_skips_already_graduated() {
    ...
    assert!(
        result.is_ok(),
        "Should skip already-graduated tokens: {:?}",
        result
    );
}
