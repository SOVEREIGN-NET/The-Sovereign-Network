...
async fn main() -> Result<()> {
    ...
    for (i, ((name, &stake), &storage_gb)) in validator_names
        .iter()
        .zip(stakes.iter())
        .zip(storage_capacities.iter())
        .enumerate()
    {
    ...
    println!(
        ...
        hex::encode(proposal_tx.hash().as_bytes())
    );
    ...
    for round in 1..=5 {
        ...
        println!(
            "Round {}: Height {}, {} pending txs, {} validators active",
            round,
    ...
    if let Some(treasury_stats) = blockchain_guard.get_treasury_statistics().await.ok() {
        println!("Treasury:");
        println!(
            "   Current treasury: {} SOV",
            treasury_stats.current_treasury_balance
        );
        println!("   UBI fund: {} SOV", treasury_stats.ubi_fund_balance);
        println!(
            "   Sector DAO fund: {} SOV",
            treasury_stats.sector_dao_fund_balance
        );
        println!(
            "   Emergency fund: {} SOV",
            treasury_stats.emergency_fund_balance
        );
        println!(
            "   Dev grants fund: {} SOV",
            treasury_stats.dev_grants_fund_balance
        );
    }
    ...
    let ubi_citizens = vec![
        (
            IdentityId::from_bytes(&validator_keypairs[1].public_key.dilithium_pk),
            ubi_amount,
        ),
        (
            IdentityId::from_bytes(&validator_keypairs[2].public_key.dilithium_pk),
            ubi_amount,
        ),
        (
            IdentityId::from_bytes(&validator_keypairs[3].public_key.dilithium_pk),
            ubi_amount,
        ),
    ];

    let ubi_transactions = consensus_coordinator
    ...
    let welfare_transactions = consensus_coordinator
    ...
    println!(
        "   - Economics: UBI ({} tx) and welfare ({} tx) transactions processed",
        ubi_transactions.len(),
        welfare_transactions.len()
    );
