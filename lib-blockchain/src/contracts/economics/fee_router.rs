...
// ============================================================================
// SECTOR DAO WALLET KEY_IDS — registered 2026-04-10 via tools/dao-register
// key_id = blake3(dilithium_pk) — matches identity server derivation
// Source of truth: keys/dao-wallets.json (DO NOT DELETE)
// ============================================================================

pub const DAO_HEALTHCARE_KEY_ID: [u8; 32] = [
    0x9f,0x8d,0x4c,0xcf,0x10,0x86,0x9f,0x9a,0x95,0xb4,0xd3,0xe7,0x68,0x34,0xda,0x41,
    0x40,0xae,0xd7,0xfe,0x6a,0xf7,0xe8,0xaa,0x3a,0xf0,0xd7,0x07,0x4c,0x4a,0xc8,0x6c,
];
pub const DAO_EDUCATION_KEY_ID: [u8; 32] = [
    0xe4,0x61,0x1c,0xc1,0xa4,0xeb,0xf2,0x8a,0xd1,0x19,0xe0,0xc4,0x15,0xb4,0x30,0x0f,
    0x53,0xdd,0x97,0x70,0x3e,0xe6,0xaa,0x79,0x38,0x47,0xc4,0x0f,0xce,0xea,0x1d,0x5d,
];
pub const DAO_ENERGY_KEY_ID: [u8; 32] = [
    0xcb,0x4f,0xc8,0x6f,0x2f,0x01,0x77,0xf2,0x4b,0xe8,0x63,0x87,0x48,0x45,0xef,0x3e,
    0x45,0x60,0xdd,0x89,0xe9,0x81,0xd1,0xff,0x4f,0xe0,0x73,0x72,0xea,0x69,0x37,0x68,
];
pub const DAO_HOUSING_KEY_ID: [u8; 32] = [
    0xcc,0xf0,0xca,0xad,0x2c,0x80,0xe5,0x1c,0x2e,0x43,0xce,0xab,0x4a,0x8b,0x5d,0x1d,
    0x34,0xd4,0x91,0x4d,0xa5,0x75,0x34,0x69,0x7b,0xa8,0x00,0x3c,0xea,0x3a,0x4c,0xa2,
];
pub const DAO_FOOD_KEY_ID: [u8; 32] = [
    0x5a,0xd4,0xd3,0x6b,0x1d,0x3a,0x1a,0xa7,0x83,0xc0,0x5f,0x31,0x6d,0x67,0x30,0x9b,
    0x2b,0x60,0xde,0x4f,0xb7,0x63,0x24,0x42,0x82,0x37,0xdd,0x25,0xc2,0xf9,0xff,0xb4,
];
...
impl FeeRouter {
    ...
    /// Create a FeeRouter pre-initialized with the well-known sector DAO wallet addresses.
    ///
    /// The DAO key_ids are registered on-chain (2026-04-10, see keys/dao-wallets.json).
    /// UBI, emergency reserve, and dev grants use the same fee_sink address until
    /// dedicated pool wallets are registered.
    pub fn new_with_dao_wallets(fee_sink: [u8; 32]) -> Self {
        let make_pk = |key_id: [u8; 32]| crate::integration::crypto_integration::PublicKey {
            dilithium_pk: [0u8; 2592],
            kyber_pk: [0u8; 1568],
            key_id,
        };
        let mut router = Self::new();
        // Ignore error — only fails if already initialized (can't happen on a fresh instance).
        let _ = router.init(
            &make_pk(fee_sink),           // ubi_pool — fee_sink until UBI wallet registered
            &make_pk(fee_sink),           // emergency_reserve — same
            &make_pk(fee_sink),           // dev_grants — same
            &make_pk(DAO_HEALTHCARE_KEY_ID),
            &make_pk(DAO_EDUCATION_KEY_ID),
            &make_pk(DAO_ENERGY_KEY_ID),
            &make_pk(DAO_HOUSING_KEY_ID),
            &make_pk(DAO_FOOD_KEY_ID),
        );
        router
    }
