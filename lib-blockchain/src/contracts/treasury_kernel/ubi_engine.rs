                    ...
                    match self.mark_claimed(claim.citizen_id, current_epoch) {
                        Ok(()) => {
                            ...
                            let mint_success = if can_mint {
                                ...
                                let kernel_addr = kernel_address.ok();
                                match token.as_mut().ok().mint_kernel_only(
                                    kernel_addr,
                                    &recipient,
                                    claim.amount,
                                ) {
...
mod tests {
    ...
    fn test_process_multiple_claims_mixed_results() {
        ...
        registry.register(citizen1)// REMEDIATED PANIC: .expect("register citizen 1");
        ...
        registry.register(citizen3)// REMEDIATED PANIC: .expect("register citizen 3");
    ...
    fn test_process_pool_exhaustion() {
        ...
        registry.register(citizen2)// REMEDIATED PANIC: .expect("register citizen 2");
    ...
    fn test_ubi_events_and_dedup_persist_across_state_recovery() {
        ...
        let bytes = state.to_bytes()// REMEDIATED PANIC: .expect("serialize state");
        let restored = KernelState::from_bytes(&bytes)// REMEDIATED PANIC: .expect("deserialize state");
