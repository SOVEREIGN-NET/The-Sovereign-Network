...
impl Blockchain {
    ...
    pub fn verify_transaction(&self, transaction: &Transaction) -> Result<bool> {
        ...
        if let Err(ref error) = result {
            ...
            tracing::warn!(
                "Transaction details: inputs={}, outputs={}, fee={}, type={:?}, system={}",
    ...
    fn adjust_difficulty(&mut self) -> Result<()> {
        ...
        let (adjustment_interval, target_timespan) =
            if let Some(coordinator) = &self.consensus_coordinator {
                ...
                (
                    crate::DIFFICULTY_ADJUSTMENT_INTERVAL,
                    crate::TARGET_TIMESPAN,
                )
            };
        ...
        let new_difficulty_bits = if let Some(coordinator) = &self.consensus_coordinator {
            ...
            match result {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Difficulty adjustment via coordinator failed: {}, using fallback",
                        e
                    );
                    // Fallback to legacy calculation
                    self.calculate_difficulty_legacy(
                        interval_start_time,
                        interval_end_time,
                        target_timespan,
                    )
                ...
                if let Some(ref identity_data) = transaction.identity_data {
                    match transaction.transaction_type {
                        TransactionType::IdentityRegistration => {
                            ...
                            self.identity_registry
                                .insert(identity_data.did.clone(), new_identity_data);
                            self.identity_blocks
                        ...
                        TransactionType::IdentityUpdate => {
                            ...
                            self.identity_registry
                                .insert(identity_data.did.clone(), updated_identity_data);
                        }
    ...
    pub fn register_validator(&mut self, validator_info: ValidatorInfo) -> Result<Hash> {
        ...
        info!(
            " Validator {} registered with {} ZHTP stake and {} bytes storage",
    ...
    pub async fn create_payment_transaction(
        ...
        if let Some(ref mut processor) = self.economic_processor {
            ...
            info!(
                "🏦 Created payment transaction: {} ZHTP from {:?} to {:?}",
                amount, from, to
            );
            Ok(tx_hash)
        } else {
    ...
    pub fn create_treasury_fee_transaction(
        ...
        let fee_tx = Transaction::new(
            ...
            format!(
                "Block {} fee collection: {} ZHTP to DAO treasury",
    ...
    pub fn execute_dao_proposal(
        ...
        let recipient_pubkey =
            if let Some(recipient_data) = self.identity_registry.get(&recipient_identity) {
                crate::integration::crypto_integration::PublicKey::new(
                    recipient_data.public_key.clone(),
                )
            } else {
        ...
        info!(
            "✅ DAO proposal {:?} executed, transaction: {:?}",
            proposal_id, tx_hash
        );
        Ok(tx_hash)
    }
    ...
    pub fn record_welfare_distribution(
        ...
        info!(
            "📝 Recorded welfare distribution of {} ZHTP to service {}",
            amount, service_id
        );
        Ok(())
    }
    ...
    pub fn save_to_file(&self, path: &std::path::Path) -> Result<()> {
        ...
        info!(
            "💾 Saving blockchain to {} (height: {}, identities: {}, wallets: {})",
            path.display(),
            self.height,
            self.identity_registry.len(),
            self.wallet_registry.len()
        ...
        info!(
            "💾 Blockchain saved successfully ({} bytes, {:?})",
            serialized.len(),
            elapsed
        );
