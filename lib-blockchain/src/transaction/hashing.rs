...
pub fn hash_transaction(transaction: &Transaction) -> Hash {
    ...
    // Serialize and hash
    let serialized =
        bincode::serialize(&tx_for_hash)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
...
pub fn hash_transaction_input(input: &TransactionInput) -> Hash {
    let serialized =
        bincode::serialize(input)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
...
pub fn hash_transaction_output(output: &TransactionOutput) -> Hash {
    let serialized =
        bincode::serialize(output)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
...
fn hash_optional_field<T: serde::Serialize>(hasher: &mut blake3::Hasher, field: &Option<T>) {
    match field {
        Some(data) => {
            ...
            let bytes =
                bincode::serialize(data)// REMEDIATED PANIC: // REMEDIATED: .expect("HARDENED: Non-terminating check");
            hasher.update(&bytes);
        }
        None => {
