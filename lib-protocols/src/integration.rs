...
use crate::identity::{IdentityServiceConfig, ProtocolIdentityService};
#[cfg(feature = "storage")]
use crate::storage::{StorageConfig, StorageIntegration};
#[cfg(not(feature = "storage"))]
use crate::storage_stub::{StorageConfig, StorageIntegration};
...
impl ZhtpIntegration {
    ...
    pub async fn new(config: IntegrationConfig) -> Result<Self> {
        ...
        let identity_service =
            ProtocolIdentityService::new(identity_manager, IdentityServiceConfig::default());
    ...
    async fn validate_crypto(&self, request: &ZhtpRequest) -> Result<()> {
        ...
        if let Some(signature_header) = request.headers.get("X-ZHTP-Signature") {
            ...
            let signature_bytes = hex::decode(signature_header).map_err(|e| {
                ProtocolError::ZkProofError(format!("Invalid signature format: {}", e))
            })?;

            // Create public key from header
            let public_key_bytes = if let Some(pk_header) = request.headers.get("X-ZHTP-Public-Key")
            {
                hex::decode(pk_header).map_err(|e| {
            ...
            self.crypto.verify_protocol_signature(
                &request_data,
                &signature,
                &public_key.as_bytes(),
            )?;
        ...
        if let Some(zk_proof_header) = request.headers.get("X-ZHTP-ZK-Proof") {
            ...
            let zk_proof_bytes = hex::decode(zk_proof_header).map_err(|e| {
                ProtocolError::ZkProofError(format!("Invalid ZK proof format: {}", e))
            })?;
            ...
            self.crypto
                .verify_zk_proof(&zk_proof_bytes, &public_inputs)?;
        }
    ...
    async fn process_economics(
        &self,
        request: &ZhtpRequest,
    ) -> Result<crate::economics::EconomicAssessment> {
            ...
            if let Ok(dao_fee_amount) = dao_fee_str.parse::<f64>() {
                ...
                if dao_fee_amount < economics_assessment.dao_fee as f64 {
                    return Err(ProtocolError::DaoFeeError(
                        "Insufficient DAO fee".to_string(),
                    ));
                }
            }
        }
        ...
        let result = match request.method {
                ...
                if !request.body.is_empty() {
                    ...
                    let uploader_id =
                        if let Some(auth_header) = request.headers.get("Authorization") {
                            ...
                            lib_crypto::Hash::from_bytes(&lib_crypto::hash_blake3(
                            ...
                            let client_data = format!(
                                ...
                                request
                                    .headers
                                    .get("User-Agent")
                                    .unwrap_or_else(|| "ZHTP-Client".to_string())
                            );
                            lib_crypto::Hash::from_bytes(&lib_crypto::hash_blake3(
                    ...
                    let keypair = lib_crypto::KeyPair::generate().map_err(|e| {
                        ProtocolError::IdentityError(format!("Failed to generate keypair: {}", e))
                    })?;

                    // Generate DID for uploader
                    let uploader_did = format!(
                        "did:zhtp:temp-{}",
                        hex::encode(&uploader_id.as_bytes()[..16])
                    );

                    // Generate NodeId for uploader
                    let uploader_node_id = lib_identity::types::NodeId::from_did_device(
                        &uploader_did,
                        "uploader-device",
                    )
                    ...
                    let _uploader = lib_identity::ZhtpIdentity {
                        ...
                        wallet_manager: lib_identity::wallets::WalletManager::new(
                            uploader_id.clone(),
                        ),
                        ...
                        dao_member_id: format!(
                            "temp-{}",
                            hex::encode(&uploader_id.as_bytes()[..8])
                        ),
                    ...
                    #[cfg(feature = "storage")]
                    {
                        match self
                            .storage
                            .store_content(&request.body, metadata, uploader, request)
                            .await
                        {
            ...
            ZhtpMethod::Get => {
                ...
                let authenticated_identity =
                    self.identity_service.authenticate_request(&request).await?;
                ...
                match authenticated_identity {
                    Some(session) => {
                        ...
                        let requester_did = format!(
                            "did:zhtp:session-{}",
                            hex::encode(&identity_id.as_bytes()[..16])
                        );

                        // Generate NodeId for requester
                        let requester_node_id = lib_identity::types::NodeId::from_did_device(
                            &requester_did,
                            "requester-device",
                        )
                        .unwrap_or_else(|_| lib_identity::types::NodeId::from_bytes([0u8; 32]));
                        ...
                        #[cfg(feature = "storage")]
                        {
                            ...
                            let requester = lib_identity::ZhtpIdentity {
                                ...
                                dao_member_id: format!(
                                    "session-{}",
                                    hex::encode(&identity_id.as_bytes()[..8])
                                ),
                                dao_voting_power: 1, // Unverified human
                                citizenship_verified: false,
                                jurisdiction: None,
                            ...
                            match self
                                .storage
                                .retrieve_content(content_id, requester, request)
                                .await
                            {
                    ...
                    None => {
                        ...
                        StorageResult::Error(
                            "Authentication required for content access".to_string(),
                        )
                    }
                }
            }
    ...
    async fn process_blockchain_integration(&mut self, request: &ZhtpRequest) -> Result<()> {
        ...
        let _blockchain = Blockchain::new().map_err(|e| {
            ProtocolError::InternalError(format!("Failed to initialize blockchain: {}", e))
        })?;
        ...
        let sender_hash = request.requester.clone().unwrap_or_else(|| {
            ...
            let identifier = format!(
                ...
                request
                    .headers
                    .get("User-Agent")
                    .unwrap_or("ZHTP-Client".to_string()),
                request.timestamp
            );
        ...
        let keypair = lib_crypto::KeyPair::generate().map_err(|e| {
            ProtocolError::InternalError(format!("Failed to generate keypair: {}", e))
        })?;
        ...
        let transaction = TransactionBuilder::new()
            ...
            .map_err(|e| {
                ProtocolError::InternalError(format!("Failed to create transaction: {}", e))
            })?;
        ...
        validator
            .validate_transaction(&transaction_with_dao)
            .map_err(|e| {
        ...
        tracing::info!(
            "Blockchain transaction recorded: {} -> {}, amount: {}, DAO fee: {}",
            sender_hash,
            recipient_hash,
            economics_assessment.total_fee,
            economics_assessment.dao_fee
        );
        ...
        let (status, body) = match storage_result {
            StorageResult::Stored(contract_id) => {
                headers.insert(
                    ...
                    format!("/content/{}", contract_id),
                );
                (
                    ZhtpStatus::Created,
                    format!("Content stored with contract {}", contract_id).into_bytes(),
                )
            }
            ...
            StorageResult::Deleted => (ZhtpStatus::Ok, b"Content deleted".to_vec()),
            StorageResult::NotFound => (ZhtpStatus::NotFound, b"Content not found".to_vec()),
            StorageResult::Error(err) => (
                ZhtpStatus::InternalServerError,
                format!("Storage error: {}", err).into_bytes(),
            ),
            StorageResult::NoAction => (ZhtpStatus::Ok, b"Request processed".to_vec()),
        };
...
pub mod packages {
    ...
    pub async fn init_identity_integration() -> Result<()> {
        ...
        identity_integration
            ...
            .await
            .map_err(|e| {
                ProtocolError::IdentityError(format!(
                    "Failed to initialize identity integration: {}",
                    e
                ))
            })?;
    ...
    pub async fn init_consensus_integration() -> Result<()> {
        ...
        let _consensus_engine = ConsensusEngine::new(
            lib_consensus::ConsensusConfig::default(),
            broadcaster,
        )
        .map_err(|e| {
            ProtocolError::InternalError(format!("Failed to create consensus engine: {}", e))
        })?;
    ...
    pub async fn get_integration_health() -> HashMap<String, bool> {
        ...
        health.insert(
            "lib-crypto".to_string(),
            crate::crypto::ZhtpCrypto::new().is_ok(),
        );

        // Test lib-economy health
        health.insert(
            "lib-economy".to_string(),
            crate::economics::ZhtpEconomics::new(crate::economics::EconomicConfig::default())
                .is_ok(),
        );
        ...
        {
            health.insert(
                "lib-storage".to_string(),
                crate::storage::StorageIntegration::new(crate::storage::StorageConfig::default())
                    .await
                    .is_ok(),
            );
        }
