...
impl ZhtpEconomics {
    ...
    pub fn calculate_operation_fees(
        ...
        let dao_fee = if self.config.mandatory_dao_fees {
            let calculated_dao_fee =
                (adjusted_base_fee as f64 * self.config.dao_fee_percentage / 100.0) as u64;
    ...
    pub fn validate_dao_fee_payment(
        ...
        if self.config.mandatory_dao_fees {
            if paid_amount < expected_fee {
                return Err(ProtocolError::DaoFeeError(format!(
                    "Insufficient DAO fee: expected {}, paid {}",
                    expected_fee, paid_amount
                )));
            }
        }
        ...
        if payment_proof.is_empty() && expected_fee > 0 {
            return Err(ProtocolError::DaoFeeError(
                "Missing payment proof".to_string(),
            ));
        }
        ...
        if expected_fee > 0 {
            ...
            let calculated_fee =
                (paid_amount as f64 * self.config.dao_fee_percentage / 100.0) as u64;
            if calculated_fee < expected_fee {
                return Err(ProtocolError::DaoFeeError(format!(
                    "DAO fee calculation mismatch: expected {}, calculated {}",
...
pub mod utils {
    ...
    pub fn calculate_request_value(
    ...
    pub fn validate_transaction_format(transaction_data: &[u8]) -> Result<()> {
        if transaction_data.is_empty() {
            return Err(ProtocolError::EconomicError(
            ...
            match serde_json::from_slice::<Transaction>(transaction_data) {
                Ok(transaction) => {
                    ...
                    match validator.validate_transaction(&transaction) {
                        Ok(()) => Ok(()),
                        Err(e) => Err(ProtocolError::EconomicError(format!(
...
mod tests {
    ...
    fn test_fee_calculation() {
        ...
        assert_eq!(
            ...
            assessment.base_fee + assessment.dao_fee + assessment.network_fee
        );
    }
    ...
    fn test_dao_fee_validation() {
        ...
        assert!(economics
            .validate_dao_fee_payment(2, 100, payment_proof)
            .is_ok());

        // Insufficient payment
        assert!(economics
            .validate_dao_fee_payment(100, 50, b"proof")
            .is_err());
    }
