...
pub use types::{
    AccessPolicy, CachedContent, ContentMetadata, ContentSearchResult, ControlMessageType,
    DeliveryReceipt, DevicePayload, GroupChange, GroupEpochKey, GroupId, GroupMember, GroupMessage,
    GroupStateUpdate, IdentityEnvelope, IdentityMessageKind, IdentityPayload, MessageTtl,
    PoUwStamp, ReadReceipt, ReceiptType, SealedSenderEnvelope, ServerContent, StorageQuality,
    StorageRequirements, StorageSearchQuery, ZhtpHeaders, ZhtpMethod, ZhtpRequest, ZhtpResponse,
    ZhtpStatus, DEFAULT_GROUP_CAP, MAX_GROUP_CAP,
};

pub use zdns::{
    web4_integration, ZdnsConfig, ZdnsQuery, ZdnsRecord, ZdnsRecordType, ZdnsResponse, ZdnsServer,
};
pub use zhtp::{ServerConfig, ZhtpServer};

// API modules moved to zhtp crate - no longer exported from lib-protocols

// Re-export handler functions
pub use handlers::{
    handle_delete, handle_get, handle_head, handle_options, handle_post, handle_put, handle_verify,
    HandlerConfig, ZhtpHandlers,
};

// Re-export validation functions
pub use validation::{
    validate_access_policy, RateLimitConfig, ValidationCategory, ValidationConfig, ValidationError,
    ValidationResult, ValidationSeverity, ZhtpValidator,
};

// Re-export integration modules
use crate::types::ZHTP_VERSION;
pub use crypto::{CryptoConfig, ZhtpCrypto};
pub use economics::{EconomicAssessment, EconomicConfig, EconomicStats, ZhtpEconomics};
pub use identity::{
    IdentityAuthRequest, IdentityAuthResponse, IdentityServiceConfig, IdentitySession,
    ProtocolIdentityService,
};
pub use integration::{IntegrationConfig, IntegrationStats, ZhtpIntegration};
#[cfg(feature = "storage")]
pub use storage::{StorageConfig, StorageContract, StorageIntegration};
#[cfg(not(feature = "storage"))]
pub use storage_stub::{StorageConfig, StorageContract, StorageIntegration};
pub use wire::{
    read_request, read_response, write_request, write_response, ZhtpRequestWire, ZhtpResponseWire,
    WIRE_VERSION,
};

// Identity messaging helpers (Phase 2+)
pub use identity_messaging::{
    build_delivery_receipt_envelope, build_identity_envelope, build_identity_envelope_with_payload,
    build_identity_envelope_with_pouw, build_identity_envelope_with_retention,
    build_layered_envelopes, build_read_receipt_envelope, control_payload_from_device_update,
    create_delivery_receipt, create_group_state_update, create_pouw_stamp, create_read_receipt,
    derive_group_epoch_key, extract_device_ciphertext, next_group_epoch, seal_sender_envelope,
    validate_group_size, verify_delivery_receipt, verify_group_state_update, verify_pouw_stamp,
    verify_pouw_stamp_with_sender_did, verify_read_receipt,
};

// Re-export ZDNS functions
pub use zdns::{MeshNodeSelection, SecurityConfig};
