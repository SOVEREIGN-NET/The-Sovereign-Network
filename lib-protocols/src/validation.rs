...
use crate::types::{AccessPolicy, ZhtpHeaders, ZhtpRequest, ZhtpResponse};
...
impl ZhtpValidator {
    ...
    fn validate_protocol_compliance(&self, request: &ZhtpRequest) -> Result<()> {
        ...
        match request.method.as_str() {
            "GET" | "POST" | "PUT" | "DELETE" | "HEAD" | "OPTIONS" | "VERIFY" | "CONNECT"
            | "TRACE" => Ok(()),
    ...
    fn validate_zk_proof(&self, request: &ZhtpRequest) -> Result<()> {
        ...
        if self.config.test_mode {
            let zk_proof = request.headers.custom.get("X-ZK-Proof").ok_or_else(|| {
                ProtocolError::ZkProofError("ZK proof header missing".to_string())
            })?;
        ...
        let zk_proof =
            request.headers.custom.get("X-ZK-Proof").ok_or_else(|| {
                ProtocolError::ZkProofError("ZK proof header missing".to_string())
            })?;

        // Decode hex proof
        let proof_bytes = hex::decode(zk_proof).map_err(|_| {
            ProtocolError::ZkProofError("ZK proof must be valid hexadecimal".to_string())
        })?;

        // Basic proof format validation
        if proof_bytes.len() < 96 {
            return Err(ProtocolError::ZkProofError(
                "ZK proof too short (minimum 96 bytes)".to_string(),
            ));
        }

        // Get public inputs if provided
        let public_inputs = if let Some(inputs_header) =
            request.headers.custom.get("X-ZK-Public-Inputs")
        {
            hex::decode(inputs_header).map_err(|_| {
                ProtocolError::ZkProofError("Public inputs must be valid hexadecimal".to_string())
            })?
        } else {
        ...
        let crypto = crate::crypto::ZhtpCrypto::new().map_err(|e| {
            ProtocolError::ZkProofError(format!("Crypto initialization failed: {}", e))
        })?;

        let is_valid = crypto
            .verify_zk_proof(&proof_bytes, &public_inputs)
            .map_err(|e| {
                ProtocolError::ZkProofError(format!("ZK proof verification failed: {}", e))
            })?;
    ...
    fn validate_proof_context(&self, request: &ZhtpRequest, proof_bytes: &[u8]) -> Result<()> {
        ...
        if let Some(timestamp_str) = request.headers.custom.get("X-ZK-Proof-Timestamp") {
            ...
            if current_time.saturating_sub(proof_timestamp) > 300 {
                return Err(ProtocolError::ZkProofError(
                    "ZK proof is too old".to_string(),
                ));
            }

            if proof_timestamp > current_time + 60 {
                return Err(ProtocolError::ZkProofError(
                    "ZK proof timestamp from future".to_string(),
                ));
            }
        }
    ...
    fn validate_proof_context_fallback(
        &self,
        request: &ZhtpRequest,
        proof_bytes: &[u8],
    ) -> Result<()> {
    ...
    fn validate_dao_fee(&self, request: &ZhtpRequest) -> Result<()> {
        ...
        if self.config.test_mode {
            let dao_fee =
                request.headers.custom.get("X-DAO-Fee").ok_or_else(|| {
                    ProtocolError::DaoFeeError("DAO fee header missing".to_string())
                })?;

            // Parse fee amount (basic validation)
            let _fee_amount: f64 = dao_fee
                .parse()
                .map_err(|_| ProtocolError::DaoFeeError("Invalid DAO fee format".to_string()))?;

            // Check fee proof exists (basic validation)
            let _fee_proof = request
                .headers
                .custom
                .get("X-DAO-Fee-Proof")
        ...
        if let Some(dao_fee) = request.headers.get("X-DAO-Fee") {
            if let Ok(fee_amount) = dao_fee.parse::<f64>() {
                ...
                if fee_amount < min_fee {
                    return Err(ProtocolError::DaoFeeError(format!(
                ...
                if request.headers.get("X-DAO-Fee-Proof").is_some() {
                    ...
                    return request
                        .validate_dao_fee(&economic_model)
        ...
        Err(ProtocolError::DaoFeeError(
            "Invalid or missing DAO fee".to_string(),
        ))
    }
    ...
    fn validate_dao_payment_transaction(
        &self,
        request: &ZhtpRequest,
        fee_amount: f64,
        proof_bytes: &[u8],
    ...
    fn verify_payment_signature(
        ...
        if proof_bytes.len() < 64 {
            return Err(ProtocolError::DaoFeeError(
                "Proof too short for signature".to_string(),
            ));
        }
        ...
        let crypto = crate::crypto::ZhtpCrypto::new().map_err(|e| {
            ProtocolError::DaoFeeError(format!("Crypto initialization failed: {}", e))
        })?;
    ...
    fn validate_payment_proof_structure(&self, proof_bytes: &[u8]) -> Result<()> {
        ...
        if proof_bytes.len() < 128 {
            return Err(ProtocolError::DaoFeeError(
                "Payment proof structure invalid".to_string(),
            ));
        }
        ...
        if tx_hash.iter().all(|&b| b == 0) {
            return Err(ProtocolError::DaoFeeError(
                "Invalid transaction hash in proof".to_string(),
            ));
        }
        ...
        if block_hash.iter().all(|&b| b == 0) {
            return Err(ProtocolError::DaoFeeError(
                "Invalid block hash in proof".to_string(),
            ));
        }
        ...
        if signature.len() < 64 {
            return Err(ProtocolError::DaoFeeError(
                "Invalid signature in proof".to_string(),
            ));
        }
    ...
    fn validate_payment_freshness(&self, request: &ZhtpRequest) -> Result<()> {
        if let Some(payment_time_str) = request.headers.custom.get("X-DAO-Payment-Time") {
            let payment_time: u64 = payment_time_str
                .parse()
            ...
            if current_time.saturating_sub(payment_time) > 600 {
                return Err(ProtocolError::DaoFeeError(
                    "DAO payment is too old".to_string(),
                ));
            }

            if payment_time > current_time + 60 {
                return Err(ProtocolError::DaoFeeError(
                    "DAO payment timestamp from future".to_string(),
                ));
            }
        }
...
impl RateLimiter {
    ...
    fn check_ip_limit(&self, ip: &str) -> Result<()> {
        ...
        if counter.requests >= limit {
            return Err(ProtocolError::NetworkError(
                "Rate limit exceeded for IP".to_string(),
            ));
        }
    ...
    fn check_identity_limit(&self, identity: &str) -> Result<()> {
        ...
        if counter.requests >= self.config.requests_per_hour {
            return Err(ProtocolError::NetworkError(
                "Rate limit exceeded for identity".to_string(),
            ));
        }
...
pub fn validate_access_policy(
    ...
    if !policy.public && client_identity.is_none() {
        return Err(ProtocolError::AccessDenied(
            "Identity required for non-public access".to_string(),
        ));
    }
    ...
    if let Some(identity) = client_identity {
        ...
        if policy.denied_identities.contains(&hash_obj) {
            return Err(ProtocolError::AccessDenied(
                "Identity is denied access".to_string(),
            ));
        }
        ...
        if !policy.allowed_identities.is_empty() && !policy.allowed_identities.contains(&hash_obj) {
            return Err(ProtocolError::AccessDenied(
                "Identity not in allowed list".to_string(),
            ));
        }
    }
    ...
    if let Some(origin) = request.headers.custom.get("Origin") {
        if !policy.allowed_regions.is_empty() {
            let origin_allowed = policy
                .allowed_regions
                .iter()
                .any(|region| origin.contains(region));
            if !origin_allowed {
                return Err(ProtocolError::AccessDenied(
                    "Geographic region not allowed".to_string(),
                ));
            }
        }
    }
...
mod tests {
    ...
    use lib_proofs::{initialize_zk_system, ZkTransactionProof};
    ...
    async fn test_request_validation() {
        ...
        headers.custom.insert(
            "X-ZK-Proof".to_string(),
            "test_zk_proof_".to_string() + &"a".repeat(64),
        );
        headers
            .custom
            .insert("X-DAO-Fee".to_string(), "100".to_string());
        headers
            .custom
            .insert("X-DAO-Fee-Proof".to_string(), create_valid_fee_proof());
        ...
        let result = validator
            .validate_request(&request, Some("127.0.0.1"))
            .await;
        assert!(result.is_ok());
    ...
    fn test_access_policy_validation() {
        ...
        headers
            .custom
            .insert("Origin".to_string(), "https://example.zhtp".to_string());
