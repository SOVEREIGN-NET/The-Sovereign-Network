...
use crate::zhtp::{
    ServerCapabilities, ZhtpMiddleware, ZhtpRequestHandler, ZhtpResult, ZhtpServerEvents,
};
    ...
    loop {
        ...
        let mut request = match request_result {
            ...
            Err(_) => {
                ...
                let timeout_response =
                    ZhtpResponse::error(ZhtpStatus::RequestTimeout, "Request timeout".to_string());
        ...
        if request.method == ZhtpMethod::Options {
            ...
            cors_headers.set(
                ...
                "Content-Type, Authorization, X-Peer-Address".to_string(),
            );
...
async fn read_request(stream: &mut TcpStream) -> ZhtpResult<Option<ZhtpRequest>> {
    ...
    if content_length > 0 {
        ...
        let body_data_read = if total_read > body_start {
            total_read - body_start
        } else {
            0
        };
        ...
        if remaining_body > 0 {
            ...
            while body_read < remaining_body {
                let bytes_read = stream
                    .read(&mut buffer[total_read + body_read..total_needed])
                    .await?;
                if bytes_read == 0 {
...
fn format_lib_response(response: &ZhtpResponse) -> ZhtpResult<Vec<u8>> {
    ...
    {
        result.extend_from_slice(
            b"Access-Control-Allow-Headers: Content-Type, Authorization, X-Peer-Address\r\n",
        );
    }
